...
const LEVELS = [
  ...
  },
  {
    ...
    enemies: [{ type: "patrol", route: [[3, 1], [4, 1], [4, 2], [4, 3], [3, 3]], index: 0, forward: true }],
  },
  {
    name: "追跡者を誘う",
    size: 7,
  ...
  },
  {
    ...
    enemies: [
      { type: "sentry", x: 5, y: 2, dir: "down" },
      { type: "patrol", route: [[2, 3], [3, 3], [4, 3]], index: 0, forward: true },
    ],
  },
  {
    name: "横切る影",
    ...
    enemies: [
      { type: "patrol", route: [[2, 1], [3, 1], [4, 1], [5, 1]], index: 0, forward: true },
  ...
  },
  {
    ...
    enemies: [{ type: "sentry", x: 7, y: 5, dir: "left" }],
  },
  {
    name: "鍵石回廊",
    size: 9,
  ...
  },
  {
    ...
    enemies: [{ type: "sentry", x: 6, y: 6, dir: "left" }],
  },
  {
    name: "押し順の回廊",
    size: 10,
...
function isBlocked(x, y, ignoreDoor = false) {
  ...
  if (!ignoreDoor && state.gates?.some((gate) => same({ x, y }, gate) && !isGateOpen(gate))) return true;
  return false;
}
...
function sightCells(enemies = state.enemies) {
  ...
  enemies.forEach((enemy) => {
    ...
    while (!isBlocked(x, y, true)) {
      ...
      if (state.gates?.some((gate) => same({ x, y }, gate) && !isGateOpen(gate))) break;
      x += dir.x;
      y += dir.y;
    }
  });
...
function move(dirName) {
  ...
  if (pushedBlock) {
    ...
    if (isBlocked(nextBlock.x, nextBlock.y) || hasEnemyAt(nextBlock.x, nextBlock.y)) {
      playSound("miss");
  ...
  if (isBlocked(nextPlayer.x, nextPlayer.y) || hasEnemyAt(nextPlayer.x, nextPlayer.y)) {
    playSound("miss");
    return pulse(hasEnemyAt(nextPlayer.x, nextPlayer.y) ? "敵のいるマスには入れません" : "そこは通れません");
  }
...
function resolveTurn() {
  const occupied = state.enemies.some((enemy) => same(currentEnemyPos(enemy), state.player));
...
function render() {
  ...
  const nextEnemyCells = new Set(nextEnemies.map((enemy) => {
  ...
  for (let y = 0; y < state.size; y += 1) {
    for (let x = 0; x < state.size; x += 1) {
      ...
      if (currentSight.has(posKey(x, y))) cell.classList.add("sight", `sight-${currentSight.get(posKey(x, y))}`);
      if (nextEnemyCells.has(posKey(x, y))) cell.classList.add("next");
      if (state.won && same({ x, y }, state.goal)) cell.classList.add("escaped");
...
function addGlyphs(cell, x, y) {
  ...
  state.oneWays?.forEach((tile) => {
    if (same(p, tile)) cell.append(glyph("one-way", tile.dir === "right" ? "→" : tile.dir === "left" ? "←" : tile.dir === "down" ? "↓" : "↑"));
  });
  if (state.door && !state.door.open && same(p, state.door)) cell.append(glyph("door", "▥"));
  state.gates?.forEach((gate) => {
    if (same(p, gate)) cell.append(glyph(`gate ${gate.color}${isGateOpen(gate) ? " open" : ""}`, "▦"));
  });
  state.switches?.forEach((button) => {
    if (same(p, button)) cell.append(glyph(`switch ${button.color}`, "●"));
  });
  if (state.key && !state.key.taken && same(p, state.key)) cell.append(glyph("key", "⚿"));
  ...
  state.enemies.forEach((enemy) => {
    ...
    const mark = enemy.type === "patrol" ? "•" : enemy.type === "chaser" ? "!" : "◉";
