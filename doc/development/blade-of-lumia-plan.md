...
- [x] NPC タイル（NPC_A/B）・会話テキスト編集 UI
- [x] アイテムタイル（剣・たて・防具・ブーメラン・爆弾・ルピーなど）のパレット
- [x] 壊せる壁（breakDef 設定 UI）
- [x] 表示条件（showConditions）設定 UI（トリガー種別選択・追加パラメータ）
- [x] MAP_ENTER 遷移先設定 UI（出口ID方式）
- [x] ショップ商品設定 UI
- [x] ダンジョンレイヤー作成 UI（新規レイヤー追加・メタ情報設定）
- [x] エディタプレビューで全新規タイルをスプライト・絵文字フォールバック表示
- [x] ドアウェイタイル（DOORWAY/DOORWAY_BOSS/DOORWAY_LOCKED）の設定 UI（Phase 6.5 未完了分）
- [x] ステージ設定（ボス部屋フラグ・BGM上書き）
- [x] ゲームからのプレビュー連携（`fromEditor=1` URLパラメータ対応・デバッグモード自動ON）

> **実装メモ**：
> - `editor/index.html`・`editor/editor.css`・`editor/editor.js` を新規作成
> - レイヤー管理（field / dungeon_N の切り替え・追加・削除）
> - ワールドグリッドはミニマップ付きセル表示・行列挿入ボタン対応
> - 右パネルに全設定UIを縦スクロール配置（ゲート連動・宝箱・NPC・ショップ・MAP_ENTER・表示条件・壊せる壁・ドアウェイ）
> - File System Access API で `work/blade-of-lumia.json` に直接保存（フォールバック: ダウンロード）
> - `game.js` に `fromEditor=1` URL パラメータ対応を追加（localStorage からマップデータ読み込み）

---

### Phase 8: フィールドタイル拡張
> **目標**: 草原・山・町・家など屋外フィールドらしい見た目のタイルを追加し、コンテンツ制作を支援する

#### Phase 8.1: フィールド用タイルセット追加
> **目標**: 通行可・不可それぞれのフィールドタイルパターンを拡充し、ゼルダらしい屋外表現を実現

##### 追加タイル一覧

| カテゴリ | タイルID | 通行 | 内容 |
|---|---|---|---|
| 地形（通行可） | `GRASS` | ✅ | 草地（明るい緑。フィールドの基本地面） |
| 地形（通行可） | `SAND` | ✅ | 砂地・砂漠 |
| 地形（通行可） | `STONE_FLOOR` | ✅ | 石畳（町・城内） |
| 地形（通行可） | `BRIDGE` | ✅ | 橋（水の上を渡れる） |
| 地形（通行不可） | `TREE` | ❌ | 木（切れない・緑の丸い冠） |
| 地形（通行不可） | `MOUNTAIN` | ❌ | 山（三角形。灰色・茶色） |
| 地形（通行不可） | `BUSH` | ❌/✅ | 茂み（剣で切ると消える → ギミック用） |
| 地形（通行不可） | `FENCE` | ❌ | 柵（村の囲いなど） |
| 建物系 | `HOUSE_WALL` | ❌ | 家の外壁（レンガ調） |
| 建物系 | `HOUSE_DOOR` | ✅ | 家のドア（入口） |
| 建物系 | `HOUSE_ROOF` | ❌ | 家の屋根（赤・オレンジ） |
| 建物系 | `SIGN` | ✅ | 看板（近づくと読める） |
...
TILE.GRASS        = 'g'  // 草地（通行可）
TILE.SAND         = 'd'  // 砂地（通行可）
TILE.STONE_FLOOR  = 'o'  // 石畳（通行可）
TILE.BRIDGE       = 'v'  // 橋（通行可・水の上）
TILE.TREE         = 't'  // 木（通行不可）
TILE.MOUNTAIN     = 'M'  // 山（通行不可）
TILE.BUSH         = 'u'  // 茂み（通行不可・切れる）
TILE.FENCE        = 'f'  // 柵（通行不可）
TILE.HOUSE_WALL   = 'h'  // 家の外壁（通行不可）
TILE.HOUSE_DOOR   = 'e'  // 家のドア（通行可）
TILE.HOUSE_ROOF   = 'p'  // 家の屋根（通行不可）
TILE.SIGN         = 'q'  // 看板（通行可）
```

##### スプライト設計（8×8px ピクセルアート）

各タイルは 8×8px のピクセルアートスプライトとして `sprites.js` に追加。
ゲーム上は CSS 背景色 + スプライト重ね描きで表現。

| タイル | 背景色 | スプライト概要 |
|---|---|---|
| `grass` | `#3a6e28` | 明るい緑地に小さな草の葉3枚 |
| `sand` | `#c8a84a` | 黄土色地に細かいドット模様 |
| `stoneFloor` | `#6a6878` | 灰色地にレンガ目地ライン |
| `bridge` | `#8a6030` | 茶色の板2〜3枚（横断）+ 側面の柵 |
| `tree` | `#1a4810` | 濃い緑の丸い冠＋茶色の幹 |
| `mountain` | `#6a6060` | 灰色・茶色の三角形シルエット |
| `bush` | `#2a6018` | 小さな丸い茂みのシルエット（2〜3つ）|
| `fence` | `#8a6830` | 茶色の柵板と杭 |
| `houseWall` | `#c09060` | ベージュのレンガ壁、窓（青）|
| `houseDoor` | `#8a4020` | 濃い茶色のドア＋取っ手（黄） |
| `houseRoof` | `#c03020` | 赤〜オレンジ屋根タイル（斜め線）|
| `sign` | `#c09060` | 茶色の板と支柱、板に縦線装飾 |

#### Phase 8.2: 茂み（BUSH）ギミック
> **目標**: 茂みを剣で切ると消えて、ランダムでハートや回復薬が出現する

- ゼルダの伝説の草刈りギミックに相当
- 剣攻撃が茂みのセルに当たると `GRASS` に変化
- 一定確率でハート（HP+1）、低確率で回復薬（小）が出現
- ステージセーブに「切った茂み」を記録（永続化）

```js
// ドロップ確率
const BUSH_DROP_TABLE = [
  { type: 'heart',    chance: 0.12 },  // 12%: ハート（HP+1）
  { type: 'healPotion', chance: 0.04 },// 4%: 回復薬（小）
  // 残り84%: 何も出ない
];
```

#### Phase 8.3: 看板（SIGN）ギミック
> **目標**: 看板に近づいてAボタンで読める

- `TILE.SIGN` は NPC と同様のダイアログ表示
- エディタで `stage.signData["row,col"] = { lines: ["..."] }` で設定
- 見た目：`sign` スプライト（看板）を表示

#### Phase 8.4: コンテンツ制作
> **目標**: 実際に遊べる最低限のゲームとして完成させる

- [ ] スタートステージ設計（剣配置・チュートリアル的構成・初 NPC 配置）
- [ ] フィールドワールドマップ設計（複数エリア・草原・村・山）
...
- [x] **Phase 8.1**: フィールド用タイルを `tiles.js` に追加（GRASS / SAND / STONE_FLOOR / BRIDGE / TREE / MOUNTAIN / BUSH / FENCE / HOUSE_WALL / HOUSE_DOOR / HOUSE_ROOF / SIGN）
- [x] **Phase 8.1**: 各タイルの `TILE_META`（ラベル・色・通行フラグ・アイコン）追加
- [x] **Phase 8.1**: `sprites.js` に各タイルスプライト追加（8×8px ピクセルアート）
- [x] **Phase 8.1**: `game.js` の `tilePassable` / `addCellSprite` にフィールドタイル対応追加
- [x] **Phase 8.1**: エディタのタイルパレットに「フィールド地形」カテゴリを追加（通行可・通行不可・建物）
- [x] **Phase 8.2**: 茂みギミック実装（剣攻撃 → 茂み消滅 → ランダムドロップ：ハート12% / ルピー4%）
- [x] **Phase 8.2**: `stageState.cutBushes` に切った茂みを記録（saveGame/loadGame 対応）
- [x] **Phase 8.3**: 看板ギミック実装（正面で剣 → ダイアログ表示、`stage.signData` / `stage.npcData` 参照）
- [x] **Phase 8.3**: エディタの看板テキスト設定UI（NPC会話パネルで代替）
- [x] **Phase 8.4**: コンテンツ制作（マップ設計）
  - フィールド 5ステージ：草原×3・石畳の村×1・砂漠×1（山・柵・砂地を活用）
  - ダンジョン 2ステージ：謎の洞窟（MAP_ENTER・ボス部屋）
  - bgTiles で各エリアの背景地形を設定済み（草地・石畳・砂地）
  - 看板・NPC会話・ショップを各エリアに配置済み
  - 木・柵・山などの障害物オブジェクトを自然配置
