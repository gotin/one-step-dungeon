...
- [ ] **4.7. 合成パズルの再設計＝色ゲートを「プレイヤーの通路」に使って石4の天井を超える（🧠 Opus）**。**この位置の理由（2026-08-02・4.57 の帰結）＝4.6 の4枚で §2 下限条件（貪欲NG・deadlock>0）は実証できたが、「石4（§3-2c の上限）を超えて難化する軸」は 24,0 の撤回で未証明に戻った。5番（鍵の関門）で「鍵に与えられる難易度の上限」を決めるにはこの軸が要る∴5番の前に置く。ただし 4.6 の4枚だけでも 5番の下限設計はできる＝4.6 のユーザー判定待ちで手が空いたらここを進める。** **設計方針（PUZZLE-DESIGN §3-2d で確定済み・実装と実測が未了）：** 排他（色）を**石の搬送路**でなく**プレイヤーの立ち位置**の制約にする＝「赤にしないと石Aの押し位置に行けない／青にしないと石Bの押し位置に行けない」。押す順序が色の切替順に縛られ、切替は往復コストを持つ∴順序制約（§6.1）が生きる。**やること：**(1) 色ゲートがプレイヤー動線だけを跨ぐ幾何を作る（石のレーンは門を跨がせない）→(2) `generate-sokoban-playable.mjs` の合成テンプレを差し替え（現テンプレは ⚠️ 撤回バナー付きで残置）→(3) 4軸測定で激難（L=89・最短解2本）を**質で**超えるか確認＝L だけでなく「両色が必須」（穴の壁化による動的検査）と最短解の細さで判定→(4) `test_mechanics` の新座標へ書き込み（24,0 は回帰フィクスチャなので**上書きしない**）→(5) 実機で再生テスト→(6) ユーザー判定。**⚠️ 前提＝押し規則は 4.57 の2規則（門へは押し込めない／元セルの下地が閉じていたら押せない）。測ったソルバーで解けても実機で手順を再生して初めて成立。**
