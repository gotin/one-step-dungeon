...
- [ ] **15〜25 部屋へ拡張：** 現状12部屋（6×3 相当）を**16〜20部屋規模**に拡張（D2=16 と揃えるか、入門なので 16 でも可）。dungeon_1 の良い構造（最短線とは別の枝分かれ・ギミック教科書配置・地図`m`/コンパス`n`/看板`i`）は活かしつつ、改善メモ（[2,1]素通り廊下→足止め部屋に）を反映
- [ ] **足止め部屋（最低1・配線済み仕掛けのみ）：** 既存の `*→S→T`（[5,0]）・`Y→T`・色スイッチ（[3,0]）・`killAll`/`torchesLit`（[0,1]/[1,2]）は配線済みで使える。`|`は使わない。入門なので難度は控えめ（鍵`K`→扉`D` 1枚 or スイッチ1つ程度）
...
- [ ] **16部屋レイアウトを確定**（4×4 グリッド・辺スクロール接続）。役割割り当てを stage 座標へ：
  - **入口**（field 1,2 の `7,5` から入る。`>`で field へ戻る出口。看板`i`で「砂嵐の蠍王が眠る神殿」ガイド）
  - 雑魚部屋 ×3〜4（`E`/`C`・砂地テーマ・道中宝箱でルピー/回復薬）
  - **盾の出番部屋**（`F`SENTRY 1〜2体・遮蔽の柱。盾で槍を防いで前進）×1〜2
  - **石押し`*→S→T`ゲート部屋**（分岐の宝＝地図`m`/コンパス`n` ルートを開ける）×1
  - **ブーメラン入手部屋**（中盤の宝箱＝`chestContents` に `{type:'item', item:'boomerang'}`。ここまでは剣だけで到達可能に）×1
  - **ブーメラン必須・鍵回収部屋（案1）**：2マス幅の水/穴で隔てた対岸に `K`鍵 → ブーメランで回収（徒歩・盾・剣では取れない＝必須化）×1
  - **`D`扉部屋**（回収した鍵で開く＝ボスへの関門）×1
  - **中ボス`W`部屋**（撃破で `killAll` 連動の宝 or ルート開放）×1
  - **最奥ボス部屋**（`isBossRoom:true`・`:`DOORWAY_BOSS・ボス`N`・ボス宝に`Q`欠片②。**報酬ブーメランはここではなく中盤**）
  - 寄り道宝部屋 ×1〜2（ハートのかけら/ルピー大/回復薬。最短線とは別ルート）
- [ ] **必須化の検算（最重要）：** ブーメラン入手 → 鍵回収 → `D`扉 → ボスの一本道が、**ブーメラン無しでは絶対に突破できない**こと（対岸の鍵は徒歩/はしご/剣ビームでは取れない＝2マス幅水ではしご不可・剣ビームは鍵を回収しない）を確認。逆にブーメラン入手前の区間は剣だけで到達できることも確認
