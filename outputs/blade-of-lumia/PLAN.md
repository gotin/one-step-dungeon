...
- [ ] **5.5k. 通常陸上敵を拡充する＝新規15種を実装（既存 E/C/F と合わせ計18種）（🧠 Opus）**。**この位置の理由（2026-08-10 ユーザー指示）＝5.5j の通し判定で 10室中 dark_tower `[1,2]`（A 戦闘型③）だけが NG。原因は「通常陸上敵が3種（E/C/F）しかなく、最終盤の戦闘部屋に集める"最強格"の選択肢が無い」＝敵カタログの貧弱さそのもの。∴ 5.5j の最終合否（1,2 の再判定）と 6番（深洋O）より前に、まず敵種を増やす。**
      ...
      - **▶ 次の着手（DECISIONS.md 2026-08-10 の3段階）＝これは #8 単体でなく陸上敵15種の共通土台。** ①**エンジン基盤**：ENEMY_META に `directional:true` フラグ／スプライト名解決を単一ソース化する `resolveEnemySprite(e)`（攻撃中→`${base}${Dir}Atk`・構え中→`${base}${Dir}Guard`・通常→`${base}${Dir}`／プレイヤーの `getHeroSpriteName` game.js:457-466 が雛形）／通常敵経路 `enemyChase`（enemy-ai.js:487-530）で向き変化時に sprite 差替／敵の `_atkUntil`（攻撃＝**近接剣振り sword 型**・既存 enemy-ai.js:586-629 経路に乗せる）・`_guardUntil`（ガード＝**接近時に構える**・見た目演出でダメージ無効化はしない）。**まず既存の正面絵をエイリアスで4方向に割り当てた状態で機構を通し実機確認**（データとエンジンを分離）。②**量子化を共通スケール化**＝`quantize-shared.mjs` を「全ポーズ・全向きで共通の接地ライン＋共通BBoxスケール」に拡張（身長不揃いの根治）。③**向き別 R/U＋攻撃/ガードのスプライト生成**（SPRITE-PIPELINE.md・左は flipX）→ `shared/sprites-enemies.js` へ本登録 → 残り14種へ反復。
