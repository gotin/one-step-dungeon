...
- [ ] **5.5g4. ユーザー報告のバグ修正＝剣を出している間は移動できないようにする（⚡ Sonnet 可・0番＝割り込み枠）**。**この位置の理由＝5.5g3 で入れた「剣を構えたポーズ＋剣スプライト」の直後にユーザーが実機で報告＝「今は移動しながら剣を出せてしまい、移動してるにもかかわらず剣は移動しないので剣だけが宙に浮いてしまってる」。5.5g3 が作った実害 ∴ 次の鍵部屋作業（5.5h）より先に直す。**
