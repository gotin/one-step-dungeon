...
- [ ] **5.5e. D4 鍵部屋＝倉庫番型①（石3・純）＋`stonesPlaced` トリガー新設（🧠→⚡）**。部屋＝`dungeon_4 / 1,0`（**孤島**＝ワープで入る・部屋を丸ごと使える）。**先に `conditions.js` へ `stonesPlaced` を足す**（§7-5・既存 `allButtonsHeldByStones` を再利用）。盤面は `generate-sokoban-playable.mjs` の逆算生成＋`measure-puzzle.mjs` の4軸で **L 20〜35 帯・貪欲NG・deadlock>0・強制手率≤0.7**。`fluteEffect:{type:'resetStones'}` を付ける。**トリガー新設なので3経路ガード（描画・踏んで拾う・ブーメランで運ぶ）を再確認する。**
