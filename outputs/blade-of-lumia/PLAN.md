...
### 7-4. リスク・リワード設計　🧠→⚡（設計＝Opus完了／実装＝Sonnet）
> **設計確定済み（2026-06-22・DECISIONS.md「Phase 7-4（設計）」参照）。** ①②は**ゼロコードのデータ配置**、③ガチャは**既存ショップ（`ui.js:446-518`）に天井つき抽選分岐を1つ足すだけ**。固定価格ショップ＋`shopData` 機構は完成済み。次は ⚡ Sonnet で実装。
- **① 「扉を開けると強敵が出るが宝もある」選択要素（ゼロコード・配置のみ）：**
  - [ ] メインルートを塞がない**任意の脇道**に「強敵＋封印宝箱」を配置。宝箱を `showConditions={trigger:'killAll'}` でゲート（全滅で開封可能・既存の封印チェックがそのまま効く）
  - [ ] 入口に石碑（`signData`）でリスク予告（「奥に強き者あり、されど宝も眠る」）
- **② 高難度任意エリアへの報酬配置（ゼロコード・配置のみ）：**
  - [ ] 隠し通路（5-2）・色スイッチ部屋（5-1）・敵石押し（5-3）の先に上位ティア装備（伝説の鎧/ミラーシールド/聖剣＝7-1/7-2）やハートの器（7-3）を配置。「難所を解いた人だけが強化を得る」
- **③ ルピーで報酬を引くガチャNPC（天井つき）＝新規実装：**
  - [ ] `player.js openChest` の報酬付与 switch を **`grantReward(content)` に切り出し**、チェストとガチャで共通化（付与のみ。SE/メッセージ/saveGame は呼び出し側）
  - [ ] `shopData[posKey]` の good に **`gacha`型**（`price`/`pityCount`/`pityReward`/重み付き`pool`）を追加。`ui.js shopBuy` にガチャ分岐：ルピー減算→`++player.gachaPulls[key]`→**天井到達で確定レア＋カウンタ0リセット**／未満は重み抽選→`grantReward`→演出
  - [ ] 乱数は**ショップ factory に `random` 依存を注入**（既定 `Math.random`・テストで差し替え可）
  - [ ] `player.gachaPulls = {}`（**プレーンオブジェクト**＝save 自動対応）を player 初期化2箇所に追加
  - [ ] ガチャNPCをデモ配置（field 2,0 旅の商人の隣 等）。エディタUIは後追い可（まず JSON 直書きでデモ）
  - [ ] テスト：①`grantReward`切り出し後もチェスト付与が不変（回帰）②ルピー不足で引けない③`random`固定で特定枠（重み境界）④天井未満はプール・天井回で確定レア＋リセット⑤天井カウンタが save/load 保持⑥強敵部屋の封印宝箱は全滅前は開かず全滅後に開く
- [ ] ※ price/weight/pityCount の絶対値の最終チューニングは **Phase 8-4** に委ねる
