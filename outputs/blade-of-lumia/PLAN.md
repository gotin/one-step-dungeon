...
- [ ] **5.5i. dark_tower `4,3` ＝合成型（石4＋色ゲート・体験の頂点）＝合格済み `test_mechanics 26,0` を移植する（🧠 Opus）**。
      **✅ 方針確定（2026-08-09・ユーザー再確認）＝「一から生成する」のではなく、ユーザーが editor で作り 4.7 で合格した `test_mechanics/26,0`（石4＋色ゲート2＋色スイッチ2・L=106・貪欲NG・I4 両色必須・実プレイ合格）を dark_tower `4,3` の部屋へ移植する。** 26,0 が合格した基準＝**4軸＋I4（色ゲート1枚ずつ壁化→解なし）＋ユーザー実プレイ判定**。それ以上は要求しない。
      - **⚠️ 下の「🚧 v2 で一から生成する」記述（2026-08-08 まで）は❌失効。** これは**ユーザーが 26,0 を editor で作る直前に書かれた古い設計**で、26,0 成功後（4.7・DECISIONS 2026-08-05）に「26,0 を流用」で合意したのに上書きされず残っていた。前セッションはこの古い記述に従って生成器（`generate-key-room-dark-tower-43.mjs`）で二作業場を一から作ろうとし、**過剰な不変条件 I5（石が色ゲートのセルを通る遷移を禁止しても同 L）を課して袋小路**に入った。この I5 は誤り＝**合格前例 26,0 自身が I5 を通らない**（26,0 の最短解は石を開いたゲート越しに運ぶ・実測で確認済み 2026-08-09）。石がゲートに乗ったまま反対色へ切替える危険は `colorSwitchBlocked`（4.7 で実装済み・切替が不発になる）で既に解決済み∴「石はゲートを渡らない」を強制する必要は無い。**教訓＝「◯◯を流用」タスクは記述の手順を信じる前に◯◯の現物を開く（WORKFLOW.md Step 1-5）。**
      - **移植の実体（＝完全コピーではない・26,0 と 4,3 で違う所を合わせる）：**
        - **ゴールが違う。** 26,0 は「全ボタンON→ゲートT(8,5)開→宝箱(8,6)」。4,3 は**鍵 `K` を (7,6) に出す**＝`showConditions {"7,6":{trigger:"stonesPlaced"}}`（ゲートT・宝箱Bは置かない）。∴26,0 の (8,5)ゲートT/(8,6)宝箱/その周辺の壁は鍵(7,6)を出す形へ作り替える。
        - **入口/出口が違う。** 26,0 は test ステージ（editor スポーン）。4,3 は**北入口 (0,5)(0,6)（`4,2` から歩いて降りる）／南鍵扉 `DD`(9,5)(9,6) → `4,4`**。26,0 の連絡通路（row1 全幅）と縦穴(2,2)を、4,3 の北入口2セル＋南鍵扉へ繋ぎ替える。
        - **敵なし・`enemyDirs {}`・`floorItems {}`**（倉庫番の部屋に敵を置かない＝enemy-ai が石を押す）。`fluteEffect {type:"resetStones"}`（笛所持済み＝色の詰みも救済）。`links: []`。
        - **移植後に必ず 26,0 と同じ 4軸＋I4 で再測定する**（部屋の外周・入口・鍵位置を変えた分、L やゲート必須性が変わり得る＝数値を測り直して合格を確認する。L ≤ 60 の帯に収まらなければ壁を調整）。
      **🚧 ❌失効した旧記述（2026-08-08・26,0 作成前の設計＝上記の通り採らない。履歴として残す）**
      - **できているもの＝`scripts/generate-key-room-dark-tower-43.mjs`**（新規・未コミット・実マップは未変更）。D8 の生成器を土台に、テンプレ解析 `analyze()`／幾何の不変条件 `assertGeometry()`（違反を全部まとめて投げる）／逆算 `pullBFS()`／マクロ押し貪欲 `makeGreedyPush()`／4軸＋`noEscape` 測定 `measurePlacement()`／**色ゲート専用検査 `checkGateInvariants()`** を実装済み。`GEOM_ONLY=1` で幾何だけ数秒で回せる。移行スクリプト（`migrate-key-room-dark-tower-43.mjs`）は未着手。
      - **部屋の位相（実データで確認済み）**＝入口は北 (0,5)(0,6)（`4,2` から歩いて降りる・ワープではない）／もう一方の出口は南の鍵扉 `DD`(9,5)(9,6) → **`4,4`**（ステージキーは "x,y"＝列,行∴`4,3` の南は `4,4`。"行,列" と読むと「扉の先が無い」と誤診する）／東西の外周は全壁＝**終端相当**。鍵 `K` は (7,6) 据え置き。入場時の道具は全部（弓あり）。
      - **v1（対称ループ＋中央1本廊下）は却下**。幾何 assert は通り、測定も回った（例：石 `4,3 5,3 6,9` で L=44・貪欲NG・deadlock 464万・強制手率 0.09・`noEscape 0`）が、**I4 で落ちた**＝青ゲートを壁にしても L=51 で解けた＝ゲートが飾り。原因＝中央廊下 (5,3)〜(5,7) の途中に南北の枝（col5）があり、ゲートを通らず廊下へ入れた。**教訓＝色ゲートの必須性は「廊下の両端に置く」だけでは生まれない。ゲート以外の入口をゼロにするか、石が迂回路を確実に塞ぐ形にする必要がある。**
      - **測定コストの実測＝1候補あたり約50秒（700万状態）**。∴フル測定は `MAX_MEASURE` を絞り、**バックグラウンド実行してログを `.scratch/` に落とす**（1ターンに重い測定を詰めるとセッションが落ちる＝この中断の原因）。
      - **確認済みのエンジン事実（再調査不要）**
        - 色スイッチ `[`/`]` は**剣（隣接）と矢/ビーム**でセット（踏んでも作用しない）。**矢は最初に当たった色スイッチで消える**（`projectile.js:509-529`）＝1本の射線上の2個目は叩けない。矢を止めるのは `WALL` と未破壊 `!` **だけ**（`isTilePassableForProj`＝`projectile.js:159`）∴**閉じた色ゲート・石・`K` は貫通する**。`blade-solver.mjs` の弓モデル（最初の色スイッチで `break`）はこの挙動に忠実＝ソルバーを直す必要は無い。
        - **`ARRIVAL_WALL_TILES` に `SWITCH_RED`/`SWITCH_BLUE` が入っている（`game.js:1099`）**＝境界扉の**着地セルに色スイッチを置くと `4,4` から北へ戻る遷移が拒否される**∴鍵扉の手前 (8,5)(8,6) をスイッチにしてはいけない（設計中に踏みかけた罠）。
        - 石は色スイッチのセルに入れない（`player.js:480`・ソルバー `passableFor`）＝**色スイッチのセルは天然の石ブロッカー**として幾何に使える。
        - `colorSwitchBlocked`＝閉じる側のゲートに石が乗っていると色の切替が**不発**。∴ゲートのセルに石が乗り得る形は避ける。
        - 救済＝隣室へ出ると未解決の石だけ戻る（`activeColor` は戻らない）／笛 `resetStones` は石と `activeColor` の両方を戻す（`game.js:1540`）＝**色の詰みにも効く**（この部屋は笛所持済み）。
      - **設計方針の転換（確定）**＝生成器に書いた不変条件⑤「色ゲートを全部壁にしても入口から全床へ歩ける」は **I4（ゲートを壁にすると解けない）と論理的に衝突する**∴**捨てる**（[[invariant-that-defeats-gimmick-is-wrong]]＝不変条件のせいで設計が捻れたら疑うのは設計でなく条件・安全は状態空間で測る）。代わりに残すのは (a) 弱い版＝「ゲート閉のままでも色スイッチのどれかに手が届く（＝機構が発見可能・開始できる）」 (b) 実測 `noEscape === 0` (c) 笛。**同じ理由で不変条件⑧（解けた後もゲート抜きで鍵へ歩ける）も捨てる**（最後の一歩をゲートにするのが必須性の一番素直な作り方）。
      - **v2 の骨格（次セッションはこれをテンプレに落として `GEOM_ONLY=1` から回す）**
        - **中央制御列 col6**＝(2,6) に青スイッチ `]`（玄関ホール (1,6) と大通り (3,6) の両方から剣で叩ける）／(8,6) 付近に赤スイッチ `[`。**(3,6) から南へ矢を撃つと、閉じた青ゲート・`K` を貫通して赤スイッチに当たる**＝1か所で両色を切り替えられる「制御点」になる。⚠️ (8,5)(8,6) は鍵扉の着地セル∴**赤スイッチはそこに置けない**（上記 `ARRIVAL_WALL_TILES`）＝制御列の下端をどこに置くかは再設計が必要（例：鍵の真上 (6,6) 側に寄せる／シャフトを col6 から1列ずらす）。
        - **赤ゲート＝西作業場の入口／青ゲート＝東作業場の入口**（大通り row3 の両端 (3,2)/(3,9)）。石も buttons も両作業場に分けて置く∴**色を替えるには必ず中央へ戻る**＝非単調が構造として効く（倉庫番と色が絡む＝合成型の実体）。
        - **玄関ホール（安全地帯）は幅を絞る**（v1 のように row1 が全幅だと西と東が常時つながって色ゲートが無意味）。口は2つ以上・どちらも induction（口の真下の石を押し上げられない）を満たすこと。
        - **ゲートのセルへ石が入れないことを「測る」のでなく「幾何で保証する」**＝安全地帯と同じ帰納法を **`noStone` ゾーン（安全地帯 ∪ 色ゲート廊下）** に一般化する（生成器の `pushableInto(..., {fromOutsideSafeOnly})` をゾーン集合で呼べるように直す）。これで 4.7 の硬い制約と `colorSwitchBlocked` 不発の両方が静的に消える（現状の I5＝測定による確認より強い）。
        - ゲートは 1色1枚に縛らない（赤2枚・青2枚も可＝全部同時に開閉する）。**非単調さは「同じ色が別の場所を同時に開け閉めする」ことから作れる。**
      - **❌ 旧残作業（v2 テンプレ生成）は上記の通り失効。** 生成器 `generate-key-room-dark-tower-43.mjs` は破棄するか、移植盤面の 4軸＋I4 再測定ツールとして流用する（`analyze/assertGeometry/measurePlacement/checkGateInvariants` のうち**過剰な I5 部分だけ外して** I4＋4軸の測定に使える）。
      **✅ 実際の残作業（2026-08-09 確定・26,0 移植版）**
      1. 26,0 の盤面（石4/色ゲート2/色スイッチ2）を 4,3 の外周（北入口 (0,5)(0,6)・南鍵扉 (9,5)(9,6)・鍵 K (7,6)）へ**手で移植**したテンプレを起こす。ゴールは鍵(7,6)＝`stonesPlaced`。
      2. 移植盤面を**4軸＋I4 で再測定**（L ≤ 60 の帯・貪欲NG・deadlock>0・強制手率≤0.7／赤ゲート壁化→解なし・青ゲート壁化→解なし）。帯外なら壁を微調整して再測定（バックグラウンド実行・ログは `.scratch/`）。**I5（石ゲート禁止）は課さない**（26,0 が通らない過剰条件）。
      3. `migrate-key-room-dark-tower-43.mjs`（自己検査型・`showConditions {"7,6":{trigger:"stonesPlaced"}}`／`fluteEffect {type:"resetStones"}`／敵・`enemyDirs`・`floorItems` を全消し／`links:[]`）で `work/blade-of-lumia.json` の dark_tower `4,3` へ書き込む。
      4. `check-dungeon-integrity.mjs`＋`check-dungeon-connectivity.mjs` を緑にする → 実機 Playwright 再生テスト（セーブ注入＋「つづきから」・ワープセルを経路にしない・座標を丸めない）。
