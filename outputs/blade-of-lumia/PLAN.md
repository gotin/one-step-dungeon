...
>
> **⚠️ 進め方：** 規模（部屋数）は先に決めない。「どんな体験をさせたいか（連鎖の役割）」が決まってから従属的に決める。**ユーザーと対話的に詰めてから PLAN/DECISIONS に記録する。** 設計が重いので各ダンジョンの「最初の1つ」は 🧠 Opus、量産は ⚡ Sonnet。

### 9-1. 進行の背骨（連鎖マップ）の設計　🧠 Opus（対話的に詰める）　🔄 進行中
- [x] **進行構造を決定：ハイブリッド型（緩い連鎖）**（序盤一本道→中盤自由→終盤収束）
- [x] **序盤の連鎖を確定（村→D1→門番→D2）：**
  - [x] **何も持たずスタート。** 最初のステージに「最初の剣」を置き、**出口が剣の先にある＝通らざるを得ない（半強制ピックアップ）**。剣だけ・盾なしで弱い敵と戦う期間＝ゲームの基本を体得
  - [x] **dungeon_1（草原の洞窟）＝盾を覚える入門ダンジョン。** 報酬は **欠片① ＋ 木の盾（最下位ティア・ガードのみ・跳ね返しなし）** に絞る（進行アイテムは出さない）。跳ね返し付きの上位盾（鉄/ミラー）は後半ダンジョン報酬として温存
  - [x] **D1→D2 のゲートは NPC 門番方式。** 砂漠への道に門番 NPC を置き、D1 クリア（ボス撃破＝`defeatedBosses` にD1ボス）を確認したら通す。クリア前は「最初の試練を越えてこい」、クリア後は「よくやった、通れ」と台詞・通行可否を切り替え
  - [x] **dungeon_2（砂漠の神殿）＝最初の「道を開く」道具。** 報酬は **欠片② ＋ ブーメラン**。ブーメランで届かない鍵/遠隔スイッチを取って中盤ハブへ
- [ ] **中盤の連鎖を設計（dungeon_3/4/6 を任意順・はしご/弓矢/ロウソク/爆弾を分配）** — 次セッションで対話
- [ ] **終盤の連鎖を設計（dungeon_5・cave_1・dungeon_7 → 祭壇 → 翼の羽衣 → 塔 → ザーネル）** — 次セッションで対話
- [ ] **全アイテムの「初期所持 → ダンジョン報酬」への移設方針を確定**（現状は6種とも最初から拾える）

### 9-2. ダンジョンの作り込み（2部屋スタブ → 本物の多部屋ダンジョン）　🧠→⚡
- [ ] dungeon_1 を「本物のダンジョンの雛形」として基準化（既に12部屋ある。実プレイでレビューし良し悪しを言語化）
- [ ] 9-1 で決めた各ダンジョンの「役割（与えるアイテム・解放する先・テーマ）」に沿って dungeon_2〜7・cave_1 を多部屋ダンジョンに作り込む（複数部屋・鍵と扉の進行・既存ギミック配置・テーマ地形）
- [ ] ※ 各ダンジョンの規模（部屋数）は役割が決まってから決定する

### 9-3. 序盤チュートリアル動線の実装　🧠→⚡
- [ ] 何も持たないスタート → 最初のステージで剣を半強制ピックアップ（出口が剣の先）の地形を作る
- [ ] dungeon_1 で木の盾を入手 → ガードを使わないと進めない配置
- [ ] D1→D2 の NPC 門番（`defeatedBosses` 連動の台詞・通行ゲート）

> ※ 9-1 の中盤・終盤が未確定のため、9-2/9-3 の着手は 9-1 完了後。**Phase 8（やりこみ・UX）と Phase 8-4（バランス調整）は Phase 9 の後に回す**（ゲームの背骨が無い状態で UX やバランスを詰めても土台がブレるため）。なお Phase 8-1（ゲームオーバー体験改善＝フクロウのヒント／墓）は**スキップ**で合意（詰みポイントが少なく発火しにくい・ヒント手段は石碑/NPCで足りている）。
