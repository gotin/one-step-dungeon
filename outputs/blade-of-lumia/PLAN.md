...
- [ ] **4.56. 石パズルを解き終えたら石をロックして解き直しを強制しない（🧠→⚡・設計は下記＝実装は Sonnet でよい）**。**この位置の理由（2026-08-01 ユーザー割り込み＝4.55 の実機確認中に発案）＝4.6 の「理不尽でないか」判定の前提。石パズルは"解いた後のちょっとのミスで再挑戦を強いる"のが不親切＝理不尽さの除去は 4.6 のテストパズル体験に直接効く∴ 4.6 より前に入れる。** 
