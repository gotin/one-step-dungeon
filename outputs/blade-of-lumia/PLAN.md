...
#### ▶▶ 次セッションはここから再開する（2026-07-06 更新・設計①〜⑤完了）

> **土台是正＋★1〜★4 確定＋設計①〜⑤ すべて完了。次にやるのは ⑥＝各画面を1枚ずつ丁寧に設計して作り込む（🧠 Opus 設計・⚡ Sonnet は流し込みのみ）。⑤で確定した型（`FIELD-9-6-DESIGN.md` §8-5）を出発点に、地域ごとに1本の migration スクリプトへ手作業で意図を詰める（W1/W2 各110・素通り94 を潰す本丸）。次候補＝砂漠 or 草原ハブ。並行で設計④（外周110＝世界の果ての閉じ方・論点2）は 🧠 Opus。**
>
...
- [ ] **9-6-⑥ 各画面を1枚ずつ丁寧に設計して作り込む（🧠 Opus 設計・⚡ Sonnet は確定データの流し込みのみ）：** **⚠️「量産」ではない（2026-07-06 ユーザー指摘・9-2 ダンジョンの PLAN:955 と同じ鉄則をフィールドにも適用）。** 各画面は「なぜこの画面が存在するか（設計①2軸以上）」を1枚ずつ考えて作る。テンプレ（§4）と密度基準（§8）・自己検証スクリプトは**設計の出発点と安全網であって、コピペのスタンプではない**（dup検出が同一配置を禁じる＝コピペは物理的に不可）。地域ごとに1本の migration スクリプトにまとめるのは接続不変を担保する"作法"であって、中身の各画面は手作業で意図を詰める。**⚡ Sonnet に回してよいのは、Opus が1枚ずつ決めた配置データの流し込み実装だけ。**
  ...
  - **▼ ⑥ 残り地域の確定作業順（番号順にこなせば 9-6 が完了する・選択の余地なし）。** 順序原則＝**攻略順（プレイヤーが道具を得て通る順）**に沿わせる（予告編は「到達時に所持済みの道具」で決まる§8-1＝攻略順に作れば道具依存が自動解決）＋**各地域は「その地域＋接する外周辺」を1セットで**（§10・外周だけ先行は不可）＋草原G(111)は攻略の節目で3ブロック分割。各タスク＝1 migrationスクリプト・完了条件は§8-5（接続緑＋該当不変条件が下がる＋BASELINE締め＋実ブラウザboot）。**各着手時に §4/§8/§9 と本リストを読む。**
    ...
    - [ ] **⑥-4 草原G-B 中央〜東・湖ぎわ（row6-12の中央≒40画面）** 🧠 Opus。D3湖入口`9,9`手前＝**弓ゲート予告編（D3手前・弓はD3報酬なので"再訪近道"扱い／代わりに石押し・柵分岐）**。湖Wとの境界を噛み合わせ。**外周撲滅済み＝接する外周辺は playable、本土側の trap を潰す。**
    - [ ] **⑥-5 草原G-C 北・火山/雪ぎわ（row1-5の北≒36画面）** 🧠 Opus。D4火山入口`12,2`・D5雪入口`13,5`手前＝**かがり火予告編（ロウソクはD4報酬∴D4手前に置ける）**。塔`T`/村`K`北辺を温存。
    - [ ] **⑥-6 湖 W（14画面）** 🧠 Opus。9-4で島橋化済み＝**はしご水渡り予告編が既存**。P1島渡りが顔。**接する外周＝東の海`~`をセットで。**
    - [ ] **⑥-7 山地/沼 M（16画面）** 🧠 Opus。D8沼入口`10,14`・cave_1`9,15`手前＝**笛reveal予告編（笛はD8報酬∴"再訪近道"／代わりに1本道関門・石）**。**接する外周＝東/南の海`~`をセットで。**
    - [ ] **⑥-8 雪 S（14画面）** 🧠 Opus。D5雪の地域＝はしごはD5報酬∴予告編にせず石/迷路が顔。**接する外周＝北東の海`~`をセットで。**
    - [ ] **⑥-9 火山 L（8画面）** 🧠 Opus。D4火山の地域＝`~`溶岩スキン+`v`橋足場（湖と同型・見た目のみ火山色）。**接する外周＝北の山`^`をセットで。**
    - [ ] **⑥-10 外周山 ^（残り≒26画面）** 🧠 Opus。内陸に接しない`^`内側＝`M`断崖の1本道・断崖奥の隠しcave（§10）。外周海1地域(⑥-11)の前に密度を試作で実測。
    - [ ] **⑥-11 外周海 ~（残り≒79画面）** 🧠 Opus。内陸に接しない`~`内側＝歩ける渚+`v`桟橋+`#`難破船+隠しcave（§10）。**最大ブロック＝密度基準を試作で先に確定してから流し込み。** **✅ ハブ南岸5枚（`3,17`/`4,18`/`5,19`/`6,19`/`7,18`）は ⑥-3 で「地域＋外周セット」として作り込み済み**（渚+桟橋+隠しの型＝`migrate-field-grassland-a.mjs` の sea-interior builder が外周海の型の第1試作＝密度基準の実測サンプル）。残り79は他地域の ⑥ で接する分＋内陸非接の内側。
    - [ ] **⑥-trap 全マップ「壁に辺が開いている」根絶チェック＋修正（全地域完了後の最終確認・🧠 Opus）＝2026-07-10 ユーザー指示で独立タスク化。** 各地域リワーク（⑥-4〜⑥-9）に subtask として溶かした trap 潰しは**そのまま維持**した上で、**全320画面を横断して arrival-wall（到達可能画面の開いた辺→着地セルが徒歩不可の壁＝クランプ/めり込み＝すり抜けの元）を機械的に全列挙し、残り全件をマップ側で是正する**（＝「そもそも壁に向かって辺を開けない」正しい地形にする＝ユーザー言「そんなマップを作るな」の本筋）。**現状 traps=97**（`field-invariants` の ratchet で測定中・BASELINE から単調減少）。各地域リワークで大半は落ちる想定だが、地域境界の噛み合わせ漏れ・温存画面の角セムが最後に残るので**全地域後に必ず横断で0を確認・残件を潰す**。**⚠️ engine 側のすり抜け防止（`checkStageTransition` の着地 footprint 壁チェック・2026-07-10 実装済み）は安全網であって本タスクの代替ではない**（安全網があってもプレイヤーは「壁に向かって進めてしまう→手前で不自然に止まる」体験をするので、マップから arrival-wall を消すのが本筋）。完了条件＝`field-invariants` の **traps=0**（該当テストが目標0で緑）。
    - [ ] **⑥-完了検査：** `field-invariants` で **W1=0/W2=0/seam=0/traps=0/under-2-axis=0/dup=0** ＝全320画面プレイ可能かつ「壁に辺が開いた画面0」の機械的証明（ユーザー確定の"最後のチェック"§10-4）。全ブラウザboot・connectivity緑。
