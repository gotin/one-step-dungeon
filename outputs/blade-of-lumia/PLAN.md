...
- **最終ダンジョン（暗黒の塔 dark_tower）は上記カーブの上限（25）も超えて明確に広く**作る（ラスダンの風格・28室以上を目安）。Phase 1-5 で作った dark_tower は2フロアスタブなので、9-2 終盤で本格拡張する。
...
- [ ] D2 のテンプレ（15〜25部屋・辺スクロール網・配線済み仕掛けのみ・**そのダンジョンの報酬を中盤で入手→必須化部屋→ボス**・(A)ソフトロック厳禁・足止め最低1・欠片はボス撃破）に沿って 1 つずつ作り込む。各ダンジョンの必須化ギミック（配線確認済み）：
  ...
  - **D8 沼地（笛）※レイヤー新設：** 笛中盤入手 → `flutePlayed` で隠し入口/通路を出して奥へ。ボス＝沼地の大蝦蟇 `I`（部品は 9-2c 作成済み）。入場は はしご。**`dungeon_8` を新設して配置**（cave_1 ではない＝位置づけ修正 2026-06-24）。テーマ・フィールド入口はゼロから設計
...
#### 9-2g. dungeon_6「森の聖域」設計確定（D6＝爆弾報酬・中盤最終／⚠️ 進行順は D4→**D6**→D5→D8→D7）　🧠 Opus 設計確定（2026-06-27・コード/JSON 変更なし）
...
- [ ] **新規ゲームコードは不要であることを確認（D5 との最大の違い）：** D6 の関門は **D4 で実装済みの `torchesLit`→隠し `>`テレポート** と **D5 で使った `!`BREAKABLE_WALL（爆弾）** の組合せ＝**新サブシステム・新ボスギミック・接続チェッカー拡張すべて不要**。ボス`O` も `shared/enemies.js:274-297` に定義済み（fire×2 弱点・hitAndAway・dropsTriforce）＝そのまま配置するだけ。**→ 流し込みは純粋な JSON 編集＋テスト座標調整のみ（⚡ Sonnet 向き）。**
- [ ] **接続グラフの機械検証**（`check-dungeon-connectivity.mjs dungeon_6` → PASS：orphan 0・dead edge 0・全22室到達／`--block-room=1,1` で boss `[0,0]` 到達不能＝かがり火関門が成立。`--with-ladder` は不要＝D6 は水/穴を臨界経路に使わない）
- [ ] **整合性チェッカーを通す**（`check-dungeon-integrity.mjs dungeon_6` → エラー0）。**現状エラー4（bossStage 未設定／triforceId 未設定／地図 `m` 無し／コンパス `n` 無し）は流し込みで解消。`UNLOCKED_AT.dungeon_6=['boomerang','bow','candle','bomb']`・`TRIFORCE_BOSS_TILES['O']` は登録済み＝チェッカー本体の変更不要。** `!`壁が出口を封鎖していても D6 は bomb 解禁済みなので MUST エラーにならない（WARNING 想定＝意図確認）
- [ ] **dungeon_6 の layer メタを整備：** `bossStage:"0,0"` / **`triforceId:5`**（⚠️ 連鎖表の「欠片⑤」＝**triforceId は 5**。dungeon_5 は triforceId=6 で既に確定済みなので番号衝突なし＝氷D5=⑥／森D6=⑤の "数字とIDが入れ替わる" 進行順を踏襲）/ `name:"森の聖域"`（field 看板と一致・既設）/ `bgm:"dungeon"` / `bossBgm:"boss"`。旧 `1,0` の `isBossRoom:true` を `0,0` へ再配置（heart-containers.spec.js は `dungeon_6 / 1,0` を期待＝⚠️ ボス部屋を `0,0` にするならテストの `1,0`→`0,0` 修正が必要＝D4/D5 と同じ対応）
- [ ] **テスト整備（流し込みと同時・⚡ Sonnet）：** 既存の `candle-gate.spec.js`（D4 で新設・`torchesLit` を実エンジンで固定）が D6 の関門もカバーする＝**新規テスト不要**（D6 専用フィクスチャは作らず既存 `test_mechanics/candle_gate` を流用）。流し込み時に修正が要るのは **(a) heart-containers.spec.js の `dungeon_6` ボス部屋 `1,0`→`0,0`**、**(b) lore-tablets.spec.js の `dungeon_6 / 0,0 / [1,5]` 石碑をボス部屋 `[0,0]` に確実に置く**の2点のみ
- [ ] **必須化の検算（最重要）：** 爆弾無しでは `[1,2]→[1,1]` の `!`壁を割れず関門へ入れない／ロウソク無しでは `[1,1]` のかがり火が点かず隠し `>`テレポートが出ず `[1,0]/[0,0]` 到達不能（接続チェッカー `--block-room=1,1`）。`fromEditor=1` で「爆弾で!壁→ロウソクでかがり火全点灯→テレポート→中ボス→鍵→D扉→ボスを炎/剣で撃破→欠片⑤→field 帰還」を実プレイ確認
- [ ] **(A) ソフトロック検算：** D6 内で後ダンジョン報酬（はしご/笛）を要求しない（`~`水・`x`穴を臨界経路に置かない／隠し通路は `torchesLit` で出す＝`flutePlayed` を使わない）。入場のロウソクは前ダンジョン D4 の報酬＝(A) OK。爆弾入手前に `!`壁を臨界経路へ置かない
- [ ] **アイテム移設（9-2a 表 #2）を反映：** D6 `[1,2]` に爆弾宝箱を配置し、`field/1,1 floorItems["4,6"]`(count3) と `field/2,0 shopData["6,7"]` の爆弾入手を撤去（爆弾は D6 報酬に集約＝補充ショップの是非は後で別途検討）。`field/1,1 tile(row4,col6)=5` の爆弾タイルも撤去
...
- [ ] 各ダンジョンの作り込み完了後、🧠 Opus で以下の3段階レビューを実施：
...
- [ ] レビュー対象ダンジョン（作り込み完了次第追加）：
  - [ ] dungeon_1「草原の洞窟」（入門・木の盾・G）
  - [ ] dungeon_2「砂漠の神殿」（ブーメラン・N）
  - [ ] dungeon_3「水の迷宮」（弓矢・J）（✅作り込み済み 2026-06-25 → Opus 3段階レビュー待ち）
  - [x] dungeon_4「炎の神殿」（ロウソク・A）（✅流し込み完了 2026-06-25・9-2e）
  - [ ] dungeon_5〜8（D8 沼地＝新設レイヤー）（作り込み後）
...
- [ ] 既存テストへの影響確認（stageState を参照するテストが `visited` フィールドに反応しないか）
