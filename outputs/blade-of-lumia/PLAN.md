...
- [ ] **1. テストの flaky 2件を安定化する**（⚡ Sonnet・`tests/field-corridor-o.spec.js` ⑨／`tests/sea-lord-boss.spec.js` ⑥b）
      **症状（2026-07-28 実測）＝フル実行4回で ⑨ が2回・⑥b が1回赤、単独実行では両方緑・同じ組み合わせでは再現しない＝並列負荷依存。** ⑨ は28秒かかる実エンジン全探索・⑥b は tick 依存の移動検証∴タイムアウト境界か tick 待ちの取りこぼしが疑い。
      **なぜ1番＝以降のタスクの完了条件が「全テスト緑」なので、赤が揺れる状態では何も判定できない。** 特に 2番はエンジンの通行判定を変える＝実エンジンテストの結果が唯一の安全網。
      **完了条件＝フル実行を3回連続で全緑**（`npx playwright test` を3回）。直せない場合も「なぜ落ちるか」を突き止めて DECISIONS に記録し、`test.slow()` などの明示的な手当てを入れる（retry で隠すのは禁止）。

- [ ] **2. ⑥-landing 辺遷移の着地を整数セルにする＋「閉じた門セルへの着地は拒否」をセットで入れる**（🧠 Opus・詳細は下記 9-6 ⑥-landing の節）
      **なぜ2番＝残 67件の見えない壁を一括で消し、以後の全画面作り込みが同じ罠を踏まなくなる∴3番（デルタ14画面）より先にやるほうが手戻りが小さい。** 逆順にすると、デルタ14画面を「半セル着地の作法」で作った直後に着地規則が変わる。

- [ ] **3. 深洋O ④＝25画面配置の残り 14画面（デルタ）**（🧠 Opus・詳細は下記 9-6 ⑥ 深洋O の節）

- [ ] **4. ⑥-完了検査**（下記 9-6 ⑥-完了検査の節＝全指標 0 の機械的証明）

- [ ] **5. 9-6-BASE 外周解体＋隣接地域編入**（外周 105画面の格上げ・残 67件の footprintBlocked もここで 0 になる想定だが、2番を入れた時点で先に 0 になる）
  ...
  - **▼ ⑥ 残り地域の確定作業順（番号順にこなせば 9-6 が完了する・選択の余地なし）。** 順序原則＝**攻略順（プレイヤーが道具を得て通る順）**に沿わせる（予告編は「到達時に所持済みの道具」で決まる§8-1＝攻略順に作れば道具依存が自動解決）＋**各地域は「その地域＋接する外周辺」を1セットで**（§10・外周だけ先行は不可）＋草原G(111)は攻略の節目で3ブロック分割。各タスク＝1 migrationスクリプト・完了条件は§8-5（接続緑＋該当不変条件が下がる＋BASELINE締め＋実ブラウザboot）。**各着手時に §4/§8/§9 と本リストを読む。**
    ...
    - [x] **🔴🔴 ⑥-footprint 着地判定を「エンジンと同じ 2×2 footprint」に揃える＝全72件の見えない壁を根絶（🧠 Opus・2026-07-27 ユーザー実プレイで発覚）** — **2026-07-28 完了（深洋O ぶん 5件を 0 に・検査は全マップに常設）**
      ...
      **やったこと（この順で・順序に意味があった）：**
      ...
      5. ⏳ **残り 67件（field 既存外周）を段階的に。** 全部が塗り絵外周の `M`／`~`＝**9-6-BASE の外周解体＋隣接地域編入**と同時に落とす。dungeon_7 `3,2→3,3 @0,6='x'` は単独で直せる（`FOOTPRINT_BASELINE` で固定済み）。
    ...
    - [ ] **🔴 ⑥-landing 辺遷移の着地を整数セルにする＋「閉じた門セルへの着地は拒否」をセットで入れる（🧠 Opus・2026-07-28 ユーザー指摘で実測）**
      ...
      **∴ 入れ方（セット必須）：**
      1. 着地を整数に（下入り `newRow = 0` / 上入り `rows-1` / 右入り `newCol = 0` / 左入り `cols-1`、押し戻しも対称に）。
      2. **相棒ルール＝着地セルが「閉じた解ける門」なら遷移を拒否する。** 例外＝`':'`（DOORWAY_BOSS は初期 open・入室後に閉じる）。
      3. 相棒ルールの到達性コスト実測＝**field 319/319・他ダンジョン全て不変**、唯一 **dungeon_7 `0,0`（ボス部屋・入口が鍵扉 `D` のみ）だけを失う** ∴「鍵を持っている扉は通す」carve-out かデータ側の手当てが必要。
      4. 検査側も同時に：`connectivity.mjs edgeLanding` を整数へ・`connectivity-tool.spec.js` の期待値（`'0.5,5'`／`10.5` 等）更新・`BASELINE.footprintBlocked` と `FOOTPRINT_BASELINE` を **0** に締める（ratchet 自体は残す＝チェッカーは新着地で無違反を証明する形になる）。
      5. 回帰テスト＝**門境界の crossing を実エンジンで歩く**（閉じた門へは入れない／開けたら入れる）を新規追加。上の 15件から各クラス1本ずつ。

      ⚠️ **エンジンの着地を変えるので影響は全マップ**。VRT・`field-corridor-o` 等の実エンジン系を全部通してからでないと確定しない。
