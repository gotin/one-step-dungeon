...
#### ▶▶ 次セッションはここから再開する（2026-07-05 更新・★確定済み）

> **土台是正は完了（下記チェック済み）。★1〜★4 も 2026-07-05 に確定した（下表）。次にやるのは「設計①面白さの言語化＋地域テンプレ設計」＝いきなり画面を触らない。**
>
...
>
> **∴ 実体の進行順：** ①面白さの言語化 → ②**地域テンプレ設計**（320画面を塗り絵にせず均等に意味づけるには量産テンプレが先） → ③honest指標の不変条件テスト化（W1=0・W2=0・シーム=0 の3つとも red 基準に＝★4確定で一意に定まった） → ④1地域を理想形で試作し密度基準を確立 → ⑤⚡ Sonnet で全地域量産。
>
> **⚠️ テンプレ・密度基準が固まる前に流し込まない**（また塗り絵になる）。設計①②が済むまで画面データ（work/blade-of-lumia.json）は触らない。
>
> **（旧 ★1〜★4 の分岐と `FIELD-9-6-DESIGN.md` は役割終了。設計本体を本セクションに移し終えたら同ファイルは破棄してよい。）**
...
- [ ] **無駄ステージ検査を追加：** W1（全セル通行不可）と W2（orphan）を列挙。両方 0 を red チェック（CI で落とす）にする。
- [ ] **不変条件をテスト化：** 「実際に踏める接続ミス=0」「W1=0」「W2=0」を Playwright/データテストで固定。以後の編集で違反したら気づけるようにする（これが無いと何度でも劣化する）。
...
- [ ] **9-6-設計①.5 フィールド面白要素カタログ（ネタ帳）　🧠 Opus（ブレスト・ユーザー同席）：** ← **設計②より先にやる。** 2026-07-05 ユーザー指摘＝「そもそも通常フィールドの中にどういう面白要素を入れるのかの検討タスクが無い」＝正しい。設計①は「どの画面も2軸以上」という**器**を、②は**形**を定義するが、**器に盛る"面白い瞬間"の具体ネタ**が抜けている。ネタが無いままテンプレ化すると「形はあるが面白くない＝また塗り絵」になる。→ 既存資産（道具6種・敵・cave/grotto・NPC・看板・宝・ハートのかけら・9-4D小ダンジョン・9-5リスポーン）で作れる"面白い瞬間"を、初代ゼルダ（金をくれる老人/嘘看板の裏の宝/森の迷路の唯一解/爆破壁の奥の店/墓を押すと階段…）と IDEA.md の断片を種に**ブレスト形式で列挙**し、各ネタを P1〜P4 に紐づける。**⚠️ 私（AI）の独断で埋めない＝ユーザー同席のブレストで出す**（superpowers:brainstorming）。成果は IDEA.md か新規カタログにまとめ、②テンプレの入力にする。
- [ ] **9-6-設計②導線と分岐の設計：** 一本道でなく、迷い・選択・遠回り/近道を地形（`M`山/`t`木/`~`水/`f`柵/`x`穴/`%`空）で作る。辺の開口（col5,6 / row4,5）を意識した画面間の噛み合わせ。**← 設計①.5 ネタ帳の後：P1〜P4 × 地域スキン のテンプレ表を作る（`FIELD-9-6-DESIGN.md` §5）。ネタ帳の"面白い瞬間"を各テンプレに割り付ける。**
- [x] **9-6-設計③地域ごとの性格（ギミック地形）：** 2026-07-05 実コード精査（`FIELD-9-6-DESIGN.md` §3）。**🔴 `passable.js` は2値判定のみ＝溶岩ダメージ床/氷滑り/毒沼/視界制限は全て新規ゲームコードが要ると確定→9-6 のスコープ外（論点3・別Phase）。** 9-6 の地域差は **(1)地形パズルの質＋(2)見た目スキン(`s`/`c`/`w`/`d`/`o`)＋(3)敵編成** の3つで出す＝全て配線済み機構でSonnet量産可能。現状 field は bush64/breakable3/stone10/pit0/switch0 と隠し・地形パズル枠がほぼ未使用＝ここを埋めるのが実作業の中心。
- [ ] **9-6-設計④無駄ステージの作り替え方針：** 右端の全水域・左上の全山を「どういう実ステージにするか」を地域基準図と整合させて決める（B方針＝削除でなく作り替え）。
- [ ] **9-6-設計⑤秘密・報酬の配置設計：** 9-4D の小ダンジョン（`cave_*`＝矢筒/爆弾袋）・9-5 の拡充アイテム・隠し通路の配置を探索動機として設計。
- [ ] **9-6-流し込み（設計確定後）：** 上記が固まってから画面単位で作り込む（⚡ Sonnet）。**設計未確定のまま流し込まない。**
