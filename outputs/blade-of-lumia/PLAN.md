...
- [ ] **5.5k. 通常陸上敵を拡充する＝新規15種を実装（既存 E/C/F と合わせ計18種）（🧠 Opus）**。**⚠️ 着手前に必ず `ENEMY-DIRECTIONAL-GUIDE.md` を読むこと（2026-08-10 skeleton の①機構実装＋ガード実効化で踏んだ罠の集約＝guardRangeの罠／敵と重なれない制約／歯の確認の落とし穴）。スプライトを作るときは `SPRITE-PIPELINE.md`（複数フレームは `quantize-shared.mjs` 必須＝§3-★）も必ず読む。** **この位置の理由（2026-08-10 ユーザー指示）＝5.5j の通し判定で 10室中 dark_tower `[1,2]`（A 戦闘型③）だけが NG。原因は「通常陸上敵が3種（E/C/F）しかなく、最終盤の戦闘部屋に集める"最強格"の選択肢が無い」＝敵カタログの貧弱さそのもの。∴ 5.5j の最終合否（1,2 の再判定）と 6番（深洋O）より前に、まず敵種を増やす。**
      ...
      - **🐣 確定ロスター（陸上通常敵・計18種／新規15種）。** 脅威度=`hp*atk/(def+1)`。🆕=新エンジン機構／♻=既存語彙の組合せ：
        ...
        - **1. 地中蟲** ♻＝既存 `submerge`（水棲鮫）を陸に転用。潜伏↔急浮上のリズム、浮上した一瞬だけ殴れる/殴られる。弱点なし・脅威 低。**✅ エンジン実装済み（k-3a・タイル `α`・脅威 4.0・機構は `meta.hide`）／絵は k-3b。**
        - **2. 跳躍蜘蛛** 🆕跳躍移動＝不規則ジャンプで間合いを詰め、滞空中は当たり判定消失（着地の瞬間を狙う）。接触ダメージ・脅威 低〜中。**✅ エンジン実装済み（k-3a・タイル `β`・脅威 8.0・機構は `meta.leap` の4相）／絵は k-3b。⚠️ 「不規則ジャンプ」は乱数にしていない**＝跳躍は間合い（`minRange`/`maxRange`）とクールダウンで決まる＝プレイヤーがリズムを読める・テストが決定論的（GUIDE §7-3）。
        ...
        - **9. コウモリ群** 🆕ジグザグ高速飛行＝不規則高速で狙いを付けにくい。単体は脆い。弱点なし・脅威 低。**✅ エンジン実装済み（k-3a・タイル `ξ`・脅威 2.0・機構は `move:'air'`＋`meta.zigzag`）／絵は k-3b。⚠️ 無敵窓は持たせなかった**（脆さと機動が売りの敵に無敵は要らない＝k-3a の項）**・「不規則」は乱数でなく `e.id` 由来の決定的な位相**。
      ...
      - **▶ 5.5k 内の実装順（2026-08-11 確定・上から順に1つずつ）＝敵1体ずつではなく「共通機構ごと」に束ねる。** 順序の根拠＝(a) 5.5j の残1室（dark_tower[1,2]）を閉じるものを最優先 (b) それ以降は**1つの新機構が解禁する敵の数が多い順**（機構を作ってから敵を並べれば、同じ機構の2〜3体目はほぼデータ追加で済む）。各段の中は「エンジン→歯のあるテスト→スプライト（SPRITE-PIPELINE §6）」の順。
        ...
        - **k-3b. #1/#2/#9 の実スプライト（32×32・SPRITE-PIPELINE §6）。** k-3a のスプライトは既存絵のエイリアス（地中蟲←lurkShark・跳躍蜘蛛←chaser・コウモリ群←fishSchool）＝GUIDE §2「機構が先・絵は後」に沿った仮置き。**この位置の理由＝各段の中は「エンジン→歯のあるテスト→スプライト」の順（5.5k 実装順の規則）∴k-4 より先に絵を入れる**（k-2 が #7 で同じ順を踏んだ）。3体とも `directional:false`＝1方向＋左右反転で足りる（向き別12枚は要らない）。
      ...
      - **✅ k-3a 完了記録＝隠れ↔出現の無敵窓を陸/空へ一般化＋新規3種のエンジン（2026-08-13・🧠 Opus）。**
        ...
        - **▶ 残り＝k-3b（実スプライト3体）。** k-3a のスプライトは既存絵のエイリアス（`burrowWorm`←lurkShark／`leapSpider`←chaser／`batSwarm`←fishSchool）。3種は**まだ本編レイヤーに置いていない**（配置は 5.5m）＝スペック⑬がそれを不変条件として固定している（置いたらこの1本を配置検証に書き換える）。
