...
- [ ] **5.5k. 通常陸上敵を拡充する＝新規15種を実装（既存 E/C/F と合わせ計18種）（🧠 Opus）**。**⚠️ 着手前に必ず `ENEMY-DIRECTIONAL-GUIDE.md` を読むこと（2026-08-10 skeleton の①機構実装＋ガード実効化で踏んだ罠の集約＝guardRangeの罠／敵と重なれない制約／歯の確認の落とし穴）。スプライトを作るときは `SPRITE-PIPELINE.md`（複数フレームは `quantize-shared.mjs` 必須＝§3-★）も必ず読む。** **この位置の理由（2026-08-10 ユーザー指示）＝5.5j の通し判定で 10室中 dark_tower `[1,2]`（A 戦闘型③）だけが NG。原因は「通常陸上敵が3種（E/C/F）しかなく、最終盤の戦闘部屋に集める"最強格"の選択肢が無い」＝敵カタログの貧弱さそのもの。∴ 5.5j の最終合否（1,2 の再判定）と 6番（深洋O）より前に、まず敵種を増やす。**
      ...
      - **▶ 5.5k 内の実装順（2026-08-11 確定・上から順に1つずつ）＝敵1体ずつではなく「共通機構ごと」に束ねる。** 順序の根拠＝(a) 5.5j の残1室（dark_tower[1,2]）を閉じるものを最優先 (b) それ以降は**1つの新機構が解禁する敵の数が多い順**（機構を作ってから敵を並べれば、同じ機構の2〜3体目はほぼデータ追加で済む）。各段の中は「エンジン→歯のあるテスト→スプライト（SPRITE-PIPELINE §6）」の順。
        ...
        - **k-2. #7 の実スプライト6枚（3方向×{通常,攻撃}）→ dark_tower[1,2] を #7 中心に組み直す。** `guards:false` なので Guard フレームは不要＝#8 の12枚より軽い。ここまで済むと **5.5j の残 1室が閉じる**（＝5.5 全体の合否が出せる）∴他の敵より前。
          - **🚧 k-2 進行中の確定事項（2026-08-12・基準ボディ3枚まで完了・未コミット）。** 生成物は `.scratch/`（gitignore・ディスクに残る）。**再開はここから。**
            ...
            - **残り**＝(a) 攻撃3枚を基準ボディからグリッド構築（SPRITE-PIPELINE §6＝生成しない。`guards:false` なので Guard は不要・歩行2も現行の他敵と同じく任意）(b) `shared/sprites-enemies.js` へ機械的に登録（`swordBeastD/R/U`＋`*Atk`。パレット行も差し替え）(c) 実機シート＋データ直並べシートを自分で見る（§6-4／ラベルは実スプライト名・攻撃窓は別に待つ・即 `pause()`）(d) フルスイート（基準 550 passed）(e) `dark_tower[1,2]` の組み直し。
      ...
      - **✅ k-1 完了記録＝#7 剣獣（SWORD_BEAST `μ`）のエンジン（2026-08-11・🧠 Opus）。**
        - **`shared/enemies.js`**＝`ENEMY_META[TILE.SWORD_BEAST]`（hp10/atk3/def2/exp20・`speed: ENEMY_SPEED_FAST`・`directional:true`・`attacks` は近接 `sword`(range1.5/cd700) と遠隔 `swordBeam`(range9/**minRange2.5**/cd1500/projectileSpeed2.0) の二段構え＝潜み鮫と同型）。**脅威度 `hp*atk/(def+1)` = 10。**
        ...
        - **▶ 残り＝#7 の実スプライト6枚と dark_tower[1,2] の組み直し（上記 k-2）。** 現在の絵は **sentry の形の借り物＋`swordBeast` パレット**（機構が先・実描画は次段＝#8 と同じ進め方）。
      - **やること：** (1) `shared/enemies.js` の `ENEMY_META` に15種を追加する（[[blade-enemy-tables-derive-from-meta]]＝タイル一覧・脅威度・エリート判定は `ENEMY_META` から導出される単一ソース∴ここに足せば表は自動追従。手書きの敵表を別に作らない）。(2) 各敵のスプライト（上記 OpenAI パイプライン＝`shared/sprites-enemies.js` 等・[[blade-tile-sprite-single-source]]＝タイル→スプライトは `shared/tile-sprites.js` が単一の真実・絵文字使わない）。(3) 行動 AI（`game/enemy-ai.js`＝上記の共通機構）。(4) 弱点属性が要るなら `weakness` 定義。(5) **最強格 #7 剣獣を `dark_tower [1,2]` に集める**＝現状の脅威度合計は D1(24.0)→D7(36.0)→dark_tower[1,2](48.0) の単調増加（`tests/dungeon-key-gate.spec.js ⑨`）＝新最強格で 1,2 を組み直しても**この単調増加を壊さない**（D7 < 1,2 を維持）。移行は `scripts/migrate-key-room-d7-tower.mjs`（境界不変・脅威度単調増加・敵密度の逃げ場を書き込み前に assert する自己検査型）を流用/拡張する。
      ...
      - **テスト：** 新規敵ごとに (a) `ENEMY_META` から導出される表に載ること (b) スプライトが引けること (c) AI の行動（近接/遠隔/突進/分裂/瞬間移動/ガード 等）が意図どおり動くこと（新機構は歯のあるテストにする＝機構を潰すと赤くなる）。1,2 は `tests/dungeon-key-gate.spec.js ⑨`（脅威度単調増加＋全滅で鍵出現）を新構成に合わせて更新。
