...
- [ ] **5.5d. D5 鍵部屋＝道具型③（はしごで水路を渡る2段）（⚡ Sonnet 可）**。部屋＝`dungeon_5 / 1,0`（終端相当・入口は南・西はボス部屋）。芯＝渡った先でしか押せないボタン／撃てない `Y`。⚠️ はしご必須の水は整合性チェッカーの WARN 対象（`UNLOCKED_AT`）＝D5 入場時にはしごを持っている前提が成立していることを確認する。
