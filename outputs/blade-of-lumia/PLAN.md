...
- [ ] **5.5k. 通常陸上敵を拡充する＝新規15種を実装（既存 E/C/F と合わせ計18種）（🧠 Opus）**。**この位置の理由（2026-08-10 ユーザー指示）＝5.5j の通し判定で 10室中 dark_tower `[1,2]`（A 戦闘型③）だけが NG。原因は「通常陸上敵が3種（E/C/F）しかなく、最終盤の戦闘部屋に集める"最強格"の選択肢が無い」＝敵カタログの貧弱さそのもの。∴ 5.5j の最終合否（1,2 の再判定）と 6番（深洋O）より前に、まず敵種を増やす。**
      ...
      - **✅ 生成手段の確定（2026-08-10・第N セッション）＝OpenAI（`gpt-image-1`）を採用。** `.scratch/gen-enemy-sprite.mjs`（使い捨て・git-ignore）で #8 骸骨剣士を生成し、ユーザーが実ファイルを開いて**透明背景・グロー無し・くっきりドットで合格と判定**（AI が Read で見た縮小版はグレー地＋グローに見えたが、それは AI 側の画像プレビュー縮小の癖＝実ファイルはクリーン）。参考に Gemini でも同プロンプトを試し良好だったが**OpenAI で確定＝Gemini は保留（フォールバック候補）**。**⚠️ ただし本当の関門は「量子化がうまくいくか」＝これは次セッションで初検証。量子化で色が破綻/グリッドがズレるなら Gemini 切替を再検討する（保留のまま）。** API パラメータ＝`{model:'gpt-image-1', size:'1024x1024', background:'transparent', n:1}`。
