...
- [ ] **3. 深洋O ④＝デルタ上半5枚（`14,16`/`15,16`/`13,17`/`14,17`/`15,17`）を作り込む**（🧠 Opus・**方針は 2026-07-29 に確定済み＝`FIELD-9-6-DESIGN.md` §19-11-G。残っている設計＝各画面のパズル幾何そのもの**〈10×12 のセル配置〉∴Opus。廊下C1〜C4 では初稿の幾何バグをソルバーが実際に見つけた＝ここは機械的な流し込みにならない・詳細は下記 9-6 ⑥ 深洋O の節）。芯＝**1画面1道具**（爆弾壁 `!`／弓ゲート `Y`／石押し `*`／はしご水渡り／ブーメラン運搬 `H`）。成果物＝`scripts/migrate-field-delta-o.mjs`＋`tests/field-delta-o.spec.js`＋実ブラウザ目視。**着手前に `scripts/generate-field-metrics.mjs` の定義ズレを直す**（DESIGN §19-11-H）。

- [ ] **4. 深洋O ④＝デルタ下半9枚（D6〜D11 ロア探索＋海の石碑A `12,18`／B `15,18`＋聖域 `11,19`＋海の主 `12,19`＋隠し報酬 `15,19`）**（🧠 Opus）。**この位置の理由＝上半で確定する「デルタ内部シームの開き方」と拡張したソルバー（`brokenWalls`/`litTorches`/はしご渡り）を下半がそのまま使う**∴先に下半をやると同じ判断を2回する。新規部品＝**海の主 `{`（`isBossRoom`＋`bossReward` を本編画面で通す）**。
      **⚠️ 2026-07-29 に旧3番（14画面いっぺん）を 3番＝上半5枚／4番＝下半9枚 に分割した。以後の 5番／6番は旧 4番／5番。** 過去ログ（DECISIONS/PROGRESS）の「キュー4番＝⑥-完了検査／5番＝外周解体」は分割前の番号。

- [ ] **5. ⑥-完了検査**（下記 9-6 ⑥-完了検査の節＝全指標 0 の機械的証明）

- [ ] **6. 9-6-BASE 外周解体＋隣接地域編入**（外周 105画面の格上げ。**`footprintBlocked` は 2番（⑥-landing）で既に 0 になった**＝このタスクの目的は外周画面の格上げだけ）
