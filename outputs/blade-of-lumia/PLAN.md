...
#### タスク
- [ ] `gameTick` を「実時間駆動」と「1フレーム進める純粋ロジック `step()`」に分離
- [ ] `Date.now()` 依存のクールダウン・無敵時間・敵AIタイマーを論理時間 `gameTime` に置換
- [ ] 入力をキュー化し、テストから注入できる API（`queueInput` / `step`）を用意
- [ ] テスト用フックを公開（`window.__game` 等でテストから状態・stepにアクセス）
...
| 色スイッチ | ⬜ 未実装 |
| 剣の複数ランク | ⬜ 未実装 |
| テスト基盤（Playwright E2E/VRT） | 🚧 着手（起動/移動/セーブ・ロードの3スモークがパス。ステージ遷移・VRTは今後） |
