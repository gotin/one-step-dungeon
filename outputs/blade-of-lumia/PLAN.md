...
#### ▶▶ 次セッションはここから再開する（2026-07-05 更新）

> **土台是正は完了（下記チェック済み）。次にやるのは「設計対話」＝いきなり画面を触らない。**
>
> **手順（毎回 /workflow で入る想定）：**
> 1. **`FIELD-9-6-DESIGN.md` を全文読む**（診断・honest指標・★1〜★4 の分岐が全部そこにある）。
> 2. **ユーザーに ★1〜★4 を質問して確定する**（AskUserQuestion）。特に **★4＝外周リング110画面（全海/全山）を「意図的な世界の境界」として許容するか** が最重要＝これで scope が「シーム91件だけ潰す」か「200+画面を作り替える」かに分かれる。
>    - ★1 着手順（土台先/面白さ先/並行）
>    - ★2 面白さの主軸（隠し要素網羅／地形パズル導線／地域固有体験／戦闘の起伏）
>    - ★3 作り込みスコープ（全210／骨格厚め＋周辺軽め／1地域試作→横展開）
>    - ★4 外周リングの位置づけ（境界として許容 ⇔ 全320作り替え）
> 3. **確定した★の答えを PLAN 9-6 本文と DECISIONS.md に書き移す**（FIELD-9-6-DESIGN.md はたたき台なので確定後に破棄可）。
> 4. その後：honest指標の不変条件をテスト化（★4確定後）→ 1地域を試作して密度基準確立 → ⚡ Sonnet で量産展開。
>
> **⚠️ ★が未確定のまま「設計②〜⑤」や流し込みに進まない**（また塗り絵になる／scope がブレる）。honest指標の不変条件テスト化も ★4 確定まで保留（外周を境界扱いにするかで W1/W2 の red 基準が変わるため）。
...
- [ ] **9-6-設計①「楽しさ・奥深さ・複雑さ」の言語化：** このゲームのフィールド探索の面白さとは何か（隠し通路・行き止まりの意味・ショートカット・道具で開ける秘密・迷いと発見）。初代ゼルダが 128 画面全部を意味あるものにした構造の分解。
- [ ] **9-6-設計②導線と分岐の設計：** 一本道でなく、迷い・選択・遠回り/近道を地形（`M`山/`t`木/`~`水/`f`柵/`x`穴/`%`空）で作る。辺の開口（col5,6 / row4,5）を意識した画面間の噛み合わせ。
- [ ] **9-6-設計③地域ごとの性格（ギミック地形）：** 火山＝溶岩/足場、雪＝滑り、沼＝毒/視界、湖＝橋と島の攻略、森＝迷路。既存タイル/ギミックで表現できる範囲を実コードで精査。
