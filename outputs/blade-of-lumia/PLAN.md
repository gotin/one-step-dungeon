...
- [ ] **5.5i. dark_tower `4,3` ＝合成型（石3＋色ゲート・体験の頂点）（🧠 Opus）**。4.7 で合格した語彙をそのまま使う＝**色ゲートは石の搬送路にせずプレイヤーの通路にする**（4.7 の硬い制約）。I4（色ゲートを1枚ずつ壁化して解なし）＋4軸（**L ≤ 60**）。
      **🚧 着手済み（2026-08-08・セッション中断で未完＝ここから再開する。以下は再導出不要な確定事項）**
      ...
      - **残作業**＝v2 テンプレ確定（GEOM_ONLY 反復）→ 測定をバックグラウンドで回して 4軸＋`L ≤ 60`＋I4 合格盤面を採用 → `migrate-key-room-dark-tower-43.mjs`（自己検査型・`showConditions {"7,6":{trigger:"stonesPlaced"}}`／`fluteEffect {type:"resetStones"}`／敵と `enemyDirs` を全消し＝倉庫番の部屋に敵を置かない）→ 整合性・接続チェッカー → 実機 Playwright 再生テスト（セーブ注入＋「つづきから」・ワープセルを経路にしない・座標を丸めない）。
