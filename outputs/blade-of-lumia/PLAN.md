...
- [ ] **5.5h. D7・dark_tower `1,2` ＝戦闘型の強化（2室）（⚡ Sonnet 可）**。両室とも A 型のまま中身を強化する＝`ENEMY_META` から **HP 合計・`isBoss` の有無が進行順で単調増加**することを機械的に確認する（手で数えない＝[[blade-enemy-tables-derive-from-meta]]）。D7 は笛（`flutePlayed`）を2段目に足す。逃げ場のない殴られ待ちにしない（部屋の広さと敵速度を見る）。
