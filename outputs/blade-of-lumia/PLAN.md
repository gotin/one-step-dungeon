...
- [ ] **4.55. 石・スイッチ・ゲートの3バグを調査修正する（🧠→⚡・4.6 の前提＝これが直るまで実プレイ検証に進めない）**。**この位置の理由（2026-07-31 ユーザー割り込み＝プレビュー実機で発覚）＝4.6 の実プレイ化は「石を全ボタンに乗せるとゲートが開く」動作が正しく動くことが大前提。実機で3つの不具合が出た＝先に直す。** ⚠️ 全437テストが緑だったのに実機で(1)が壊れていた＝**新ルール（全ボタンON→全ゲート開）を実際に踏む既存テストが1つも無かった＝テストの穴**。修正と同時に `field 10,14`（＝ユーザーが当初自作したパズル・AI の改変でゲートが消えていた／今回 `TT` を復活）を使った回帰テストを必ず追加する。**3バグ：**
