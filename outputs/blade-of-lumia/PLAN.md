...
- [ ] **4.7. `test_mechanics/24,0` を色ゲート/色スイッチで合成した「23,0（激難）より質で難しい」パズルに作り直す（🧠→⚡ Sonnet＝規則は 2026-08-04 に確定・幾何の反復は Sonnet でよい。ただし手詰まりが続いたら幾何再検討は🧠に戻す）**。
  ...
  - **(B) 24,0 本体（未着手・次にやること）：**
    (1) **23,0（激難・rows3-7/cols1-9・4隅ボタン・千鳥ピラー）の壁構造をそのまま土台にする**（ユーザー指定＝倉庫番としての回り込み難度をゼロから作り直さない）。ボタン/ピラー配置は変えない。
    (2) **中央付近（row5 の横断帯など）に色ゲート＋色スイッチで通路を分断する仕切りを追加する。** 新規則により曲がり角は不要＝直線上に置いてよい。**ゲートは「経路を邪魔するもの」であって「何かを守るもの」ではない**（ユーザー指摘＝§3-2eの旧「押し位置をゲートの奥に置く」という設計思想も参考程度に留め、実際に石3〜4個を運ぶ経路のどこかを色で分断できれば十分）。
    (3) **石が2個連続で通路を塞いで連絡通路への口を完全に潰す事故だけは避ける**（25,0で実際に踏んだ詰みパターン＝バグではないが、24,0では「詰んだら笛で戻せる」ことを前提にしても、口が1つしかないと難易度調整の柔軟性が下がる。可能なら連絡通路への口は2つ確保する）。
    (4) 4軸測定（`generate-sokoban-playable.mjs`）で **23,0（激難・L=89・最短解2本）を「質で」超えるか確認**＝L 単独でなく、色の切替が本当に必須か（片方の色を壁化して測り直し、両方とも解なしになるか）、最短解の細さ、貪欲NG・デッドロック>0・強制手率≤0.7（PUZZLE-DESIGN §2下限）。
    (5) `24,0` に `fluteEffect: {type:'resetStones'}` を付け、`migrate-test-sokoban-tiers.mjs` の BOARDS へ追加（`fixture` は使わない＝可解パズルとして書く）。
    (6) `tests/sokoban-tiers.spec.js` に24,0の実機再生テスト＋`test-stage-keys.js` の `sokoban_color` コメント更新。
    (7) ユーザーが実際に解いて「23,0より質で難しい／理不尽でない」を判定する（＝合否）。
  - **この位置の理由（2026-08-02 ユーザー指摘でキューを是正・2026-08-04 に規則を再設計して継続）＝4.6 の4枚は判定済みで、その判定が「もっと難しいものが必要」だった。** 5番（鍵の関門）で「鍵に与えられる難易度の上限」を決めるにはこの軸が要る∴5番の前。
