...
- **⚠️ 規模は「進むほど大きく」漸増させる（2026-06-24 ユーザー確定・最重要）：** 全ダンジョンを 16 で固定しない。序盤は小さく終盤は大きく＝歯ごたえのカーブを部屋数でも作る。**確定カーブ（緩やかに漸増）：**
  ...
  | D3 | dungeon_3 | **18** | 5×4 等から2室抜き or 4×5 | 弓矢・今回設計 |
...
- [ ] D2 のテンプレ（15〜25部屋・辺スクロール網・配線済み仕掛けのみ・**そのダンジョンの報酬を中盤で入手→必須化部屋→ボス**・(A)ソフトロック厳禁・足止め最低1・欠片はボス撃破）に沿って 1 つずつ作り込む。各ダンジョンの必須化ギミック（配線確認済み）：
  - **D3 水（弓矢）：** 弓を中盤入手 → `Y`SWITCH を**矢で遠隔トグル**→`T`ゲートで奥へ。ボス`J`弱点 beam（剣チャージ可）。内部の水は既存橋 `v` で渡る（はしご＝D5報酬を要求しない＝(A)）。**→ 設計確定（2026-06-24・🧠 Opus・18部屋）＝下記 9-2d。流し込みは ⚡ Sonnet**
...
#### 9-2d. dungeon_3「水の迷宮」作り込み（D3＝弓矢報酬・2番目の進行ダンジョン）　🧠 設計確定（2026-06-24・18部屋へ拡大）／⚡ データ流し込み
...
- [ ] **18部屋レイアウト＋接続グラフを確定**（5列×4行=20 から袋小路2室を欠いて18・辺スクロール接続・`check-dungeon-connectivity.mjs` で論理検証する）。**stage 座標は `sx,sy`（sx=列／sy=行）。** 役割と接続は下記マップのとおり（D2=9-2b と同粒度・列を1本増やして漸増規模に）：
...
- [ ] **接続グラフの機械検証スクリプトを通す**（`node scripts/check-dungeon-connectivity.mjs dungeon_3` → PASS／dead edge 0／orphan 0（**実部屋18・欠け `[4,0]`/`[0,3]` は orphan ではなく "stage 無し" 扱い**）。`--block-room=1,1` でボス到達不能を確認）
- [ ] **整合性チェッカーを通す**（`node scripts/check-dungeon-integrity.mjs dungeon_3` → エラー0。`bossStage:"0,0"`/`triforceId:3`/`name:"水の迷宮"`／ボス部屋に `J`＋ハート器／地図`m`・コンパス`n` 存在／後ダンジョンアイテム依存タイルが出口封鎖していないこと）
- [ ] **dungeon_3 の layer メタを整備：** 現状 `meta.name` 形式だが D1/D2 と揃えて **layer 直下に `bossStage:"0,0"` / `triforceId:3` / `name:"水の迷宮"` / `bgm` / `bossBgm`** を設定（現 `1,0` の `isBossRoom:true` ボス部屋は `0,0` へ再配置）
- [ ] **必須化の検算（最重要）：** 弓無しでは `Y` に届かず `T` が開かない（水堀幅2マス以上＝ブーメラン maxRange 外）＝中ボス・鍵・ボスすべて到達不能になるか実プレイで確認
- [ ] **(A) ソフトロック検算：** D3 内で後ダンジョン報酬（ロウソク/はしご/爆弾/笛）を要求していないこと。内部水路は `v`BRIDGE で渡る
- [ ] **アイテム移設（9-2a 表）を反映：** D3 `[1,2]` に弓矢宝箱を配置。field 側・他レイヤーの旧弓配置の撤去は 9-3 で整理
- [ ] **動作確認：** `fromEditor=1` プレビューで 入口→弓入手→矢でY→T開放→中ボス→鍵→D扉→ボスJ→欠片③→field 帰還
...
- [ ] レビュー対象ダンジョン（作り込み完了次第追加）：
  ...
  - [ ] dungeon_3〜8（D8 沼地＝新設レイヤー）（作り込み後）
