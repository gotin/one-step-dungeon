...
### 9-6. フィールドを「ゲームとして成立」させる　🧠 Opus（対話で設計・複数タスクに分割）　⏳ 未着手＝最優先
...
- [ ] **チェッカーを上記3条件で絞る**（`check-field-connectivity.mjs` / `scripts/lib/connectivity.mjs`）。現状の deadEdges を「到達可能 from」かつ「踏み出しセルが通行可」でフィルタした数を**正の指標**にする。※以前 `borderCellWalkable` 相当の絞り込みを試みたところで中断（この定義合わせのため）。
- [ ] **「実際に踏める接続ミス」件数と該当ステージ一覧を出す**（規模把握）。これを 0 にするのが 9-6 の構造ゴール。
...
#### 設計で洗い出すべき検討タスク（← まず対話でこのリスト自体を精緻化する）

> 以下は**叩き台**。9-6 着手時の対話で「本当に必要な検討項目」を洗い出し・追加・分割する。各項目は独立した設計サブタスク（9-6a, 9-6b, …）になる想定。
