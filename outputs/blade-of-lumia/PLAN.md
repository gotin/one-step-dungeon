...
>
> **⚠️⚠️ 「量産」ではない（2026-06-23 ユーザー指摘で明確化）：** ボス入りダンジョンは **D1〜D7 ＋ cave_1 ＋ 暗黒の塔の9個だけ**。これは"一点物の作品"であり、**1つずつ設計意図を持って作り込む**対象。stage を継ぎ足す接続作業は機械的でも、**体験（テーマ地形・固有の謎・難易度カーブ・足止め）は手作業で詰める**。「量産」「機械作業」という表現は誤り。各ダンジョンの作り込みは原則 🧠 Opus で1つずつ設計し、確定した配置データの流し込みのみ ⚡ Sonnet に回す。
>
...
- [x] **中盤・終盤の連鎖を確定（影響力の小さい鍵→地形を開く広い鍵→ワープ、の順）：**
  ...
  | 終 | **cave_1 沼地の洞窟（沼地の大型ボス＝新規作成）** | **はしご** | **笛**＋欠片⑦＋ミラーシールド | **最広（ワープ/隠し入口）** | （新ボスの弱点は作成時に決める） |
  ...
  - **⚠️ 現状とのズレ（9-2 で必ず是正・潜在バグ）：** 現在 `dropsTriforce:true` の大型ボスは **7体のみ**（G/N/J/A/L/O/U＝D1〜D7）。**cave_1 には大型ボスがおらず `W`（MONSTER＝欠片を落とさない雑魚ボス）が置かれているだけ**。欠片必要数は `countTriforces()` がマップ上の dropsTriforce ボス実数から動的計算するため、**今は実質7欠片**で、ストーリー前提の8欠片と食い違う。→ **cave_1 に新規の沼地大型ボス（dropsTriforce:true）を作って8体に揃える**（下記 9-2c）。
...
- [x] **全アイテムの「初期所持→報酬」移設の撤去対象を実データで確定**（下表）。9-2/9-3 実装時にこの表どおり撤去＋報酬位置へ再配置する。**座標規約：grid `tiles` は `tiles[row][col]`／`chestContents`・`floorItems` のキーは `"row,col"`。**
  ...
  | 6 | 笛 | `field/2,0 chestContents["4,3"]`（B宝箱あり） | chest | **cave_1 沼地報酬** |
...
##### 📐 全ダンジョン共通の規模基準（2026-06-23 ユーザー確定）
- **各ダンジョンは 15〜25 部屋**（初代『ゼルダの伝説』のダンジョン規模に準拠）。dungeon_1（12部屋）は下限未満なので**品質・規模の基準にはしない**。
- **最終ダンジョン（暗黒の塔 dark_tower）はこれより明確に広く**作る（ラスダンの風格）。
...
> **この後の運用：** D1〜D7・cave_1・フィールド拡張の流し込み後は必ずこのツール（＋`--block-room`）に通し、デッドエッジ0・孤立0・一本道成立を機械確認してから「接続OK」とする。
...
- [ ] **接続グラフの機械検証スクリプトを通す**（Sonnet 流し込み後）：`node scripts/check-dungeon-connectivity.mjs dungeon_1` で①全室到達（D扉/`:`は閉として「unreached」に出るのが正＝鍵/ボス入室が要る部屋）②ボス`[0,0]`の隣接が`[0,1]`だけ、を確認。`… --block-room=1,0` で**鍵部屋を塞ぐとボス到達不能**（D扉は鍵がなければ開かない＝一本道）を確認。**期待と違えば流し込みミス＝修正**
- [ ] **撤去（9-1 連鎖と矛盾する装備を dungeon_1 から全て除去）：** 聖剣tier3×2・伝説のよろいtier2×2・ミラーシールドtier2・銀の剣tier2・さいごの鎧tier2 を撤去。**上位装備は重複を消し、tempor温存先＝鉄の盾→D5／ミラーシールド→cave_1（9-1 表）へ**。聖剣/伝説のよろいは「後半ダンジョン報酬 or 撤去」を作り込み時に判断（序盤で最強剣が手に入らないことが最優先）
- [ ] **報酬を木の盾（tier0）＋欠片①に集約：** 木の盾を D1 内のチェスト（できればボス前後の宝＝盾の意味づけが立つ位置）に置く。欠片①はボス`G`撃破ドロップのまま
- [ ] **15〜25 部屋へ拡張：** 現状12部屋→**16部屋（4×4 グリッド）**に確定（上記接続グラフ）。D2=16 と揃える。dungeon_1 の良い構造（最短線とは別の枝分かれ・ギミック教科書配置・地図`m`/コンパス`n`/看板`i`）は活かしつつ、改善メモ（素通り廊下→足止め部屋に）を反映
- [ ] **足止め部屋（最低1・配線済み仕掛けのみ）：** 臨界経路の足止め＝**`[1,0]`中ボス`W`撃破→鍵`K`→`[0,1]`の`D`扉を開けてボスへ**（鍵1枚ゲート）。寄道の任意ギミックとして `[2,1]`に`*→S→T`（石押し）・`[0,2]`に`torchesLit`封印宝箱を配置。`|`DOORWAY_LOCKED は使わない（死にコード）。入門なので難度は控えめ（必須は鍵1枚のみ）
- [ ] **🔑鉄則の検算：** D1 はサブアイテム皆無（ブーメラン/弓矢/爆弾/はしご/笛/ロウソク無し）でクリアできること。**特にボス`G`の弱点は爆弾だが爆弾未所持＝剣で倒す前提**（HP30・剣で撃破可）を確認。`!`壊せる壁を必須経路に置かない（爆弾が無いと詰む）
- [ ] **村スタートの剣集約と整合：** 9-3 で「最初の剣＝村スタート出口に半強制1本」へ集約するので、D1 内に剣を置かない（拾うのは木の盾のみ）。フィールドの木の剣重複（chest `field/1,2["8,10"]`・floor `field/0,1["4,8"]`）も 9-3 で村スタートへ集約
- [ ] **動作確認：** `fromEditor=1` プレビューで 村（剣のみ）→D1入場→雑魚→足止め→ボス`G`→木の盾＋欠片①→帰還。**剣のみ（盾なし入場）で詰まずクリアできる**こと＋**序盤で聖剣/ミラー等が手に入らない**ことを確認
...
- [ ] **接続グラフの機械検証スクリプトを通す**（上記 🔗 規約の5項目）。Sonnet 流し込み後に `node scripts/check-dungeon-connectivity.mjs dungeon_2` と `… --block-room=1,1` を実行し、①全室到達（`>`扉/鍵を開けた前提の意図と照合）②ボス`[0,0]`の隣接が`[0,1]`だけ ③`--block-room=1,1` でボス到達不能（一本道）を確認。**期待と違えば流し込みミス＝修正**
- [ ] **必須化の検算（最重要）：** ブーメラン入手 → 鍵回収 → `D`扉 → ボスの一本道が、**ブーメラン無しでは絶対に突破できない**こと（対岸の鍵は徒歩/はしご/剣ビームでは取れない＝2マス幅水ではしご不可・剣ビームは鍵を回収しない）を確認。逆にブーメラン入手前の区間（[1,3]→[1,2]→[1,1]）は剣だけで到達できることも確認
- [ ] **(A) ソフトロック検算：** D2 内で後ダンジョン報酬（弓/ロウソク/爆弾/はしご/笛）を要求していないこと。水/穴は「ブーメランで鍵を取る用＝渡る必要なし」に留め、**渡らないと進めない水/穴を作らない**（はしご＝D5報酬を要求しないため）
- [ ] **アイテム移設（9-2a 表）を反映：** ブーメランの旧入手（tile `field/1,1`(r4c1)・孤児chest `field/1,0["7,6"]`・旅商人ショップ price50）を**撤去**し、D2 中盤の宝箱へ。`>`戻り出口・field 側 `field_dungeon2` 配線は既存維持
- [ ] **dungeon_2 の `meta` を整備**（`bgm`/`bossBgm`/`bossStage`/`triforceId:2` を dungeon_1 同様に。現状は `meta:{name:"砂漠の神殿"}` だけ）
- [ ] **動作確認：** `fromEditor=1` プレビューで入口→ブーメラン入手→鍵回収→扉→中ボス→ボス→欠片②→field 帰還。**ブーメラン入手前にボスへ到達できない**こと＋**剣＋盾＋ブーメランだけでクリアできる**ことを確認

#### 9-2{d〜} dungeon_3〜7（D3 水／D4 炎／D6 森／D5 氷／D7 空）の作り込み　🧠 1つずつ設計（古典ゼルダ型）
- [ ] D2 のテンプレ（15〜25部屋・辺スクロール網・配線済み仕掛けのみ・**そのダンジョンの報酬を中盤で入手→必須化部屋→ボス**・(A)ソフトロック厳禁・足止め最低1・欠片はボス撃破）に沿って 1 つずつ作り込む。各ダンジョンの必須化ギミック（配線確認済み）：
  - **D3 水（弓矢）：** 弓を中盤入手 → `Y`SWITCH を**矢で遠隔トグル**→`T`ゲートで奥へ。ボス`J`弱点 beam（剣チャージ可）。内部の水は既存橋 `v` で渡る（はしご＝D5報酬を要求しない＝(A)）
  - **D4 炎（ロウソク）：** ロウソク中盤入手 → `H`かがり火点灯（`torchesLit`）or 茂み焼き（`bushBurned`）で封印扉/通路を開く。ボス`A`弱点 arrow（弓＝D3報酬で◎・剣可）
  - **D6 森（爆弾）：** 爆弾中盤入手 → `!`BREAKABLE_WALL を割って奥へ。ボス`O`弱点 fire（ロウソク＝D4報酬で◎・剣可）
  - **D5 氷（はしご）：** はしご中盤入手 → 1セル幅の水/穴 `~`/`x` を渡ってボスへ。**D5 入場は爆弾`!`で**（はしごは D5 報酬＝D5 内で入手するので入場には使わない＝(A)）。ボス`L`弱点 fire
  - **cave_1 沼地（笛）：** 笛中盤入手 → `flutePlayed` で隠し入口/通路を出して奥へ。ボス＝新規沼地ボス（9-2c）。入場は はしご
  - **D7 空（報酬は欠片⑧のみ＝新アイテムなし）：** D7 は固有の進行アイテムを配らないので、古典ゼルダ型の「自分の報酬で解く」は適用外。**笛ワープで入る**空島ダンジョンとして、既得アイテム（弓等）の総合ステージにする。ボス`U`弱点 arrow

#### 9-2c. 8体目の大型ボス（沼地）を新規作成し「ボス8＝欠片8」に揃える　🧠→⚡
- [ ] **cave_1 用の新規大型ボス（沼地テーマ・2×2）を作る。** 3-2 で確立した「型」で作成：スプライト（24×24）→ `ENEMY_META[新TILE]`（stats＋`size`＋`dropsTriforce:true`＋`weakness`）→向きエイリアス→`shared/tiles.js` にタイル文字追加→cave_1 ボス部屋に配置。現状の `W`（欠片を落とさない雑魚ボス）を置き換える
- [ ] **欠片数が動的に8へ揃うことを確認**（`countTriforces()` がマップ集計で 7→8 になる）。祭壇の必要欠片・ガイド台詞・エンディング条件が8前提で整合するかテスト
- [ ] 新ボスの `weakness` は連鎖と矛盾しない範囲で作成時に決める（cave_1 は「はしご」で入り「笛」を報酬に落とす＝直前報酬はしごを弱点にする必然はない。剣で倒せる前提でテーマに合う弱点を選ぶ）
- [ ] ※ ザーネル（Z・dark_tower）は欠片を落とさない最終ボスのまま（計9ボス＝大型8＋最終1）

### 9-3. 序盤チュートリアル動線＋ガイド導線の実装　🧠→⚡
- [ ] 何も持たないスタート → 最初のステージで剣を半強制ピックアップ（出口が剣の先）の地形を作る
- [ ] dungeon_1 で木の盾を入手 → ガードを使わないと進めない配置
- [ ] **ガイド導線（フィールドは塞がない・誘導のみ）：** 村NPC・各ダンジョン入口の看板（`signData`）・`linesAfterBoss[type]` を使い、**「次はどこを目指すか／そこに入るには何が要るか」**を段階的に提示する。`defeatedBosses` の進行に合わせて NPC 台詞が次の行き先へ進む（ボスを倒すたびにガイドが更新＝Phase 6-1b 機構の再利用・ほぼゼロコード）
- [ ] ※ 「D1→D2 の NPC 門番（通行ゲート）」案は撤回（フルオープン方針と矛盾）。順序強制は 9-2 のアイテムロックで効かせ、NPC はガイドに徹する

> ※ 9-1 の連鎖設計は完了（2026-06-23）。次は 9-2（ダンジョン作り込み）/9-3（チュートリアル＋ガイド導線）の実装。**Phase 8（やりこみ・UX）と Phase 8-4（バランス調整）は Phase 9 の後に回す**（ゲームの背骨が無い状態で UX やバランスを詰めても土台がブレるため）。なお Phase 8-1（ゲームオーバー体験改善＝フクロウのヒント／墓）は**スキップ**で合意（詰みポイントが少なく発火しにくい・ヒント手段は石碑/NPCで足りている）。
