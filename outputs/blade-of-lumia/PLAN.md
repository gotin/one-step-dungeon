...
- [ ] D2 のテンプレ（15〜25部屋・辺スクロール網・配線済み仕掛けのみ・**そのダンジョンの報酬を中盤で入手→必須化部屋→ボス**・(A)ソフトロック厳禁・足止め最低1・欠片はボス撃破）に沿って 1 つずつ作り込む。各ダンジョンの必須化ギミック（配線確認済み）：
  ...
  - **D4 炎（ロウソク）：** ロウソク中盤入手 → `H`かがり火を全点灯（`torchesLit`）→ 隠し `>`テレポートでボス翼へ。ボス`A`弱点 arrow（弓＝D3報酬で◎・剣可）。**✅ 設計確定（2026-06-25・🧠 Opus・20部屋・コード/JSON 変更なし）＝下記 9-2e。流し込みは次ステップ**
...
#### 9-2e. dungeon_4「炎の神殿」設計確定（D4＝ロウソク報酬・3番目の進行ダンジョン）　🧠 設計確定（2026-06-25・コード/JSON 変更なし）／⚡ 流し込みは次ステップ
...
- [ ] **20部屋レイアウト＋接続グラフを確定**（5列×4行=20・欠けなし・辺スクロール接続・`check-dungeon-connectivity.mjs` で論理検証する）。**stage 座標は `sx,sy`（sx=列／sy=行）。** 役割と接続は下記マップのとおり（D3=9-2d と同粒度・列を1本増やして漸増規模に）：
...
- [ ] **接続グラフの機械検証スクリプトを通す**（`node scripts/check-dungeon-connectivity.mjs dungeon_4` → PASS（dead edge 0／orphan 0／全20室到達）。`--block-room=1,1` で **boss 到達不能（gate holds）** を確認）
- [ ] **整合性チェッカーを通す**（`node scripts/check-dungeon-integrity.mjs dungeon_4` → エラー0。`bossStage:"0,0"`/`triforceId:4`/`name:"炎の神殿"`／ボス部屋に `A`＋ハート器／地図`m`（`[3,2]`）・コンパス`n`（`[3,1]`）。`A` は `TRIFORCE_BOSS_TILES` に既登録・`UNLOCKED_AT.dungeon_4={boomerang,bow,candle}` も既存＝チェッカー変更不要）
- [ ] **dungeon_4 の layer メタを整備：** layer 直下に `bossStage:"0,0"` / `triforceId:4` / `name:"炎の神殿"` / `bgm:"dungeon"` / `bossBgm:"boss"` を設定。旧 `1,0` の `isBossRoom:true` ボス部屋を `0,0` へ再配置
- [ ] **必須化の検算（最重要・D3 の教訓を適用）：** ロウソク無しでは `[1,1]` の篝火が点かず隠し `>` が出ない＝ボス翼へ行けない。**接続チェッカーは "部屋内で飛び道具が届くか／篝火が点くか" を見ない**ので、`fromEditor=1` の実プレイ（or fixture を使った実エンジンテスト）で「ロウソクで全点灯→テレポート出現／剣・矢・ブーメランでは不可」を確認するまで完了にしない。`tests/candle-gate.spec.js` 新設候補（D3 の `bow-gate.spec.js` と同型）
- [ ] **(A) ソフトロック検算：** D4 内で後ダンジョン報酬（はしご/爆弾/笛）を要求していない（`~`水・`x`穴・`!`壊せる壁を臨界経路に置かない）
- [ ] **アイテム移設（9-2a 表）を反映：** D4 `[1,2]` にロウソク宝箱を配置。field `2,0` の `2,6` 旧ロウソク宝箱の撤去は 9-3 で整理
...
- [ ] レビュー対象ダンジョン（作り込み完了次第追加）：
  ...
  - [ ] dungeon_4「炎の神殿」（ロウソク・A）（✅設計確定 2026-06-25・9-2e → 流し込み後にレビュー）
