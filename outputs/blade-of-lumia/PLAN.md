...
- [x] **0.（割り込み・2026-08-12 完了）剣獣（μ）の行動 AI を作り直す＝ユーザー実プレイ判定「だめだね」への対処（🧠 Opus）**。**この位置の理由＝5.5k-2 の完了条件(3)＝「ユーザーが `dark_tower[1,2]` を実プレイして歯応えのある最終戦闘と判定すること」が NG で返ってきた∴次タスク（k-3）より先。** 指摘は5点：①攻撃動作中も動く ②追従が完璧すぎて逃げ切れない ③複数体で倒せない/強すぎ ④ビームを一度も撃ってこない ⑤遠隔攻撃があるのに1セルまで詰めてくる（＝常にくっついてくる）。**原因と対処は下記「0番 完了記録」。実装は敵種を問わない汎用機構（`meta.attackFreezeMs` と `meta.combat`）＝残り14種でも使い回す。** フル **557 passed**（基準552 +5）。
...
- [x] **4.57. ユーザー報告のバグ修正＝閉じた門の中に立って石を押せないようにする（2026-08-02 完了）（🧠 Opus・0番＝割り込み枠）**。**この位置の理由（2026-08-02 ユーザー割り込み＝4.6 の合成盤面を実機で見て報告：「石がしまってるゲートをとおれるし、そのときはプレーヤーも通れてしまう。そのバグがないとクリアできないよね？」）＝報告どおり実コードのバグで、しかも 4.6 の5枚目（合成）はそのバグに依存して成立していた＝先に直す。** **報告の実体＝(B1) `player.js` は押し成功時にプレイヤーを石の元セルへ**通行判定なしで**移す＝閉じた門の中に立てる。**∴「開いた門へ石を押し込む→門を閉じる→閉じた門の中に立ってさらに押す」で閉じた門越しに石とプレイヤーを渡せた。色ゲート固有ではなく `T`（モーメンタリ）／潮 `=` でも同型＝背骨のバグ。** **最終的に入れた規則は1つだけ：押した後にプレイヤー（敵）が入る「石の元セル」の**下地**が閉じていたら押せない**（`passable.js statefulTileClosed` を石を無視して呼ぶ・押し先は通常の `tilePassable` に任せる）。**⚠️ 一度は「石は `STONE_UNPUSHABLE_TILES`（`T` `=` `(` `)` `|`）へは**開いていても**押し込めない」という強い規則も入れたが撤回した**＝廊下C3（`field 15,14`・2026-07-27 出荷）が「石でボタンを押さえて開けた潮ゲートを通して別の石を運ぶ」設計で、強い規則がこの正当なパズルを壊した（`tests/field-corridor-o.spec.js ⑧` が実際に赤になった）。**∴開いた門への押し込みは正当な語彙として残す。** **敵側（`enemy-ai.js tryEnemyPushStone`）とソルバー（`scripts/lib/blade-solver.mjs`）にも同じ規則を入れた**（片方だけだと敵に押させる抜け道／バグ前提の難易度測定が残る）。**(B2)＝開いた門に石を置いてから門を閉じると石が門の中に残る、は許容して文書化した**（未解決のまま画面を出れば石位置はリセットされる＝`game.js enterStage`∴倉庫番のデッドロックと同じ回復可能な状態。∴設計側の作法＝石が絡む門は再び開けられる形にする）。**結果＝4枚（20,0〜23,0）の実測値は完全不変（L=33/40/56/89・デッドロック・強制手率・状態数まで一致）。5枚目 24,0 は解なしになった∴パズルとしては撤回し、`fixture:true` の回帰フィクスチャに転用**（migrate は「解けたら失敗」に反転・EXPECT は `L:null`／状態 46,836・spec に Ⓐ〜Ⓔ の回帰5本＝Ⓒ/Ⓔ は「開いた門へは押し込める」側と「門が絡まない普通の押しは通る」側＝過剰ブロックの回帰も張った）。修正前の挙動が赤になることも一時 revert で裏取り済み。**🔑 教訓＝(a) バグ修正の規則は出荷済みコンテンツで回すまで確定しない（フル suite が C3 で捕まえた）／(b) 過剰修正は正当なパズル語彙を殺す＝禁止は最小限にして「バグの実体そのもの」だけを塞ぐ。**

- [x] **4.6. 方法論の実プレイ検証＝お試しパズル4枚（易/中/難/激難）を作りユーザーが解いて判定する（2026-08-02 完了）（🧠 Opus）**。**✅ (6) ユーザー判定の結果（2026-08-02）＝4枚（20,0〜23,0）は実際に遊んで確認済み。判定＝「方法論は成立している。ただし**もっと難しいものが必要**」**＝この判定が 24,0（石＋色スイッチ/色ゲートの合成）を作る動機になった（その 24,0 がバグ依存だったので 4.57 でバグを先に直した）。∴ 4.6 はここで完了とし、**残りは「24,0 をバグなしで可解なパズルに作り直す」＝4.7** に引き継ぐ。（当初は3枚＝2026-08-01 のユーザー指摘で「難より上」の激難を追加。2026-08-02 のユーザー選択「C」で5枚目「合成」を追加したが、**同日の 4.57 で撤回＝4枚に戻した**）。**この位置の理由（2026-07-31 ユーザー指摘で実行キューを是正）＝4.5 を `[x]` にしたが、PROGRESS 現在地では「生成盤面を実際に遊べる `test_mechanics` ステージ3枚に落とし、ユーザーがブラウザで解いて『十分な難しさ／理不尽でない』の線を判定する＝これが 4.5 の真の合否」が未完のまま残っていた。実行キューが取りこぼしていた＝ここに明示的に積み直す。** ユーザーの根本要件＝「PUZZLE-DESIGN を元に実際に難しいパズルを作れるか試さないと 5番の鍵配置の設計もできない／人が十分な難しさと感じるものができないと意味がない」。**やること：** (1) 3〜4石で易/中/難の壁テンプレを作り分ける（§3-2c 確定＝難易度の主役は石数でなく壁の幾何・石数レンジ3〜4）→(2) 逆算生成（`generate-sokoban.mjs`）→(3) 4軸測定（`measure-puzzle.mjs`／`puzzle-metrics.mjs`）で §2 下限条件を満たすものを選別→(4) 実プレイ化の型＝既存 `test_mechanics/4,0 [enemy_stone]`（石→ボタン→`links`でゲート`T`→宝箱・ボタンを壁で囲む）に倣い `test_mechanics` レイヤーへ3枚組む→(5) 実ブラウザで確認→(6) **ユーザーが解いて「十分な難しさ／理不尽でない」を判定**（＝これが合否）。**測定コスト＝3〜4石は deadlock が数十万〜で `measureMetrics` が重い∴生成＋測定はバックグラウンド実行前提。難帯は逆算の引き距離で深さを保証し順ソルバーはサンプル検証に限定する妥協が要り得る（§3-2c）。** これが済んで初めて 4.6 完了→キュー5番へ。**方法論を鍵の関門設計（5番）に適用する最初の対象。**
      **🔄 実装側 (1)〜(5) 完了（2026-08-01・残るのは (6) ユーザー判定だけ＝このチェックボックスはユーザー判定が済むまで `[ ]` のまま）：**
        - **(1)(2)(3) 実プレイ形のまま生成・測定する新スクリプト `scripts/generate-sokoban-playable.mjs`。** 前セッションの学び（測定盤面を後から `T` で隔離すると L が激減する）を織り込み、**最初から「宝はゲート `T` の奥・ゴール＝石ロック成立かつ宝セル到達」の形で逆算生成＋4軸測定**する。壁テンプレは3帯とも「ピラー部屋」（開けた部屋は実測で必ず貪欲に解ける＝作業ゲー／1幅回廊だけだと石3が即詰みする＝§3-2c のスイートスポット）。**採用値＝易 L=33／中 L=40／難 L=56・3枚とも貪欲NG・デッドロック 3.1万/16.5万/159万・強制手率 0.06/0.12/0.07・最短解本数 1/6/22。** 帯は引き距離でなく**実測 L** で分けた（引き距離は歩きが支配的で L と相関が弱い）。
        - **(4) `scripts/migrate-test-sokoban-tiers.mjs`（自己検証型）で `test_mechanics/20,0`（易）/`21,0`（中）/`22,0`（難）へ書き込み。** 書く前に5検査＝①10×12の文字配列②ゲート閉のまま宝へ到達不能③ボタン↔ゲート歩行距離 ≥2（足踏みで開いた瞬間に1歩ですり抜けるのを封じる）④**書き込む盤面を再測定して記録値と一致**（測定値と実データの乖離を防ぐ）⑤row1 連絡通路の連続性。`tests/test-stage-keys.js` に `sokoban_easy`/`sokoban_medium`/`sokoban_hard` を追加。
        - **(5) `tests/sokoban-tiers.spec.js` 8本＋実ブラウザ目視。** ①静的な非空虚性（ゲート閉で宝に届かない・ボタン3・石3）②実機ロード（3枚ともゲート閉・0 pageerror）③**ソルバーの最短手順を実エンジンで再生して易盤面を実際に解く**（非デバッグのセーブ注入経路＝debugMode OFF・全ボタンに石／`stonesLocked`／ゲート開／宝を開けてルピー30 を確認）④ロック条件の回帰（足踏みで全ボタンONならゲートは開くが `locked` は立たない・足を離すと閉じる・3個目を石で埋めた瞬間にロック）。**フル 448 passed（既存3失敗＝melee-only-boss⑦/sea-lord-boss⑩/test-new-dungeons dungeon_5 は `git stash` で変更前も同じ3件が落ちることを確認済み＝無関係）。** 3枚のスクリーンショットで「ピラー部屋・石3・ボタン3・ゲート＋宝」が実際に見えることも確認。
        - **🆕 4枚目「激難」を追加（2026-08-01 ユーザー指摘＝「なかなかいいね。でも難よりももっと難しいものも必要だと思う」）→ `test_mechanics/23,0`（`sokoban_extreme`・ルピー200）。** **一段上げる主役は石数（3→4・§3-2c の上限）**＝L を伸ばすだけでは歩きが増えるだけで思考は増えない。石4で干渉の方向が増え、ボタンを部屋の4隅寄りに置くと運ぶレーンが必ず交差する（ピラーは行ごとにずらした千鳥＝まっすぐ運べる石が存在しない）。**採用値＝L=89（難 56）・貪欲NG・デッドロック 185万（難 159万）・強制手率 0.13・最短解本数 2（難 22）・状態 235万。** 選び方は新しい `pick:'maxThin'`＝「最深帯（最大 L の95%以上）の中で**最短解が最も細い**もの」（最深の1件は解が168本＝どう運んでも同手数＝太い。L を数手諦めて軸④の細さを採る方が「一本道を見つける快感」が出る）。**部屋を1列縮めた版は L が 69 で止まった**（運ぶ距離が足りない）∴壁の幾何は難と同じ広さに据え置き石だけ +1 した対照にした。石4は配置候補が桁で増える∴`CAND_CAP`（既定700）で引き距離順に等間隔サンプル＝**この帯の探索は全候補ではない**ことをログで明示（黙って切り詰めない）。テストは 11本に増やし、`ボタン3個`固定の検査を帯ごとの石数に一般化＋**「記録 L が 易<中<難<激難 の順に深い」静的検査**を追加（盤面差し替えで浅くなる事故を検出）。**フル 451 passed（既存3失敗のみ）。**
        - **❌ 5枚目「合成」は撤回（2026-08-02・下の 4.57 のバグ修正の帰結）。** 以下の記録は経緯として残すが、**採用値（L=91 等）はバグ入りエンジンで測った数値＝再利用しない**。修正後の `test_mechanics/24,0` は**解なし**＝回帰フィクスチャに転用した（4枚＝20,0〜23,0 だけが遊べるパズル）。**生き残る学びは3つだけ**＝(a) 単調な関門（潮 `=`／弓 `Y`）では難しくならない (b) 排他（非単調）な色スイッチだけが制約を増やす (c) 並列に置いた穴は片方が飾りになりやすい。**新たに確定した硬い制約＝石は閉じ直せる門を渡れない∴色ゲートは「石の搬送路」ではなく「プレイヤーの通路」に使う**（再設計＝下記 4.7）。
        - **🆕（撤回済み）5枚目「合成」を追加（2026-08-02 ユーザー選択「C＝石＋別ギミックの合成パズルを1枚試作」）→ `test_mechanics/24,0`（`sokoban_color`・ルピー300）。石は3に戻し、難化の主役を「石数」から「排他（非単調）な関門」へ移した。**
              - **第2ギミックの選定＝色スイッチ `[`/`]`＋色ゲート `(`/`)`。潮ゲート `=`・弓ゲート `Y` は候補から落とした**＝どちらも「開くと通れる場所が増えるだけ」＝**単調**で、最適プレイは「最初に開けて以後放置」＝手数が数手伸びるだけで思考は増えない（激難で学んだ「長いだけ」と同じ罠）。色スイッチは**その色にセット**（トグルではない）＝赤を開けると青が閉じる**排他＝非単調**∴石の搬送路が「今どちらの色か」に依存する。
              - **幾何＝直列2枚のエアロック。** col5 の仕切りで部屋を東西に割り、通れる穴は row4 の1レーンだけ。そこに赤ゲート (4,5) → 青ゲート (4,6) を**直列**に置く。**最初に試した「穴を上下2箇所（上=赤・下=青）に分ける並列案は失敗**＝生成後の必須性検査で「赤を壁で塞いでも L=70 のまま」＝最短解は青の穴しか使わず**赤が飾り**だった。並列だとプレイヤーは片方だけで済ませられる∴**直列にして両方を通らせる**のが合成の作り方。**❌ ただしこのエアロックは「石を門越しに渡す」形＝修正後のエンジンでは石が門を通れない∴この幾何そのものは成立しない**（4.57）。
              - **採用値＝L=91（激難 89）・貪欲NG・デッドロック 340万（激難 185万）・強制手率 0.06・最短解本数 348・状態 585万。** 最短解本数は激難（2本）より桁で太いが、最深帯（最大 L の95%＝86 以上）に入るのは L=91（348本）と L=89（2040本）だけ＝`maxThin` は帯の中では最も細いものを選んでいる。深さを取るか細さを取るかのトレードオフで**深さを取った**（合成帯の狙いは「順序制約」なので、細さより「両色が必須」が本質）。
              - **飾りにしない担保を2段で入れた：** ① 静的＝`forceCross`（色ゲートを閉じたときの島ごとに石数とボタン数が食い違うことを要求）＝色ゲートを1度も通らずには解けない。**これが無かった初回生成は「西1石/西1ボタン・東2石/東2ボタン」で下限をクリアしていた＝測定上は高難度なのに色ゲートを一度も通らない空虚な盤面**だった。② 動的＝採用後に**穴を1つずつ壁にして再測定**し「両方とも塞ぐと解なし＝必須」を確認（今回そうなった）。
              - **ハードロックを作らない不変条件（新規 assert）：** 石を連絡通路 row0-2 へ押し出せない（通路の石は隣ステージへの継ぎ目を塞ぐ）／色ゲートを両方閉じたままでも入口から全床へ歩ける（＝どの部屋からも通路へ出られる＝詰んだら隣へ出て戻れば石がリセットされる）。**単一出口の部屋は採らない**＝出口に石を2連で並べると押し出せず脱出不能になる。
              - **ソルバー側の修正2件（どちらも「実エンジンと乖離したまま測っていた」類）：** ① `[`/`]` を**踏める**ように明示上書き（`connectivity.mjs HARD_BLOCKED` は field 指標のベースライン維持のため保守的に壁扱い＝そのままだと「スイッチの上に立って石を押す」立ち位置が消え、合成盤面は状態 730 で解なしになった）。② **矢/ビームでも色スイッチをセットできる**（`projectile.js:501`）＝ヘッダの「矢では反応しない」記述が誤りで、弓ループに色セット遷移を追加した（＝遠くから色を変える手を見落とすと L を過大に測る）。**∴L=91 は道具あり（弓）で測った値＝剣だけなら手数はこれ以上。可解性はどちらでも同じ（剣は隣接で必ず叩ける）。**
              - **テストは 17本に増やした（`tests/sokoban-tiers.spec.js`）。** 合成専用に Ⓐ色ゲートが直列2枚＋赤青スイッチが揃う Ⓑ島ごとの石数≠ボタン数 Ⓒ色ゲート閉でも全床へ歩ける（ハードロックなし）Ⓓ**実機で排他を再現**（青のまま赤ゲートへは石を押せない→赤に替えて1マス→青ゲートは赤のままでは通せない→青に替えて通す→エアロックを抜ける）。**❌ 当時ここに「Ⓓで実エンジンの押し規則も裏取りできた＝押し成功時 `player.js:447` は通行判定なしでプレイヤーを石の元セルへ移す∴閉じた色ゲートの上に立つ（ソルバーも行き先セルだけを見る＝乖離なし）」と書いたのは誤り。** 乖離が無かったのは事実だが、**両方が同じバグを持っていた**だけ（＝B1／閉じた門の中に立てる）。ユーザー指摘で 4.57 として修正＝今は押し先の門と「石の元セルの下地」の両方を見る。**教訓＝「実機とソルバーが一致した」は「仕様として正しい」ではない。** 実機の挙動を正解として写す前に「その挙動が破綻しないか」（＝門が閉じたら石はどうなるか）を問う。副産物の学び＝**入場直後の一振りは `gameNow()` がまだ 0 付近で剣クールダウン（100ms）に食われて空振りする**∴テストは色が変わるまで振り直す。
        - **▶ 残り (6)＝ユーザーが4枚（`test_mechanics/20,0`〜`23,0`＝易/中/難/激難）をブラウザで解いて「十分な難しさ／理不尽でない」を判定する**（＝方法論の合否）。判定で「浅い／理不尽」と出たら壁テンプレか下限条件を直して再生成する。**⚠️ 5枚目 24,0 は解なしのフィクスチャ＝遊ばない。** ブラウザは `G` で DEBUG OFF にしてから（デバッグ中は壁・石をすり抜ける）。

- [x] **4.7. 色ゲート/色スイッチで合成した「23,0（激難）より質で難しい」パズルを仕上げる（2026-08-05 完了）（🧠→⚡ Sonnet＝盤面は確定済み・残りはテスト整備のみ）**。**✅ ユーザー最終判定＝合格（2026-08-05）＝「質で難しくなってる。理不尽でもない」。** 盤面はユーザー自身がeditorで作った26,0＝ユーザー確定の判定として完了とする。
  - **⚠️ このタスクの本体（色ゲートで24,0を難化する）は第2セッション終了時点でまだ着手していない。** §3-2e（I1・I1'・曲がり角強制の旧設計）は**2026-08-04 に丸ごと撤回した**。以下の「(A) 済んだ土台」を必ず前提にして進める（旧 PUZZLE-DESIGN §3-2e の I1/I1' 幾何制約はもう存在しない＝読まない）。
  - **(A) 済んだ土台（2026-08-04・第2セッション完了）＝規則そのものの再設計＋検証環境。**
    - **新規則（旧 I1/I1' を丸ごと置き換えた）＝「色ゲートは閉じている間、石にもプレイヤーと同じ壁を適用する」＋「色スイッチは石を通さない」。** 曲がり角を強制する幾何制約（旧 I1/I1'）は「窮屈すぎる」とユーザー指摘で撤回＝**幾何は自由**（直線上にゲート/スイッチを置いてよい）。
      - `game/player.js setActiveColor()`＝開いたゲートに石が乗ったまま反対色へ切り替える操作は**不発**（`switchDenied` 音・`shared/sounds.js` に新設）。
      - `game/player.js` の `stoneDestOk`／`game/enemy-ai.js tryEnemyPushStone`＝石は色スイッチのマスへ押し込めない（プレイヤーは踏める）。
      - `scripts/lib/blade-solver.mjs`＝`colorSwitchBlocked()` で不発を再現、`passableFor` の `forStone` で色スイッチを石だけ通行不可に。実機とソルバーの乖離なし。
    - **`generate-sokoban-playable.mjs`／`migrate-test-sokoban-tiers.mjs` から旧 I1/I1'（押し込み直線の幾何禁止）を完全削除。** ①②③（宝の非空虚性・ボタン↔ゲート距離・通路への押し出し禁止）はそのまま残る。
    - **笛の新効果 `{type:'resetStones'}`（`game/game.js playFlute`）＝詰み救済。** 未解決の石配置をその場でリセット（`enterStage` の石リセットと同じ規則）。**全ボタン充足済み（`stonesLocked` or 全ボタン埋まり）なら不発**＝解けたパズルは壊さない。「画面外に歩いて出られること」自体が本質でないとユーザー指摘で判明＝**今後 I3（noEscape=0 の強制）はこの救済で満たされるとみなし、新盤面に無理に適用しない**（下記(B)参照）。
    - **`25,0`（`sokoban_gate_push_regression`）を新規則向けに作り直した＝23,0（激難）の壁構造の一部を土台に、赤ゲート1枚(4,5)＋赤スイッチ(3,1)＋青スイッチ(5,10・パズル本体とは無関係の不発検証専用)のパズル（石3・可解・L=76・状態407万・デッドロック233万・noEscape=12132）。** `noEscape` は「石を通路(3,7)/(3,9)へ連続で押し込むと連絡通路の口(2,8)への道が塞がり色を戻せなくなる」正常な倉庫番の詰み（既存4帯と同種・バグではない）＝笛の resetStones で回復できるので `requireEscape` は付けていない。`tests/sokoban-tiers.spec.js` のバグ回帰 describe を Ⓐ〜Ⓖ に刷新（新規則の不発・石不可・笛リセットを検証）＝**18件全緑**。
    - **これで分かったこと＝色ゲートの幾何自由度が大きく上がった。** 旧設計は「ゲート/スイッチの手前2マスが両方床だと違反」という制約のせいで、曲がり角を作るための壁を延々と試行錯誤していた（第1セッションで数十回失敗）。新規則ではこの制約が無いので、**直線通路にそのままゲート/スイッチを置いて構わない**＝24,0の幾何設計はゼロから作り直せる自由がある。
  - **(B) 24,0 本体＝方針変更→盤面確定（2026-08-05・第3セッション）：**
    - **🔴 第3セッションの前半で作った暫定 `24,0`（col2/col8を縦壁にして西袖/東袖を分離する版）は「23,0を土台にする」と言いながら列を潰して別物にしていた＝ユーザー指摘で発覚。** §2下限未達（貪欲で解ける）だったため不採用・後に削除した。
    - **ユーザー判断＝AIが幾何を試行錯誤する進め方を撤回し、`26,0` に23,0の完全複製を置いてユーザー自身が editor で直接編集する方針に変更。** 前提として editor に色ゲート/色スイッチの配置機能が無いバグを発見・修正した（`editor-palette.js` の `PALETTE_CATEGORIES` に4タイルが漏れていた＝PLAN 5-1 は当時実機確認せず `[x]` にしていた）。
    - **✅ ユーザーが `26,0` に色ゲート2枚（赤(5,2)・青(5,7)）＋色スイッチ2個（青(8,2)・赤(8,8)）を配置して盤面を確定した。** AI（自分）は最初、色ゲートを壁扱いにした歩行BFSだけで「全ボタンに到達できる＝飾り」と誤判定した＝ユーザー指摘で発覚し訂正（教訓は PUZZLE-DESIGN.md §6-1・DECISIONS.md に記録済み）。**ソルバーで正しく検証した結果＝色スイッチの遷移を一切禁止したBFSは解なし（726万状態超）＝色ゲートは必須。** 4軸実測＝**L=106・貪欲NG・deadlock=2,653,649・強制手率0.18・最短解320本・状態3,272,882・noEscape=343,552**。§2下限クリア（✅高難度）。I4（両色必須）を静的に確認＝赤ゲート(5,2)を壁化→解なし／青ゲート(5,7)を壁化→解なし。23,0（L=89・最短解2本）をLで上回る。ソルバーで実際の解も1本復元済み（106手・色切替2回）。
    - **✅ 座標は26,0のまま採用と確定＝24,0へ移す作業自体を撤回した。** 「24,0に置く意味は何か」というユーザー指摘どおり、座標に設計上の意味は無かった。暫定24,0（draft:true）は削除し、`tests/test-stage-keys.js` の `sokoban_color` を26,0へ直接向けた。`migrate-test-sokoban-tiers.mjs` の BOARDS 管轄外（手動管理・ユーザーがeditorで直接編集する運用）。
    - **✅ 26,0専用の実機再生テスト＋笛リセット付与 完了（2026-08-05・第4セッション・⚡ Sonnet）。** ①noEscape 測定＝**343,552**（非ゼロ＝25,0と同種の正常な倉庫番の詰みが存在。バグではない）∴PLAN の申し送り方針どおり `fluteEffect:{type:'resetStones'}` を26,0に付与した（`work/blade-of-lumia.json`）。②ソルバー解の再検証＝`noTools:true`（弓なし・剣の隣接叩きのみ）でも解けて **L=106**（comment記載の実測値と一致）＝道具なしでも成立する解が実プレイの再生対象として使える。③`tests/sokoban-tiers.spec.js` に「Phase 4.7 – 合成パズル 26,0（sokoban_color）」describe を新設（4本）＝(a) 静的：色ゲート2枚+色スイッチ2個+ゲートT・閉のままでは宝に届かない (b) 実機：初期状態は両方閉 (c) 実機：ソルバーの最短手順（move103+色スイッチ叩き3・移動系ヘルパー`shortestPlanColor`/`replayMixed`を新設）を`movePlayer`/`swordAttack`で再生し宝（ルピー200）を取得・全ボタンに石が乗り`stonesLocked:true`まで確認 (d) 実機：笛で未解決の石配置がリセットされる。**フル 462 passed（既存3失敗＝melee-only-boss⑦/sea-lord-boss⑩/test-new-dungeons dungeon_5 は無関係・PLAN既知）。** ⚠️ **2026-08-05 訂正＝この「dungeon_5 は board 読込タイムアウトで無関係」という判定（4.55 でも同じ書き方をしている）は誤りだった。**実体は `links:{}` による**入室即死**＝dungeon_5 に入った瞬間 `refreshGates` が TypeError で落ち、board が永久に描画されないのでタイムアウトに見えていた。5番で修正済み（フル 472 passed・失敗0）。**教訓＝「board 読込タイムアウト」を flaky と決めつけるな。描画されない理由が入室時の例外である可能性を先に排除する。**
    - **▶ 残＝ユーザーが実際に26,0を解いて「23,0より質で難しい／理不尽でない」を最終判定（＝4.7の合否）。**
  - **この位置の理由（2026-08-02 ユーザー指摘でキューを是正・2026-08-04 に規則を再設計・2026-08-05 に進め方をAI主導からユーザー主導の editor 編集へ変更して盤面確定）＝4.6 の4枚は判定済みで、その判定が「もっと難しいものが必要」だった。** 5番（鍵の関門）で「鍵に与えられる難易度の上限」を決めるにはこの軸が要る∴5番の前。

- [x] **5. ダンジョン進行の背骨を修復する＝鍵配置に難易度を与える＋ `links:{}` クラッシュ修正（2026-08-05 完了）（🧠 Opus）**。
  - **✅ 完了（2026-08-05）＝背骨は通った。** 内訳：
    - **① `links:{}` の入室即死を潰した（43部屋）。** 症状の実体は PLAN が書いていた「5箇所の古いコード／特定タイルを踏むと落ちる」ではなく**入室した瞬間の即死**＝`game.js enterStage` → `checkStoneOnSwitch` → `refreshGates` が必ず呼ばれ、そこで `for...of {}` が TypeError。∴ボタンを踏むどころか board も描画されずゲームが死ぬ。データを `[]` に正した（dungeon_5 19/20・dungeon_8 23/23・test_mechanics 1/26）うえで、`conditions.js refreshGates` に `Array.isArray` ガードを入れて形式に依らず落ちないようにした。
    - **② 鍵不足は D5〜D8 の4本・各ちょうど −1 だった。** ⚠️ **上の初期監査（D4＝扉4に鍵1／D6・D7・D8＝扉4鍵0）は誤り。** 原因＝`D` の**セル数**を数えていた。実際は 4近傍で連結した `D` は物理的に1枚（`player.js collectDoorRun`）で、境界を跨ぐ扉は両画面に描かれるので畳んで1枚（DECISIONS 2026-07-29 決定2）。畳むと **D4 と dark_tower は収支ゼロ（不足なし）／D5〜D8 が各 −1**。さらに **dark_tower は初期監査の対象からそもそも漏れていた**（最終ダンジョンだけ無検査）。→ D5〜D8 の鍵部屋 `[1,0]` (5,7) に `K` を1個ずつ追加（D4 の既存 `K` と同座標・4部屋すべて床・扉の手前＝新しい詰みを作らない）。
    - **③ 全ての鍵を関門付きにした（床置きの鍵ゼロ）。** 2026-08-05 ユーザー確定＝**まず `killAll`（部屋の敵全滅）で背骨を通し、難易度の強化は後続タスク（5.5）**。既存の床置き鍵（D1〜D4／dark_tower）も遡って是正＝計10個すべてに `showConditions {trigger:'killAll'}`。**関門は3つ塞がないと意味を失う**＝描画（`render-board.js`）・踏んで拾う（`player.js handleTileEvent`）・**ブーメランで運ぶ（`player.js collectFieldItem`）**。描画だけ隠しても見えない鍵の上を歩けば拾えた。
    - **④ 整合性チェッカーを4検査ぶん拡張し、dark_tower を対象に入れた**（`check-dungeon-integrity.mjs`）＝(5) links が配列 (6) 鍵数 == 論理扉枚数（境界畳み込み込み） (7) 全鍵に関門 (8) **鍵ゼロから全扉を開けきれるか**。⚠️ (8) は単発 BFS では作れない＝dark_tower は扉①の奥に鍵②がある正当な段階進行で、「扉を1枚も通らずに全鍵に到達」を要求すると誤検出する。∴**開けた扉を開に足して再走する状態探索（開扉集合の bitmask 探索）**にした。dark_tower/cave_1 は `LAYER_KIND` で「トライフォースを落とすボス・ハートの器・地図/コンパス」の要求だけ免除（緑の意味を保つため）。**結果＝全10レイヤーで 0 エラー / 0 警告。**
    - **⑤ 回帰テスト＝`tests/dungeon-key-gate.spec.js`（7本）。** チェッカーをサブプロセス実行して終了コードで縛る＋鍵の総数が10であることも assert（`K` を消せば「全鍵に関門」は自明に真になる抜け道を塞ぐ）＋実機で「全滅前は描画されず・踏んでも拾えず・ブーメランでも運べない／全滅後は拾える」。**フル 472 passed。** 接続性チェッカー（dungeon 9レイヤー・field）も回帰なし。
    - **⚠️ テスト実装で判明した2点：** (a) **`fromEditor=1` プレビューは `debugMode=true` で鍵無しでも扉を素通りできる**（`passable.js:223`）∴鍵まわりはプレビューで検証できない＝セーブを仕込んで「つづきから」で入る。(b) 鍵部屋の番人 `W` は `isBoss` ＝ HP0 でも `boss.js onBossDefeated` の**実時間**演出（`await sleep` 約2.2秒）を挟んでから消える∴`__game.step(n)` の手動 tick では進まず、実時間で待つ必要がある。
    - **🔑 教訓＝「機械チェックが緑」は「壊れていない」を意味しない。検査していない軸は必ず壊れる。** D5/D8 は入室即死なのにチェッカーは「合格」と表示し、D5〜D8 はボスに永久到達不能なのにどのテストも落ちなかった。**さらに PLAN 自身の監査数値も間違っていた（扉セル数を数えた）＝人手の数え上げは信用しない・判定は単一のスクリプトに集約する。**
  - **（当初の記述・保存）****この位置の理由（2026-07-30 ユーザー指示＋検証で発覚）＝8ダンジョン中5つ（D4〜D8）が鍵不足でボスに到達できず進行不能＝ゲームの背骨が半分成立していない**（深洋O は任意探索なので無関係・背骨はダンジョン側）。**⚠️ 実装は 4番（パズル方法論）確定後**＝ユーザー指示「あまりに簡単に鍵を取得できたらゲームとして成り立たない。強敵全滅・難しいパズル等の難易度を鍵取得に与える必要がある」∴鍵は「置くだけ」でなく「関門を解いて得る」設計にする＝4番の高難度パズル方法論が前提。**検証で確定した事実（自分で実コード裏取り済み）：** (i) 鍵 `K` は `player.keys` のグローバル持ち越し（`game.js:1887`／消費 `player.js:243`／取得 `:742`＝ダンジョン入場でリセットされない）。D1〜D3 は各「鍵1・扉ラン1」で収支ゼロ＝繰り越し無し。**D4＝扉4（ボス部屋両面 `DD`＝2ラン必須）に鍵1のみ／D5＝扉あり鍵0／D6＝扉4鍵0／D7＝扉4鍵0（ギミックは `D`+`T`+`!` と最も豊富なのに鍵ゼロ）／D8＝扉4鍵0**。(ii) **`links:{}` クラッシュ（致命・別件だが同じダンジョン群）＝dungeon_5（19画面）と dungeon_8（23画面）は `links` が空配列でなく空オブジェクト `{}`。`conditions.js:52,68`／`player.js:310,354,750` は `for (const link of stageData.links ?? [])`＝`{} ?? []` は `{}` を返し反復不可 TypeError。dungeon_8 の `3,3`・`3,0` はボタン `S` を持つのに `links:{}`＝踏むと確実に落ちる。** (iii) **`scripts/check-dungeon-integrity.mjs` は D5/D8 を「合格」と誤表示＝鍵総数 vs 扉ラン数も `links` 非配列形式も検査していない（回帰テストの穴）**∴本タスクで整合性チェッカーを両検査に拡張し、修復と同時に回帰ガードを入れる。**⚠️ 実装着手前に「鍵の供給方法＝各ダンジョン内に関門付きで `K` を追加する／持ち越し設計を変える／鍵扉の数を減らす のどれか」をユーザーとディスカッション**（§19-10 と同じ運用）。

- [x] **5.5a. 鍵の関門強化の設計＝鍵部屋への方法論翻訳・型カタログ・10鍵の配分表を確定する（2026-08-05 完了）（🧠 Opus）**。**成果物＝`PUZZLE-DESIGN.md` §7（新規）。** 以下を確定した：
      - **① 鍵部屋10室の位相を実データで棚卸しした（§7-2）＝設計の自由度は位相で決まる。** **10室のうち8室が「実質終端」**（唯一の入口＋その鍵で開ける扉の2辺しかない）∴パズルを部屋いっぱいに置ける。**通り抜けは D1 `1,0` と D3 `1,0` の2室だけ**（東↔南が素通し＝通行路を1本空ける必要がある）。**D4/D6/D8 の `1,0` はワープで入る孤島**（辺で隣室と繋がっていない）＝最も自由度が高い3室∴重い倉庫番はここへ置く。
      - **② 型カタログ4型と「型ごとの合否基準」を決めた（§7-3）。** **⚠️ §2 の4軸下限（貪欲NG・deadlock>0）は単調な道具の関門（弓・爆弾壁・かがり火・はしご）には適用できない**＝§3-2d のとおり単調な関門は必ず貪欲で解ける∴4軸で測ると全部 ✗ になるが、これは盤面が悪いのではなく**指標の誤用**。∴ A 戦闘型＝脅威度の単調増加／B 道具型＝**手順の段数 ≥2 ＋ 道具を抜いたら解なし**／C 倉庫番型＝4軸下限（ただし必須進行路なので**上限 L ≤ 60** も置く＝26,0 の L=106 は寄道の上限）／D 合成型＝C＋I4。
      - **③ 10鍵の配分表を確定した（§7-4）＝型の並び `A B B C B C+B A C A D`（同じ型の3連続なし・道具型はその階で得た道具と一致・重い型は孤島3室と最終ダンジョンに集中）。**
      - **④ 足りない部品を「必要」に更新した（§7-5）＝`stonesPlaced` トリガー。** `allSwitchesOn` では関門にならない＝ボタンはモーメンタリで**プレイヤーが踏んでも ON** ∴「石を n−1 個運んで最後は自分が踏む」で成立し、しかも `showConditions` の達成は恒久（`conditionsMet`）∴**石1個ぶん難易度が落ちる**。ゲート `T` 方式（4.6/4.7）はこの穴を踏まないが、鍵は5番の不変条件で `showConditions` 必須∴石だけで判定するトリガーが要る。実装は既存 `allButtonsHeldByStones` の再利用で軽量。
      - **⑤ 整合性チェッカーの穴を1つ見つけた＝検査(7) は「`showConditions` があること」しか見ていない**∴**対象が存在しない関門（例：`torchesLit` なのに `H` が無い＝永久に達成不能＝進行不能）を通す**。検査(9)「関門トリガーの対象がステージに存在するか」を足す（実装は 5.5b と同時）。
      - **⑥ D1 `1,0` は現状据え置きが結論**（`W`＋`E`×2＝道具ゼロで戦える唯一の関門＝A型の下限として妥当）∴以降のサブタスクに D1 は無い。

- [x] **5.5b. D2 鍵部屋＝道具型①（ブーメランで炎を運び全かがり火点灯）＋整合性チェッカー検査(9)（2026-08-05 完了）（⚡ Sonnet 可・設計は §7-4 に確定済み）**。**この位置の理由＝以降は進行順（D1→dark_tower）に1室ずつ作る＝ユーザーが通しでプレイして判定する順序と一致させる。** 部屋＝`dungeon_2 / 0,1`（終端・入口は東 (4,11)(5,11)・北が鍵扉 `DD`→ボス）。**結果：**
      - **盤面＝中央に祭壇室（外周は回廊）。かがり火3本＝`L(6,4)` を `initLitTorches` で点灯／`A(6,6)` は L と同じ行／`B(4,6)` は A と同じ列だが L とは行も列も共有しない。** ブーメランは同一直線上しか炎を運べない∴**L→A を点けてから投げ位置を変えて A→B**＝段数2で順序が強制される（§7-3 B）。鍵 (5,5) は `torchesLit` で出現。敵 `C` は回廊 (2,8) に置き、投げ位置を作る間の圧だけ残した。
      - **移行スクリプト `scripts/migrate-key-room-d2.mjs` が書き込み前に盤面を自己検査する**＝形（10×12 文字配列）・継ぎ目（東入口・鍵扉・扉の外側）・H の本数と点灯数・**「火元から直接運べる H がちょうど1本」**・**各段の「実際に運べる立ち位置が存在する」**（射程3の直線走査・壁で打ち切り）・入口から鍵/扉/両投げ位置へ歩けること。幾何が崩れたら書き込まない。
      - **検査(9) を追加した（§7-5 ⑤）**＝鍵の関門トリガーごとに対象の実在と成立可能性を見る（`killAll`→敵が居る／`torchesLit`→`H` があり全点灯済みでなく火元か点火手段がある／`switchOn`→`switchId` が `S`/`Y`／`wallBroken`→`wallId` が `!` かつ爆弾所持／`bushBurned`→`u`＋ロウソク／`hasItem`・`flutePlayed`→所持順／**未知のトリガーは error**）。**4通りの壊し方（`H` の無い `torchesLit`・存在しない `switchId`・トリガー名の typo・敵ゼロの `killAll`）を一時的に仕込んで、全部 error になることを確認した**（緑のまま通す検査は検査ではない）。
      - **engine のバグを1つ直した＝`switchOn` は `switchStates`（ボタン `S`）しか見ておらず、`switchToggles`（`Y` を武器で叩く）を見ていなかった**∴`Y` を指した関門は永久に開かない。`refreshGates` は両方を見ていた＝**同じ「ON」の判定が2箇所で食い違っていた**。5.5c（D3 の弓の関門＝`switchOn`）の前提なので先に修正した。
      - **テスト＝`tests/dungeon-key-gate.spec.js` を 6→11 本に。** 「全ての鍵が `killAll`」の固定を**部屋ごとの期待トリガー表**に置き換え（強化の取り消しも未着手部屋の放置も検出する）、D2 に ① 幾何の不変条件（1投で両方点く配置への劣化を止める）② 敵全滅では鍵が出ない ③ **実機2投リプレイ**（セーブ注入→つづきから→投げ→歩いて位置替え→投げ→鍵出現→拾う）④ 剣ではかがり火が点かない（＝道具無しでは解なし）を追加。フル 476 passed。
      - **🔴 見た目の層を間違えた（同日ユーザーが塗り直して発覚）＝祭壇室の砂地を `tiles` 層の `d`SAND で塗っていた。** `tiles` 層の SAND は `ANIMATED_FIELD` に入っている＝`.obj-sprite` の**アニメーションする前景 canvas** として描かれる／`bgTiles` 層の `d` は `bg-sand` クラス＋CSS `background-image` の**静止した下地**∴**同じ「砂地」でも見た目が別物**（`game/render-board.js` の `applyBgTileClass` と `fieldSpriteMap`）。ユーザーが `bgTiles` に塗り替え（祭壇室＝砂地／鍵 (5,5) だけ `o`石畳）＝こちらが正。**移行スクリプトの盤面を `.` に戻し、検査⑧「tiles 層に見た目だけの地面タイル（d/g/o/s/a/m）を混ぜていない」を追加**＝再実行で塗りを巻き戻さない。塗り直し後も整合性/接続チェッカー緑・鍵ゲート11本緑（幾何は不変）。

- [x] **5.5c. D3 鍵部屋＝道具型②（弓の2段）（2026-08-06 完了・⚡ Sonnet）**。部屋＝`dungeon_3 / 1,0`（**通り抜け**＝東(row4,5 col11)↔南(row9 col5,6) の本道を1本空けた・gate の状態に関わらず常時通行可であることを機械 assert 済み）。**盤面＝チャンバー1に Y①(2,3)（水(1,3)(2,2)(2,4)(3,3)で完全隔離）→矢で撃つ→`links`で gate T(5,8) が開く→チャンバー2（Y②(8,9)・水(7,9)(8,8)(8,10)で隔離）へ入れる→(6,9) から矢で Y② を撃つ→`showConditions{trigger:'switchOn', switchId:'8,9'}` で鍵(6,10) が出現。** Y①/Y②とも4近傍が全部 水/壁＝剣では絶対に叩けない（弓必須・段数2）。**実コード裏取り＝矢の通行判定 `game/projectile.js isTilePassableForProj` は WALL/未破壊`!`だけで止まる（水もゲートも素通し）**＝Y を水で隔離すれば「歩いて剣で叩く」を封じつつ「対岸から矢で撃つ」だけ通る関門になる（既存D3`1,1`の弓ゲートと同型・湖/深洋Oの弓ゲートパターンを鍵部屋に翻訳）。新規 `scripts/migrate-key-room-d3.mjs`（D2テンプレと同じ自己検査型＝①10×12②継ぎ目③個数④剣で叩けない⑤Y①は常時矢で撃てる⑥Y②の射線位置はgate開のときだけ到達可（閉のときは到達不能＝段数2の機械保証）⑦本道は gate状態に関わらず常時到達可⑧チャンバー1は常時アクセス可⑨鍵の関門⑩装飾タイル混入なし）。**テスト＝`tests/dungeon-key-gate.spec.js` に④として5本追加**（盤面の幾何不変条件／本道の常時通行可／gate閉でチャンバー2に入れない／Y①→Y②の順で矢を撃つ実機リプレイで鍵取得まで／剣では絶対に届かない）＋EXPECTEDテーブルを`switchOn`に更新。整合性チェッカーは新規の警告1件のみ（水がはしご専用の`ITEM_LOCKED_TILES`対象と誤認されるが接続チェッカーはPASS＝寄道でなく本道の外にある詰め物パズルの水なので想定内）。実ブラウザでチャンバー1→矢→gate開→チャンバー2→矢→鍵出現を目視確認済み。**フル 481 passed（既存3失敗は無関係）。**

- [x] **5.5d. D5 鍵部屋＝道具型③（はしごで水路を渡る2段）（2026-08-06 完了・⚡ Sonnet）**。部屋＝`dungeon_5 / 1,0`（終端相当・入口は南(9,5)(9,6)・西(4,0)(5,0)はボス部屋への出口＝本道はパズルに関与しない）。**盤面＝Y①(4,8) は水(3,9)で隔離＝通行可能な隣接セルは対岸(4,9)の1か所だけ（矢でも剣でも迂回して届く立ち位置が無い）。はしごで対岸へ渡って叩く→links で gate T(5,9) が開く→奥のチャンバー2へ入れる→そこも水(7,9)で隔離した Y②(8,8) を同じ要領で渡って叩く→showConditions(switchOn) で鍵(6,10) が出現。** D3（弓の2段）と違う核心＝矢は水を素通しするので「隔離すれば剣封じ」で足りたが、はしご関門は「対岸から矢で狙えない」ことも同時に保証する必要がある＝Y①/Y②の唯一の隣接床セルを水の向こう側に置き、矢を放てる立ち位置自体をチャンバー外に作らない設計にした。**新規 `scripts/migrate-key-room-d5.mjs`（D2/D3と同じ自己検査型・11項目）**＝①10×12②継ぎ目③個数(Y2/T1/K1/水2)④Y①/Y②とも通行可能な隣接セルがちょうど1つ（水の対岸）⑤その水がはしごで渡れる1セル幅の橋⑥⑦⑧到達性（はしご有無×gate開閉の4状態BFS＝はしご無しでは対岸に到達不可・はしご有りgate閉ではチャンバー2に到達不可・gate開で鍵に到達可・本道はどの状態でも到達可）⑨鍵の関門⑩装飾タイル混入なし⑪敵存在。**テスト＝`tests/dungeon-key-gate.spec.js` に⑤として6本追加**（盤面の幾何不変条件／本道の常時通行可／はしご無しでは対岸に立てない／gate閉でチャンバー2に入れない／はしご有りでY①→Y②の順に叩くと鍵が出現し拾える実機リプレイ／本道はパズル状態に関わらず西出口まで通れる）。**既存の汎用killAllテスト（LAYER/ROOM/KEY_R/KEY_C定数を使う3本）は dungeon_5/1,0 から dungeon_1/1,0 へ差し替えた**（5.5dでD5がswitchOn型に変わったため。D1は§7-2⑥で「据え置き確定」の唯一の戦闘型のみの部屋）。EXPECTED表の `dungeon_5/1,0` を `killAll`→`switchOn` に更新。**フル 493 passed（既存の3失敗は今回発生せず＝再現しなかった）。** 整合性チェッカー・接続性チェッカー（`--with-ladder` 込み）ともに dungeon_5 は0エラー0警告。実ブラウザで盤面表示・全手順（Y①叩く→gate開→チャンバー2→Y②叩く→鍵出現→拾う）をスクリーンショット付きで確認済み。

- [x] **5.5e. D4 鍵部屋＝倉庫番型①（石3・純）＋`stonesPlaced` トリガー新設（2026-08-06 完了・🧠 Opus）**。部屋＝`dungeon_4 / 1,0`（**孤島**＝ワープ `>`(7,9) で入る）。**結果：**
      - **`stonesPlaced` トリガーを新設した（`game/conditions.js evaluateConditions`）**＝既存 `allButtonsHeldByStones` をそのまま共有＝`refreshGates` の `stonesLocked` と**同じ判定関数**なので「ロックと関門が必ず同時に成立する」。`allSwitchesOn` を使えない理由（§7-5）はテストで実機に固定した＝**石2個をボタンに乗せ残る1個を足で踏むと `switchStates` は全 ON になるが鍵は出ない**（この状態を作る手順もソルバーの BFS で求めている＝手で書いた手順ではない）。
      - **3経路ガードは追加コード不要と確認した**＝描画（`render-board.js` 4箇所）・踏んで拾う（`player.js:808`）・ブーメラン（`player.js:989`）はいずれも `showConditions[posKey]` + `conditionsMet` の**トリガー非依存**の判定∴新トリガーは自動的に塞がれる。テストで実機に固定（描画0枚・踏んでも `keys` 0・ブーメランでも増えない）。
      - **盤面＝石3（3,1 / 7,6 / 8,2）・ボタン3（1,1 / 4,4 / 8,7）・鍵(5,7)。実測 L=32（帯 20〜35）・貪欲では解けない・deadlock=90,358・強制手率=0.05・最短解2本・状態123,833・脱出不能0。** 新規 `scripts/generate-key-room-d4.mjs`（逆算 pull-BFS＋4軸測定・実マップ不変）＋`scripts/migrate-key-room-d4.mjs`（書き込み前に generate 側の解析器を**import して再利用**＝規則を2箇所に書かない）。
      - **🔴 v1 の形（東に1幅の袋小路を2つ彫って宝箱とワープを置く）は実測で全滅した＝4軸は全候補クリアなのに `noEscape` が常に非ゼロ**（例 1,248/162,757）。原因を `.scratch` の診断で特定＝**袋の喉に石が縦2個並ぶと双方動かせずワープに永久に行けない**（真のハードロック）。∴**袋小路をやめ、東列を「石が絶対に入れない安全地帯（前室）」にして口を2つ持たせる形に作り替えた**（宝箱もワープも前室の中）。作り替え後は測定した全候補で `noEscape=0`。
      - **`fluteEffect:{type:'resetStones'}` は付けたが、D4 の詰み救済にはならない**（笛は D8 の報酬＝進行順 D4→D6→D5→D8→D7 で D4 時点は未所持）。∴救済は **(a) ワープ `>` の双方向性**（出ると `enterStage` が未解決の石を初期位置へ戻す）**(b) 実測 `noEscape=0`**（脱出判定＝プレイヤーがワープセルに立てる）**(c) 前室に石が入れないことの静的な帰納法**（「石は初期に前室に居ない」＋「前室の全セル・全方向で、押し元が前室の外なら押すプレイヤーの立ち位置が壁」⇒ 永遠に入れない）の3段。詳細は DECISIONS 2026-08-06。
      - **この部屋には敵を置かない**＝敵は `enemy-ai.js tryEnemyPushStone` で**石を押す**（`stonesLocked` になるまで止まらない）∴測定した倉庫番が崩れる/詰む。D5 の移行スクリプトは逆に「敵が1体は居る」を要求していたが、倉庫番型は敵ゼロを要求する（`check-dungeon-integrity.mjs` の `stonesPlaced` ケースも敵が居たら error）。
      - **検査(9) に `stonesPlaced` を追加**（ボタン `S` がある／石 `*` がある／石 ≥ ボタン／敵が居ない）。**5通りの壊し方（石ゼロ・ボタンゼロ・石＜ボタン・敵1体・トリガー名の typo）を壊したマップのコピーで実行して全部 error になることを確認**（緑のまま通す検査は検査ではない）。そのために `check-dungeon-integrity.mjs` に `BLADE_MAP_PATH` の読み替えを足した（実マップは触らない）。**⚠️ 検査を1つ削った**＝「石が初期からボタン上にある」は tiles では表現不能（1セルは `S` か `*` の一方だけ）＝壊そうとすると単に「ボタンが1個少ない部屋」になる∴書いても永久に発火しない死んだ検査だった。石とボタンを別リストで重ねる移行/生成スクリプト側にだけ残した。
      - **テスト＝`tests/dungeon-key-gate.spec.js` に⑥として6本追加**（盤面の形／前室へ石を押し込める向きが無い＋口が2つ以上／ソルバー最短解が記録値 L=32 と一致／入室時は描画されず踏んでも拾えない／ブーメランでも運べない／**足踏みでは鍵が出ない**／最短手順の実機再生で鍵出現→ロック→拾う）。EXPECTED 表を `stonesPlaced` に更新。**フル 494 passed・失敗0。** 整合性チェッカー全レイヤー0エラー（既存のD3警告1のみ）・`check-dungeon-connectivity.mjs dungeon_4` PASS。
      - **⚠️ 実機再生で踏んだ罠＝ワープセル(7,9) に立って再生を始められない。** `MAP_ENTER` の再遷移クールダウン（1.5秒）は「ワープで到着した直後」だけ効くので、そこへ**歩き込む**と即座に隣室へ飛ぶ（実測で `room` が `1,1` に変わった）。∴実機再生は前室の1つ北 (6,9) 始点で別に手順を求め、手順がワープセルを踏まないことも assert した。記録値 L=32 はワープ始点の測定値（テストは BFS だけで別に固定）。

- [x] **5.5f. D6 鍵部屋＝倉庫番＋道具（石3＋爆弾壁）（2026-08-06 完了・🧠 Opus）**。部屋＝`dungeon_6 / 1,0`（**孤島**＝ワープ `>`(7,9) で入る）。芯＝爆弾で搬送路を1本開けないと石が目標に届かない＝**爆弾壁を壊さない状態では解なし**を I4 と同じ動的検査（壁を戻して再測定）で担保する。**結果：**
      - **盤面＝ボタン3（1,1 / 4,4 / 8,7）・石3（3,4 / 7,6 / 8,4）・鍵(5,7)・壊せる壁 `!`(1,2)。実測 L=39（帯 34〜39）・貪欲では解けない・deadlock=1,037,378・強制手率=0.03・最短解1本・状態1,408,454・脱出不能0。** 新規 `scripts/generate-key-room-d6.mjs`（D4 と同じ逆算 pull-BFS＋4軸測定＋I4）＋`scripts/migrate-key-room-d6.mjs`（書き込み前に generate 側の解析器を import して再利用）。
      - **爆弾必須性の作り方＝「部屋を割る壁」ではなく「1セルの袋（ポケット）の喉」にした。** ボタン (1,1) の**床の隣人が `!`(1,2) ただ1つ**∴石をそこへ入れる押しは「`!` の上の石を押す」だけ＝壊すまで石は絶対に届かない。**部屋を割る壁は却下**＝壁の向こうにプレイヤーが取り残されるとワープに戻れず `noEscape > 0`（D4 v1 を全滅させた失敗と同型）。
      - **I4（道具の必須性）は3通で測った**＝`!`→`#` 固定は**解なし**（状態642,810）／`!`→**歩けるが石は通れない**タイル（`[`）も**解なし**（状態1,400,516）＝この壁が塞いでいるのはプレイヤーの通路ではなく**石の搬送路**／`!`→`.`（最初から穴）は解ける L=38＝**L の差 1 手がちょうど爆弾を置く1手**（盤面自体は成立している＝解なしの原因が石配置ではないことの裏取り）。
      - **難易度の帯を D4 32 < D6 39 < D8 40〜60 に収めた**（当初 34〜44 で探索したら L=43 が採れてしまい D8 の帯と重なった＝**帯を 34〜39 に狭めて再探索**し、L=39・最短解1本（D4 は2本）の方が薄くて良い盤面だった）。
      - **詰み救済は D4 と同じ三段**（東列＝石が絶対に入れない安全地帯／口2つ／実測 `noEscape=0`）＋**爆弾の消費に対する救済**＝ワープは双方向で戻れる・爆弾は敵ドロップで補充できる（`combat.js` の抽選は在庫が少ないほど爆弾が出やすい）・壊した壁は `ss.brokenWalls` に入り `save.js` で永続化される∴**爆弾を切らして入室してもハードロックにならない**。爆弾は D6 の報酬（`[1,2]` の宝箱）＝鍵部屋へ来る前に必ず取れる。
      - **🔴 既存バグを1つ見つけて直した（5.5f の副産物）＝D6/D7/D8 の鍵部屋 `1,0` はどれも鍵を2個持っていた**（タイル `K`(5,7) ＋ `floorItems['6,5']`）のに、`check-dungeon-integrity.mjs` の `keyCells()` が**タイル `K` しか数えていなかった**∴検査(6)「鍵の数 == 鍵扉の枚数」は緑のまま、実ゲームでは**鍵が3個余っていた＝`player.keys` はグローバルなので dark_tower の鍵扉2枚をパズル抜きで開けられた**（5.5 で積み上げた関門を丸ごと迂回できる状態）。直しは2点セット＝① 余剰の床置き鍵を削除（`scripts/fix-stray-floor-keys.mjs`＝同室に `K` が無い床置き鍵は「唯一の鍵かもしれない」ので削除せず error にする安全弁付き）② `keyCells()` が `floorItems` の鍵も数えるよう拡張。**壊したマップのコピーで①を戻して②が error 2件（鍵の収支 +1・床置きの鍵に関門が無い）を出すことを確認**（緑のまま通す検査は検査ではない）。
      - **テスト＝`tests/dungeon-key-gate.spec.js` に⑦として9本追加**（盤面の形／ポケットの隣人が壊せる壁だけ＝爆弾必須が幾何で決まる／前室へ石を押し込める向きが無い（`!` が壊れた後の最悪ケースでも）＋口2つ以上／最短解が記録値 L=39 かつ爆弾の手をちょうど1手含む／**I4 の動的検査＝`#` でも「歩けるだけ」でも解なし**／入室時は描画されず踏んでも拾えない／ブーメランでも運べない／**実機で壊す前はポケットに入れず爆弾で壊すと入れる**／**最短手順（爆弾込み）の実機再生で鍵出現→ロック→拾う**）＋EXPECTED 表を `stonesPlaced` に更新＋**「ダンジョンに床置きの鍵が1個も無い」回帰テスト**（上の表はタイル `K` しか見ない∴別軸で縛らないと再発する）。`startAt` に `bombs` オプションを追加。**フル 505 中 504 passed**（1件＝`dark-tower.spec.js` の飛行テストが並列実行時のみ落ち、単体では緑＝既存の flaky・今回の変更と無関係）。
      - **⚠️ 実機再生で踏んだ罠＝座標を丸めて比較すると再生が崩れる。** 通常移動は半セル刻み∴`Math.round` で比較すると「まだ半分」の位置を到着と誤判定し、次の手が半セルずれた場所から始まって数手先で別のセルに居る（`8,4` を待っているのに `7,6` に居た）。**素の座標で比較する**（D4 の `d4Replay` は最初からそうしていた）。爆弾の手は「プレイヤーが動かない遷移」＝`useSubItem()` を撃って `brokenWalls` に `1,2` が入るまで実時間で待つ（導火線 2000ms）。

- [x] **5.5f2. ユーザー報告のバグ修正＝境界を跨ぐ鍵扉に埋まって動けない（2026-08-06 完了・🧠 Opus・0番＝割り込み枠）**。**この位置の理由（2026-08-06 ユーザー割り込み＝5.5f で作った D6 鍵部屋を実機で遊んで報告：「パズルをクリアして鍵をとって左のステージに移動したら、ドアの上に埋まった状態になって動けなくなった」）＝報告どおり実コードのバグで、しかも恒久詰み（セーブにも残る・自力復帰不能）。D4/D6/D7/D8 のボス入口と dark_tower の2枚＝計6枚の論理扉が同じ形∴5.5 で作った関門の先へ正当な手段で進めない状態だった＝先に直す。**
      - **原因は2つ重なっていた。** ① **データ＝1枚の扉を境界の両面に `D` で描いていた**（2026-07-29 の決定2「境界扉は両面に描く」の帰結）。エンジンの扉は部屋単位（`ss.openedDoors`／`player.js collectDoorRun` は部屋内の連結成分だけ）∴手前を鍵1個で開けて隣室へ抜けると隣室側の `D` は閉じたまま残る＝**同じ1枚の扉が実質2枚**。② **エンジン＝`game.js arrivalTileBlocked` が `'D'` を常に着地可としていた**∴閉じた扉セルの上に着地する。半セル移動＋`Math.floor(v+0.5)` のタイル解決では4方向すべての次の半歩が同じ扉セルに解決される＝**一切動けない**。鍵は消費済み∴自力で開けられない。
      - **直し＝3点セット。** ① データ側で**片面だけ**にする（`scripts/migrate-boundary-doors.mjs`＝着地側の `D` を `';'`＝常時開放の境界通路に置換。鍵を持って来る側＝同室に `K` がある側を残す・両方/どちらにも `K` が無ければ error で止まる。12セル＝6枚の論理扉を変換）② `arrivalTileBlocked` が `openedDoors` を見て**閉じた `'D'` への着地を拒否**（＝鍵が足りなければ遷移そのものが起きない＝ユーザーの言う「左へ行こうとした時点で拒絶」）③ `check-dungeon-integrity.mjs` に**検査(6b)＝境界を跨ぐ両面 `D` を error** にする（再発は必ずデータ側から来る）。**`D` を `STATEFUL_TILES` に入れる案は却下**＝`tilePassable` の `D` 経路には debugMode の素通り免除がある∴そこに混ぜると免除の意味が変わる。
      - **ユーザーの詰みセーブは①だけで自動復帰する**ことを確認（部屋 `1,0`・x=-0.5 → 着地側が `';'` になった∴次の半歩で `0,0` の (4,9) へ抜けられる）∴救済コードは足さなかった。**2026-07-29 の決定2 は撤回**（DECISIONS.md に撤回と ⚠️訂正 を追記＝当時「境界を跨ぐ `D` は0件」と書いたのが事実誤認）。
      - **テスト＝`tests/boundary-door.spec.js`（3本）。** ①全レイヤー静的＝境界上の `D` の対向セルは `D` ではない ②`dungeon_6 [1,0]→[0,0]` を鍵1個で抜けて**着地後に動ける** ③`dark_tower [1,3]→[1,2]` に鍵0で突撃＝**遷移しない**が境界セルから南へ戻れる。**②③は debugMode（`fromEditor=1`）では鍵扉が素通りになる∴セーブ注入＋「つづきから」で本編と同じ経路で起動する**。③は修正を一時撤去すると赤になることを確認（歯があるテスト）。壊したマップのコピーで検査(6b)が error を出すことも確認。**フル 508 passed**・接続チェッカー PASS・整合性 0 error（既存の D3 水 warning 1件のみ）。
      - **🔑 教訓＝「状態を持つタイル」は着地判定と通行判定の両方に同じ状態を通さないと片方だけ穴になる。** 通行（`tilePassable`）は閉じた扉を弾いていたのに、着地（`arrivalTileBlocked`）だけが素通しだった＝**遷移で入る経路はプレイヤーの通常移動とは別の入口**∴タイルを増やすたびに両方を見る。

- [x] **5.5g. D8 鍵部屋＝倉庫番型②（石4・関門の上限帯）（2026-08-07 完了・🧠 Opus）**。部屋＝`dungeon_8 / 1,0`（孤島）。ボタン4個（1,1／4,4／8,1／8,7）に石4個（3,2／3,4／6,1／8,4）を乗せると `stonesPlaced` で鍵(5,7) 出現。**実測 L=60（帯 40〜60 の上限に張り付いた＝候補が全て上限超か上限張り付きだったため §7-6手順11 の「上限に張り付いたら最短解が細い方を採る」を適用）・貪欲NG・deadlock=1,470,558・強制手率0.07・最短解4本・状態1,972,594・脱出不能0**。
      - **`scripts/generate-key-room-d8.mjs`（新規）＝D4 の純倉庫番テンプレを4ボタン/4石に拡張。** 候補は桁で増える（引き距離26まで・展開状態51万・候補10,165通り）∴`PULL_CAP`（逆算BFSの展開上限）・`CAND_CAP`（候補の等間隔サンプル）・`MAX_MEASURE`（フル測定の間引き）の3箇所で**打ち切りを明示ログ**する。当初「深い候補優先」でサンプルしたら L=60〜96 ばかりで帯を外れた（**L と引き距離は単調対応しない**＝盤面の配置次第で同じ引き距離でも L が数十変わる）→**引き距離の分布全体から等間隔サンプル**する方式に直して帯40〜60の候補を捕まえた。
      - **`scripts/migrate-key-room-d8.mjs`（新規）＝generate 側の analyze/assertGeometry/measurePlacement をそのまま import**（D4/D6 と同じ作法）。宝箱が無い部屋（元から chestContents 空）＝D4 にあった宝箱まわりの検査（engine/ソルバーの stoneDestOk 食い違い回避）は無い。書き込み前に9項目を検査（帯40〜60・L=60固定・noEscape=0 等）。余剰の床置き鍵 `floorItems['6,5']` も削除。
      - **テスト＝`tests/dungeon-key-gate.spec.js` に⑧として7本追加**（盤面の形／前室へ石を押し込める向きが無い＋口2つ以上／最短解が記録値 L=60 と一致／入室時は描画されず踏んでも拾えない／ブーメランでも運べない／石3個＋足踏みでは鍵が出ない／**最短手順の実機再生で鍵出現→ロック→拾う**）＋EXPECTED 表を `stonesPlaced` に更新。**鍵の西隣 (5,6) は壁**（D4/D6 と違う幾何）＝入室テストの立ち位置を北隣 (4,7) に変更（D4 のコピペでは座標が合わずテストが一度赤くなった＝実測で直した）。
      - チェッカー＝`check-dungeon-integrity.mjs` 0 エラー（既存の D3 水 warning 1件のみ・無関係）／`check-dungeon-connectivity.mjs dungeon_8` PASS。Playwright フルスイート **515 passed**（回帰なし）。

- [x] **5.5g2. ユーザー報告のバグ修正＝D8 の宝箱の shield が盾として認識されず Bボタンにも反応しない（2026-08-08 完了・⚡ Sonnet・0番＝割り込み枠）**。**この位置の理由＝5.5g の直後にユーザーが D8 ボス部屋を実機で確認して報告。装備系宝箱データの誤形式（`type:'item', item:'shield'`）が原因＝報告どおりの実害（見た目は shield なのに盾扱いされず Bボタンでも無反応）。**
      - **原因＝`chestContents` に誤形式が3件混在**（dungeon_1[3,0](3,10) `armor.tier`→正 `armorTier`／dungeon_5[0,1](4,6)・dungeon_8[0,0](7,3) `{type:'item',item:'shield',...}`→正 `{type:'shield',shieldTier:...}`）。`grantReward` は装備系（weapon/armor/shield）を `type` 自体で分岐するが、`type:'item'` は `giveSubItem(content.item)` を呼ぶ通常サブアイテム専用の別経路＝`ITEM_META['shield']` が無いため無条件に `player.subItems.shield` を作って `activeSubItem` に登録してしまう（`useSubItem` は `id==='shield'` を処理しないので無反応）。**boomerang は `type:'item'` が正しい**（装備系3種と同列に扱えない＝混同禁止）。
      - 3件をデータ修正＋**新規 `tests/chest-content-shape.spec.js`（3本）で全ダンジョン横断の形式検査を新設**（装備系の `type:'item'` 誤形式／必須 tier フィールドの欠落／`grantReward` の未知 `type`）。壊れたコピーで3件とも検出されることを確認済み。実ブラウザでミラーシールド取得を確認（`shieldTier===2`・`activeSubItem` は汚染されない）。**フル518 passed（回帰なし）。**

- [x] **5.5g3. 剣・盾のティア別見た目差分（2026-08-08 完了・🧠 Opus・0番＝割り込み枠）**。**この位置の理由＝5.5g2 の直後にユーザーが指摘＝「剣と盾ってアイテム取得してレベルアップしたとき見た目も変わるようにしてたっけ？変わるようにした方がいい」。実装済みと思い込んで進めていた背景に「見た目が変わらない」を放置していたのは体験の欠落＝次の鍵部屋作業（5.5h以降）より先に直す。**
      - **現状調査の結果＝盾は「データはあるが繋がっていない」、剣は「そもそも装備の見た目が無い」で状態が違う。**
        - **盾＝`SHIELD_TIERS`（`shared/items.js`）に `sprite:'shieldWood'|'shieldIron'|'shieldMirror'` が既に定義済み**（`shared/sprites-items.js` にスプライトデータも実在）なのに、`game/render-chars.js addShieldOverlay()` が**常に固定 `'shield'`（木の盾のパレット）を描いている**＝ティアが変わってもプレイヤーが装備している盾の色が変わらない。`player.shieldTier` はステータス欄のテキスト表示にしか反映されていない。
        - **剣＝装備中の見た目がそもそも存在しない。** `heroD`/`heroR`（`shared/sprites-player.js`）は丸腰のプレイヤー本体スプライト（2フレーム歩行アニメのみ）＝剣は絵に焼き込まれていない。攻撃時の見た目は `game/combat.js showSwordSlashFloat()` が作る `.sword-thrust` という**CSSの光線（linear-gradient の棒が伸びるだけ）**＝ユーザー添付画像で指摘された「まっすぐのビームにしか見えない」はこれの実体。`SWORD_TIERS` の `sprite:'swordWood'|'swordBronze'|'swordSilver'|'swordHoly'` は**フィールドに落ちている剣アイテムのアイコン**用（`ITEM_SPRITES`）であって、プレイヤーが構えている絵ではない＝ティア別の攻撃演出も存在しない。
      - **参考にした初代ゼルダの実際の表現（ユーザー提示のスクリーンショット2枚・`.scratch/reference-images/sword1-zelda-attack-frame1.png`→`sword2-zelda-attack-frame2.png` の順に数フレーム表示）＝「プレイヤーが剣を前に突き出す構えのボディポーズ」＋「その先端に剣本体のスプライト（柄も含む）」を重ねる2レイヤー構成。ビームではなく**剣という物体が前方に出る**絵。
      - **方針（設計はここで確定・実装は次セッション）：**
        1. **盾＝低コストな修正から着手。** `addShieldOverlay()` の `spriteName='shield'`／`makeSprite(spriteName,'shield',...)` の第2引数（パレット名）を `SHIELD_TIERS[player.shieldTier].pal`（`shieldWood`/`shieldIron`/`shieldMirror`）に差し替える。`shieldTier===-1`（盾未所持）は現状どおり `!player.shield` で早期 return。既存の `shieldSide` 系スプライトの見た目（上/横向き）はそのまま使い、パレットだけティアで変える最小差分。
        2. **剣＝2レイヤー構成を新設。** (a) プレイヤー本体に「剣を構えたボディポーズ」フレームを追加する（`heroD`/`heroR` に3フレーム目＝攻撃ポーズを増やすか、既存の歩行2フレームとは別に `heroD-attack`/`heroR-attack` を新設）。(b) その構えの手元〜前方に `SWORD_TIERS[tier].sprite`（`swordWood` 等）を**剣オーバーレイ**として重ねる（`addShieldOverlay` と同型の `addSwordOverlay()`＝向き別に位置・回転を出し分け、柄がプレイヤーの手の位置に来るよう座標を調整）。(c) `showSwordSlashFloat()` の攻撃中だけこのポーズ＋オーバーレイを一定時間（既存の `SWORD_COOLDOWN_MS` 相当）表示し、終わったら通常の歩行スプライトに戻す。**現行の `.sword-thrust` ビーム CSS は削除**（新しい2レイヤー表現に置き換える）。
        4. **ティアで剣スプライト自体が変わる**＝`SWORD_TIERS[tier].sprite` を上記オーバーレイにそのまま使う（既存データを再利用＝新規アセット不要）。
      - **ユーザー確認事項（次セッション着手前に要確認）：** 攻撃ポーズは4方向（上下左右）分の新規ピクセルアート（またはプレイヤー本体を回転/反転で流用できるか）が必要＝**ドット絵の作成/調整はユーザー主導 or AI生成のどちらでやるか**を着手時に確認する。
      - **テスト方針：** ①盾＝`shieldTier` を0/1/2で切り替えて `addShieldOverlay` が呼ぶ `makeSprite` の第2引数（パレット名）を assert（DOM要素のスプライトソースまでは実機目視）。②剣＝攻撃中に剣オーバーレイのDOM要素が出現し、`SWORD_TIERS[tier].sprite` に対応するクラス/パレットを持つこと、かつクールダウン後に消えることをPlaywrightで確認。③既存 `tests/sword-tiers.spec.js`／`tests/armor-shield-tiers.spec.js` はステータス値（ATK/DEF/名前）のみを見ているので**見た目のテストはこのタスクで新設**（既存テストの流用不可）。
      - **✅ 実装結果（2026-08-08）：**
        - **盾＝`addShieldOverlay()` のパレットを `SHIELD_TIERS[player.shieldTier].pal` に差し替え**（形状は共通＝この方針は `ITEM_SPRITES.shieldWood/Iron/Mirror` が全部同じ形の既存前例に合わせた）。テストから配色を観測できるよう `dataset.pal` / `dataset.shieldTier` を canvas に付ける。
        - **剣＝2レイヤー構成を新設。** ①本体＝`heroDAtk`/`heroRAtk`(`heroLAtk`は flipX)/`heroUAtk`（`shared/sprites-player.js`・`pose()` で歩行 frame 0 に腕/手だけパッチ）。②剣本体＝`ITEM_SPRITES.swordHeldUp/Down/Right`（`shared/sprites-items.js`・**8×28** の直刃を新規作成し `rot180`/`rot90cw` のピクセル完全回転で4方向を生成・左は flipX）。`game/combat.js` の `showSwordSlashFloat()` を全面書き換え＝向きごとの `SWORD_HELD_GEO`（セル単位 ox/oy/w/h）で配置し、パレットは `SWORD_TIERS[swordTier].pal`、tier≥2 だけ `drop-shadow` で淡く光らせる（銀=青／聖=金）。
        - **ポーズの解除は論理時間**＝`player._atkUntil = gameNow() + ATTACK_POSE_MS`（`ATTACK_POSE_MS = 180` ＞ `SWORD_COOLDOWN_MS = 100` ∴連打でポーズが途切れない）を `game/game.js gameTick()` の `tickAttackPose()` が戻す。`getHeroSpriteName()` が攻撃中だけ `*Atk` を返す。`setTimeout` にしないのは `__game.step()` の手動 tick と実ループで挙動を揃えるため。
        - **攻撃中の盾＝消さずに「右手側へ回す」**（`getGameNow` を `createRenderChars` の deps に追加）。当初は「攻撃中は盾を描かない」で実装したが**ユーザー却下**＝盾は消えるものではない。**プレイヤーは左利き（初代ゼルダと同じ）＝剣は左手・盾は右手**という1つの規則から向きごとの見え方を導く（`game/render-chars.js SHIELD_ATK_GEO`）：待機＝盾は向いている方向へ／攻撃＝盾がワールドで 90° 回ってプレイヤーの右手側へ来る ∴ 下向き＝側面が我々の左／上向き＝側面が我々の右／右向き＝**正面**がカメラ側（プレイヤーより前面 z 5）／左向き＝裏面（`shieldBack`・プレイヤーが上に重なる z -1）。横向きは**右手の位置＝盾の中心**に合わせる（頭上や足元に置くと手から離れて浮く）。
        - **盾の「防御が効かない窓」を見た目の窓と一本化**（`game/projectile.js isShieldActive()`）＝`player._atkUntil` を見る。以前は `SWORD_COOLDOWN_MS = 100` だけで判定していたので **100〜180ms の間だけ「盾が右手側へ回った絵なのに正面ブロックできる」**食い違いがあった（`TICK_MS = 120` ∴攻撃直後の1 tick がちょうどその隙間）。旧セーブ互換のため `getLastSwordTime` 側の判定も残す。
        - **⚠️ 方針からの逸脱＝`.sword-thrust` の CSS は削除しなかった。** 敵の攻撃演出（`game/enemy-ai.js:647`）が今も同じクラスを使っており、`tests/sea-enemies.spec.js` がその要素数を数えている＝削除するとプレイヤー無関係の演出が消える。プレイヤー側は新要素 `.sword-held` に完全移行済み。
        - **🔑 落とし穴2件（実測で判明）：** ①`canvas.sprite` は `board.css` で `transform:translate(-50%,-50%)` が掛かり、打ち消しは `.char-abs` 配下だけ＝`.char-abs` 以外に置く新スプライトは自分で `transform:none` に戻さないと半セルずれる（剣が頭の上に描かれていた）。②スプライトの**縦横比が見た目の太さを決める**＝最初 8×22 で描いたら柄が胴体を塗り潰すほど太く、幅を 0.24 セルに固定したまま 8×28 に描き直して解決。
        - **ピクセルアートの検証手段＝`.scratch/shot-attack.mjs`（git-ignore・使い捨て）**。`deviceScaleFactor:4` で起動→ティア/向きをセーブに仕込む→`pause(); step(3)`（**`gameTime=0` では `lastSwordTime=0` とのクールダウン判定で初撃が弾かれる**＝ここに気づかず「絵が変わっていない」とユーザーに指摘された）→`setTimeout` を no-op に差し替えてポーズを凍結→拡大クロップを保存。**実際の描画画像を見るまで「できた」と言わない**が今回の教訓。
        - **上下向きのポーズは「真上から見た体の前後関係」で描く**（ユーザー却下5回のあと合格）。平面に腕を並べるのではなく、**奥（カメラから遠い側）＝画面で上・かつ体に隠れる／手前＝画面で下・体より前**という3D の重なりで決める。上向き（背面）＝左腕を前＝奥へ突き出す ∴ 垂れていた腕のシルエットをまず削り、肩から生えた腕は**肌色を1ドット（col10）だけ**残して残りは頭の裏へ消す（右側に輪郭線を描くと「腕が細くなった」ように見えて破綻する）。剣は柄と手が頭の裏に隠れ**刃だけが頭上に出る**＝`SWORD_HELD_GEO.up` を頭の中心に置き **z-index -1**（char-layer 内で盤面セルより上・`.char-abs` より下＝はしごオーバーレイと同じ手）で**プレイヤーが剣の上に描かれる**。足は左足を前＝奥（腿はズボンの陰・ブーツだけ覗く）＋右足を後ろ＝外へ広げてスタンスを取る。下向き（正面）は**下半身を上向きの左右鏡面（col c → 31-c）**にする＝同じスタンスを反対から見た絵（横へ太くはみ出させると脚が体から生えたように見えて気味が悪い＝実測で却下）。
        - テスト＝**`tests/equip-visual-tiers.spec.js` 新規7本**（盾パレット3ティア／剣スプライトの4ティア／4方向のポーズ差し替えと `dir-*` クラス／論理時間でのポーズ解除／**攻撃中も盾を描き向きごとに見える面が変わる**（`dataset.shieldState` / `dataset.shieldSpr`）／剣未所持なら剣スプライト無し／**攻撃ポーズの窓の間は正面ブロックが効かない**（投擲物を注入して HP 減少を確認・ポーズ解除後は再びブロック））。`renderChars` は `__game` に露出していないので盾ティアの検証はティアごとに再シード。フルスイート **525 passed**（518＋7・VRT 含めて回帰なし）。

- [x] **5.5g4. ユーザー報告のバグ修正＝剣を出している間は移動できないようにする（2026-08-08 完了・🧠 Opus・0番＝割り込み枠）**。**この位置の理由＝5.5g3 で入れた「剣を構えたポーズ＋剣スプライト」の直後にユーザーが実機で報告＝「今は移動しながら剣を出せてしまい、移動してるにもかかわらず剣は移動しないので剣だけが宙に浮いてしまってる」。5.5g3 が作った実害 ∴ 次の鍵部屋作業（5.5h）より先に直す。**
      - **原因（要確認だが見えている）＝`.sword-held` は `combat.js showSwordSlashFloat()` が生成時の `player.x/y` で `left/top` を固定した使い捨て要素で、プレイヤーに追従しない**（`.char-abs` の子ではなく char-layer 直下）。∴攻撃ポーズ中に移動すると剣だけがその場に残る。
      - **方針＝追従させるのではなく「攻撃中は移動できない」にする**（初代ゼルダと同じ＝突きの間は足が止まる）。**窓は `player._atkUntil`**（`ATTACK_POSE_MS = 180`）＝ポーズの絵・盾の防御無効と**同じ1つの窓**で判定する（[[blade-async-finale-needs-gate]] と同じ作法＝窓を増やさない）。
      - **入れる場所＝`game/player.js movePlayer(dir)` の先頭ガード**（`getIsDialog() || getIsPaused() || ...` と同列に1行足す）＝ここが唯一の移動の入口なので、キー押しっぱなし（`input.js processHeldKeys`）・`__game.movePlayer()` の両方が同時に止まる。**`setHeroDir(dir)` より前で return する**＝向き変更も止める（向きだけ変わると剣スプライトが古い向きのまま残り、同じ「宙に浮く」破綻が出る）。
      - **要確認（実装時に実機で見る）：** ①窓 180ms が体感で引っかかるか（`SWORD_COOLDOWN_MS = 100` より長い ∴連打すると足が止まり続ける＝初代ゼルダもそうだが、押しっぱなしで前進できないのが不快なら窓をクールダウン側に寄せる案を検討）。②石押し・水/穴のはしご渡り中に攻撃した場合に詰まないか（移動が止まるだけで状態は変わらない見込み）。
      - **テスト＝`tests/equip-visual-tiers.spec.js` に追加**（`swordAttack()` → `movePlayer('right')` で `player.x` が変わらない・`step(2)` でポーズが切れた後は動ける・`heroDir` も攻撃中は変わらない）。実機で「攻撃中に方向キーを押しても剣が置き去りにならない」ことをスクショで確認。
      - **✅ 実装結果（2026-08-08）＝方針どおり `game/player.js movePlayer()` の先頭ガード1つだけ。** `getIsDialog() || getIsPaused() || ...` の直後・`setHeroDir(dir)` より**前**に `if (p?._atkUntil != null && gameNow() < p._atkUntil) return;` を置いた＝押しっぱなし（`input.js processHeldKeys`）・`__game.movePlayer()`・スワイプの3経路が同時に止まり、向き変更も止まる（＝剣スプライトが古い向きで残る破綻も同時に防ぐ）。**窓は `_atkUntil` の1つだけ**（ポーズの絵・盾の防御無効と共有）。
      - **要確認①（窓 180ms の体感）＝連打で足が止まり続けることは無い。** 攻撃は `input.js:101` の `if (!e.repeat)` でキーリピートを弾く∴押しっぱなしでは攻撃が再発火しない＝足止めは1押しにつき1回 180ms だけ。ただし**体感の最終判定はユーザーの実プレイ**（気になれば窓をクールダウン側 100ms に寄せる案を残す）。
      - **要確認②（詰み）＝無し。** 石押しは押し自体が完了してから止まるだけ（クールダウン 600ms とは独立）／はしごの水・穴渡りは「移動が遅れる」だけで状態は変わらない／溺れ・時間制限の仕組みは存在しない（`grep drown` 0件）。ソルバーの `L` は手数ベース＝実時間の足止めでは変わらない∴既存パズルの測定値は不変。
      - **テスト＝`tests/equip-visual-tiers.spec.js` に ⑧ を追加（8本）。** `field 7,14` の (4,5) は右隣 (5,5) が木 `t` ∴**baseline は左**で取る（右で書くと壁のせいで偽陽性）。左へ1歩＝`heroDir='left'` ∴攻撃ポーズは `heroRAtk`（左は flipX で右ポーズを共有）。検証＝攻撃中は `movePlayer('left')` でも `queueInput('left')+step(1)` でも座標が変わらない／`movePlayer('up')` でもスプライトが `heroRAtk` のまま（向きロック）／`step(1)` 追加でポーズが切れると `heroR` に戻り再び動ける。**ガードを `if (false && ...)` に潰すと ⑧ が赤くなることを確認（歯があるテスト）。**
      - **🔴 テストヘルパーへの波及（実装前には見えていなかった）＝`tests/field-corridor-o.spec.js` の `walkAndStrike()` が ②/⑤ で赤化した。** 攻撃直後に `step(1)`（=120ms）しか進めずに歩き出していたので、180ms の足止めに**最初の半歩が黙って捨てられ**歩数が1つ足りずタイルを跨げなかった（「両方開けたのに南へ抜けられない」）。ヘルパーを `step(2)` にして解消＝**ゲームの回帰ではなくハーネスの前提ズレ**（このテストの意図は「攻撃してから歩く」であって「攻撃と同時に歩く」ではない）。
      - **実機確認＝`.scratch/shot-attack-move.mjs`（使い捨て・git-ignore）** で `deviceScaleFactor:3` の4枚（待機／攻撃直後／攻撃中に方向キー押しっぱなし／ポーズ切れ後）を撮り、**攻撃中に方向キーを押しても剣が手の中に残っている**ことと座標が動かないことを確認。**フル 526 passed（525＋1・回帰なし）。**

- [x] **5.5g5. ユーザー報告のバグ修正＝ため攻撃（剣ビーム）を溜めている間も移動できるのを止める（2026-08-08 完了・🧠 Opus・0番＝割り込み枠）**。**この位置の理由＝5.5g4 の完了報告の直前にユーザーが実機で報告＝「ため攻撃機能でためている最中も移動できるのはへん。溜めてる間は移動できなくするべき」。5.5g4 で「剣を振っている間は足が止まる」を入れた直後∴同じ規則がチャージにも要る（片方だけだと食い違いが残る）。**
      - **入れたもの＝`game/player.js movePlayer()` に2つ目のガード。** ただし**`setHeroDir(dir)` の後**に置き、`updatePlayerCharEl(); return;`（壁でブロックされたときと同じ形）＝**向き変更は通す**。理由＝離した瞬間に飛ぶビームの狙いを付けられないと溜めそのものが使えない。攻撃ポーズ（5.5g4）と違い**チャージのオーラは `char-player` の子要素**（`charge.js updateAura`）＝プレイヤーに追従する∴向きだけ変えても置き去りになる描画が無い。窓は `charge.js` の `_chargeStart`（`isCharging()`）＝オーラ表示と同じ1つの窓。
      - **旧「チャージ中は半速」（`charge.js getMoveSpeedFactor` → `input.js` の `speed` 係数）は撤去した。** 足止めが入れば係数は死んだコードで、しかも「溜め中の移動」の規則が2箇所に散る（[[blade-execution-queue-single-source]] と同じ作法＝規則は1箇所）。`game.js` の `getChargeMoveSpeedFactor` 配線・`initInput` の dep も削除。テストで速度係数を見ているものは無かった（`grep` 済み）。
      - **`createPlayer` の deps に `getIsCharging` を追加**（`game.js` は `let getIsCharging` を factory 生成後に上書きする形なので `() => getIsCharging()` の間接呼びで初期化順に依らない）。**低ティアの剣は影響ゼロ**＝`startCharge()` は `SWORD_TIERS[tier].beam` が無ければそもそもチャージを始めない（ビーム解禁前は今までどおり歩ける）。
      - **テスト＝`tests/charge-beam.spec.js` に条件4（4本目）。** 前提として「チャージしていなければ右へ動ける」を取ってから、`startCharge()` 中は `movePlayer('right')` でも `queueInput('right')+step(1)` でも座標が変わらないこと／`movePlayer('down')` でスプライトが `heroR`→`heroD` に変わる（＝向きは変えられる）が座標は不変／`releaseCharge()` 後は再び動けることを検証。**ガードを `if (false && ...)` に潰すと条件4が赤くなることを確認（歯があるテスト）。**
      - **実機確認＝`.scratch/shot-charge-move.mjs`（使い捨て・git-ignore）** で4枚（待機／チャージ中／チャージ中に方向キー押しっぱなし／離した後）を撮影＝③でオーラが出たまま座標 (2,5) から動かず、④で (3,5) へ動くことを確認。**⚠️ 撮影中に気づいた点＝足止め時の `updatePlayerCharEl()` はプレイヤー要素を作り直す＝オーラが一度消える。** 実プレイでは `gameTick` が `processHeldKeys()` → `tickCharge()` の順に呼ぶ∴同じ tick 内で描き直されて見た目には消えない（スクリプトで `movePlayer` の後に `step` を置かないとオーラが消えた画像が撮れる＝実害ではなく撮影手順の問題）。

- [x] **5.5g6. ユーザー報告の見た目修正＝ため攻撃を溜めている間は剣を構えたままにする（2026-08-08 完了・🧠 Opus・0番＝割り込み枠）**。**この位置の理由＝5.5g5 の完了報告の直後にユーザーが実機で報告＝「ためのとき、剣はだしっぱなしにするべき」。溜め（`CHARGE_FULL_MS`=720ms）は攻撃ポーズ（`ATTACK_POSE_MS`=180ms）より長い∴溜めている途中で剣とポーズが消えて「足は止まっているのに手ぶらで光っている」絵になっていた（5.5g5 で足を止めた結果、余計に目立つ）。**
      - **入れたもの＝`game.js tickAttackPose()` が `isCharging()` のとき窓（`player._atkUntil`）を毎tick延長する。** 窓は今までどおり **1 つだけ**＝これで「ポーズの絵・構えた剣・盾の防御無効・足止め」が全部同期する（窓を増やすと必ず食い違う＝5.5g4/5.5g5 と同じ作法）。剣を振らずに溜めだけ始めた場合（`startCharge()` 単独）にも絵を合わせるため、窓が `null` → 設定に変わる tick だけ `updatePlayerCharEl()` を呼ぶ。
      - **剣スプライトの寿命を `setTimeout`（実時間）から窓（論理時間）に付け替えた。** `combat.js` の `showSwordSlashFloat()` を `drawSwordHeld()`（既存の `.sword-held` を捨ててから今の向きで描く）へ改名し、`setTimeout(() => el.remove(), ATTACK_POSE_MS)` を削除。代わりに `clearSwordHeld()`（窓が切れた tick に `tickAttackPose` から）と `ensureSwordHeld()`（溜め中に無ければ描く＝毎tick作り直すとちらつくので存在チェック付き）を追加して export。**副作用として良くなった点＝ポーズ・ダイアログで世界が止まっている間は剣も消えない**（実時間タイマーだと止まっている最中に消えていた）。
      - **溜め中の向き変更で剣も描き直す。** `.sword-held` は生成時の `player.x/y` で `left/top` を固定する要素∴向きだけ変えても古い向きの剣が残る。`player.js movePlayer()` のチャージ分岐に `drawSwordHeld()` を足し、**この分岐を攻撃ポーズ判定より前に移した**（溜め中は窓を延長し続けている＝順番が逆だと 5.5g4 のガードに向き変更まで塞がれる）。`createPlayer` の deps に `drawSwordHeld` を追加。
      - **確認した副作用＝溜めている間は盾で防御できない**（`projectile.js isShieldActive()` が同じ窓を見る）。剣を突き出して盾が利き手側へ回っている絵と一致する∴仕様として受け入れる（＝溜めはリスクを伴う）。低ティアの剣は影響ゼロ（`startCharge()` が `beam` 無しでは始まらない）。
      - **テスト＝`tests/charge-beam.spec.js` に条件5（5本目）＋条件4の期待値更新。** 条件5＝実プレイと同じ順（`swordAttack()` → `startCharge()`）で剣が出た後、満タン（6フレーム）を越える `step(8)` でも `.sword-held` が 1 個・スプライトが `heroRAtk` のままであること／`releaseCharge()`+`step(2)` で 0 個・`heroR` に戻ることを検証（二重描画も同時に検出）。条件4は溜め中のスプライトが `heroRAtk`→（下を向くと）`heroDAtk`、`.sword-held` の `dir-*` も `right`→`down` に追随することへ更新。**延長を `if (false && isCharging())` に潰すと条件4・条件5が赤くなることを確認（歯があるテスト）。**
      - **実機確認＝`.scratch/shot-charge-sword.mjs`（使い捨て・git-ignore）** で5枚（待機／溜め開始／満タン越え／溜め中に下を向く／離した後）。③で `swords:1 / heroRAtk` とオーラが同時に出ていること、④で `dir-down` に描き直されること、⑤で `swords:0 / heroD` に戻ることを画像とログの両方で確認。

- [x] **5.5h. D7・dark_tower `1,2` ＝戦闘型の強化（2室）（2026-08-08 完了・⚡ Sonnet）**。両室とも A 型のまま中身を強化した＝`ENEMY_META` から算出した脅威度（`hp*atk/(def+1)` の合計）が進行順で単調増加することをテストで固定（[[blade-enemy-tables-derive-from-meta]] のとおり手書きでなく `ENEMY_META` から導出）。
      - **脅威度＝D1（据え置き確定・魔物+パトロール2）24.0 → D7（魔物+センチネル3）36.0 → dark_tower[1,2]（魔物+チェイサー3）48.0** に単調増加。据え置きの`W`（中ボス・鍵の番人）は残し、周囲に敵を分散配置（部屋10×12の内部84セルに対し敵3〜4体＝逃げ場を残し各個撃破できる間合いを確保）。
      - **D7 は笛（`flutePlayed`）を2段目に足した**＝新規トリガー `killAllAndFlute`（`game/conditions.js`）＝全滅かつ笛を吹いたの AND（順序は問わない）。**killAllAndFlute は `ss.flutePlayed` を見るが、これは `playFlute()` の `type:'reveal'` 分岐でしか true にならない**（`game/game.js`）＝D7 `1,0` に `fluteEffect:{type:'reveal', message:...}` を新規で付けないと永久に成立しない（実装時に気付いて追加）。
      - **`check-dungeon-integrity.mjs` の検査(9) に `killAllAndFlute` のケースを追加**（敵の実在＋`flute` が UNLOCKED_AT に入っているかを確認）＝未知のトリガーとして error になるのを防ぐ。
      - **`getStageStateSnapshot()`（テスト用）に `flutePlayed` を追加**＝テストで笛を吹いたかを実機観測できるようにした（既存のスナップショットに漏れていた）。
      - 移行スクリプト `scripts/migrate-key-room-d7-tower.mjs`（新規・自己検査型）＝境界セル（外周・継ぎ目）を変更していないこと・鍵セルが据え置きのままであること・脅威度の単調増加・敵密度の逃げ場を書き込み前に assert。
      - **テスト＝`tests/dungeon-key-gate.spec.js` に⑨として5本追加**（脅威度の単調増加／D7:全滅だけでは不成立／D7:笛だけでは不成立（敵が残っている）／D7:全滅+笛の両方で鍵出現・拾える／dark_tower[1,2]:全滅で鍵出現・拾える）。EXPECTED表を`killAllAndFlute`（D7）に更新。**AND条件をORに一時変更して2本が赤化することを確認済み（歯があるテスト）。**
      - **🔑 ハマり＝`startAt`ヘルパーに `subItems.flute = {count: Infinity}` を渡すとJSON.stringifyが`Infinity`を`null`に変換し実質カウント0になる**＝`count: 99` に修正。**`walk()`は1コール=0.5セル移動**（既存テストの規約どおり）＝鍵までの歩数を書くとき2倍で計算する必要がある（最初 half の歩数で書いて鍵に届かず赤化→修正）。
      - **整合性チェッカー0エラー（既存D3水警告1件のみ・無関係）・接続チェッカー dungeon_7/dark_tower ともPASS・フル534 passed（回帰なし）。**
      - **🔴 完了報告直後にユーザー実機報告で実害2点が発覚（同日追加修正）＝(1)「全部倒したのに鍵が出ない」＝笛が要ることを示す手がかりが部屋に一切無かった（他のD7部屋`1,1`/`1,2`はヒント看板`i`があるのに、この部屋だけ看板ゼロで作った）。(2)「ここに何かありそうと分かる工夫が要る」＝鍵の周りに視覚的な目印が無かった。** ユーザー自身が鍵(5,7)周囲8セルを石畳`o`（bgTiles）で囲って「特別な場所」を視覚的に示す修正を先に実施。AI側は空床`(8,1)`に看板`i`＋`signData`（「守護者を すべて討てば／音色に応えて 何かが動く。」）を追加して手がかりを補った。**教訓＝新規トリガーを作るとき「プレイヤーがそれに気づく手段」（看板・視覚的目印）を同時に用意しないと、機械検査は緑でも実プレイは詰んで見える**＝[[field-axis-met-not-noticed]] と同型（指標を満たすことと気づかせることは別軸）。整合性チェッカー0エラー・接続チェッカーPASS・関連52テストpassedを再確認。

- [x] **5.5i. dark_tower `4,3` ＝全10鍵の最終＝体験の頂点（2026-08-09 完了・🧠 Opus）。**
      **✅ 完了＝ユーザーが editor で作り込んだ `test_mechanics[0,1]`（床47・石4・ボタン4隅）を実プレイし「23,0 と同程度の難しさ」＝合格と判定。この盤面を本番 `dark_tower[4,3]` と `test_mechanics[24,0]` ミラーへ同時投入した。** 濃さ軸のフル BFS は状態空間が V8 Map 上限（1670万＝2^24）を超えて完全展開不能＝測れないが、**合否は実プレイ判定（4.6/4.7 と同じ運用＝ユーザーが解いて判定するのが合格の定義）**。境界（入口 0,5/0,6・扉 9,5/9,6・鍵 7,6・ボタン4隅）は本番 4,3 と完全一致を機械 assert。`migrate-key-room-dark-tower-43.mjs` を「生成器テンプレを組む」から「SOURCE ステージをそのまま写す」方式へ改稿（既定 SOURCE=24,0 で自己参照＝冪等）。検証済み候補 `0,1` は本採用後に削除（24,0 と重複＝役目終了・`write-candidate-0-1.mjs` も削除）。`check-dungeon-integrity.mjs` 0エラー（既存 D3 水警告1のみ）・`check-dungeon-connectivity.mjs dark_tower` PASS・実機 pageerror ゼロ（石4/ボタン4/鍵扉2枚を目視）・フル 533 passed。**教訓＝濃さ軸が「測れない」＝広すぎて完全展開できないことの証明にはなるが、ユーザーが手で作り込み実プレイで合格と判定した盤面は、機械測定の可否に関わらず採用する（測定は補助・実プレイ判定が合否）。**
      **──── 以下は差し替え前の記述（履歴として残す）────**
      **🔴 いったん純倉庫番（石4・L=42）で作ったが、ユーザー実感「簡単。23,0 の方が難しくない?」が正しかった＝作り直す。** 押し手数を同尺度（pull-BFS）で測り直した：**24,0(=4,3)＝L=36（状態800万＝広くスカスカ）／23,0 激難試作＝L=21（状態66万＝狭く濃い）**。押し手数だけなら 4,3 が長いのに体感は 23,0 が上＝**L は難易度の必要条件であって十分条件でない**。23,0 の難しさは L でなく「濃さ」の軸（貪欲NG・強制手率0.13・最短解本数2・deadlock185万）で作られていた。**私の基準再設計の誤り＝「測れないから」と濃さの軸（deadlock/強制手率/最短解本数）を全部捨て、広い盤面(床66)で石を遠くへ運ぶだけの薄いパズルを『頂点』に据えた。** ユーザーが最初に危惧し私が「盤面固定なら L 大＝難しい」と否定した、まさにその反例。
      **🔴 いったん純倉庫番（石4・L=42）で作ったが、ユーザー実感「簡単。23,0 の方が難しくない?」が正しかった＝作り直す。** 押し手数を同尺度（pull-BFS）で測り直した：**24,0(=4,3)＝L=36（状態800万＝広くスカスカ）／23,0 激難試作＝L=21（状態66万＝狭く濃い）**。押し手数だけなら 4,3 が長いのに体感は 23,0 が上＝**L は難易度の必要条件であって十分条件でない**。23,0 の難しさは L でなく「濃さ」の軸（貪欲NG・強制手率0.13・最短解本数2・deadlock185万）で作られていた。**私の基準再設計の誤り＝「測れないから」と濃さの軸（deadlock/強制手率/最短解本数）を全部捨て、広い盤面(床66)で石を遠くへ運ぶだけの薄いパズルを『頂点』に据えた。** ユーザーが最初に危惧し私が「盤面固定なら L 大＝難しい」と否定した、まさにその反例。
      - **✅ 作り直しの方向＝(1) に確定（2026-08-09 ユーザー）＝「23,0 みたいな難易度のものをもう一つ作る」。** 狭く濃い純倉庫番（23,0 系の詰まった幾何＝床を絞り石4の干渉を上げる・L は落ちても濃さを取る）。合成移植(26,0)は採らない。**次セッションで着手。**
      - **着手時の指針＝23,0 を「基準機」にして超える。** まず 23,0 の濃さ指標を全部測り直して並べる（貪欲NG・強制手率・最短解本数・deadlock・状態数・押し手数 L＝`.scratch/push-count.mjs` は押し手数だけ測れる∴濃さ軸は `measure-puzzle.mjs`/`puzzle-metrics.mjs` で測る）。それを**下限**に、新盤面がどの軸でも 23,0 以上（特に「最短解本数が少ない＝一本道」「強制手率が高い」）になるよう幾何を詰める。**盤面は 23,0 と同程度に狭く**（床を絞る）＝広さで L を稼ぐ愚を繰り返さない。生成器 `generate-key-room-dark-tower-43.mjs` は L だけの2軸に削ってあった∴濃さ軸の測定を戻す（狭い盤面なら deadlock/noEscape も全列挙が回る＝23,0 は状態66万で完全展開できた実績あり）。
      - **基準の修正＝L 単独では不可。** 濃さの軸を測れる範囲で復活させる（少なくとも貪欲NG＋最短解本数＋強制手率。deadlock/noEscape は狭い盤面なら全列挙が回る＝23,0 は状態66万で完全展開できた＝「難しい倉庫番は測れない」は広すぎる盤面での話で、狭く濃い盤面なら測れる）。**広さで L を稼ぐのをやめ、狭さで濃さを稼ぐ。**
      - **今ある成果物（作り直しても再利用可）：** 生成器 `generate-key-room-dark-tower-43.mjs`（pull-BFS＋貪欲NG）／移行 `migrate-key-room-dark-tower-43.mjs`（本番＋test_mechanics 24,0 ミラーを同時に書く）／同尺度の押し手数測定 `.scratch/push-count.mjs`。**現状の 24,0 ミラー・本番 4,3 の盤面は「薄い」＝差し替える前提。**
      - **▶ 進捗（2026-08-09・今セッション）＝濃さ軸を復活させて基準機を数値で確定・候補を検証場に投入。**
        - **基準機 23,0 の濃さを実ゲーム遷移フル BFS で再測定**（`.scratch/density.mjs`）＝**状態 234万（完全展開）・L押 21・L歩 80・貪欲NG・deadlock 185万（比 0.79）・最短解本数 6・強制手率 0.13**。24,0(=現4,3) は同じフル BFS が **OOM**（状態が桁で大）＝「広くスカスカ」の動かぬ証拠。∴基準＝**新盤面はこの全軸を下限に超え、かつ状態が DENSITY_CAP 内で完全展開できる（＝狭い）**。
        - **生成器を「L だけの2軸」から「濃さ版」へ全面改稿**（`scripts/generate-key-room-dark-tower-43.mjs`）＝pull-BFS で L押候補を作り、L上位を `measureMetrics`（実ゲーム遷移フル BFS）に掛け deadlock比/強制手率/最短解本数を測る。状態>DENSITY_CAP は「広すぎ」で自動棄却＝**狭さの強制フィルタ**。テンプレは**座標指定＋buildTemplate で機械生成**（文字列直書きの幅ミスを構造的に排除）。
        - **実測で判明＝「広さで L を稼ぐ」は全滅**。開けた広間テンプレ（床64-71）は L押21+ 出るが濃さ測定で**全件 tooWide（状態>4M）**＝前作の轍。規則的千鳥（床61）は L押8止まり。**⚠️ まだ 23,0 を全軸で超える狭く濃い盤面は未発見**（e1 床69 も広すぎ側）＝テンプレの幾何を 23,0 並み（床48-54・不規則ピラー）に詰め直す作業が残る。
        - **⚠️ 方針転換（ユーザー・2026-08-09）＝「検証段階の候補を本番 4,3 に入れるな」「今の候補盤面を test_mechanics の空きに置いてプレイさせろ」。** ∴生成器の探索を詰めきる前に、**pick で解ける配置1つ（e1・L押=34・石 2,5 3,10 4,8 6,4）を `test_mechanics[0,1]` に投入**（`scripts/write-candidate-0-1.mjs`）＝ユーザーが実プレイで難易度を判断する。**⚠️ 当初 1,0 に入れたが 1,0 は torch テスト部屋（使用中）＝誤り。git checkout で 1,0 を復元し、空きの 0,1 に入れ直した。0,1 は本番でなく検証場＝濃さが 23,0 を超えると確認できるまで dark_tower[4,3]/24,0 は差し替えない。** 濃さの追い込み（23,0 全軸超え）は**ユーザーのプレイ判定を待ってから**（自動探索でトークンを浪費しない）。
      - **✅ ユーザーが 0,1 を実プレイ→手で難化改修した（2026-08-09）。** ユーザー自身がエディタで 0,1 の石・壁配置を編集し「少し難しくなった」。次セッションはこの**ユーザー修正版 0,1** を評価対象にする（下の次セッションタスク参照）。
      - **▶ 次セッションのタスク＝ユーザー修正版 0,1 を評価し本番 4,3 に採用可か判定する（🧠 Opus）。手順：**
        1. **現盤面を読む**＝`work/blade-of-lumia.json` の `test_mechanics[0,1]`（ユーザーが手で改修済み＝AI 未把握の内容。まず現物を読む）。参考：改修直後スナップショット＝床58・石4・ボタン4・鍵7,6・showConditions{'7,6':stonesPlaced}・fluteEffect resetStones（次セッションでは必ず再読して最新を確認）。
        2. **濃さ軸を測る**＝`node --max-old-space-size=6144 .scratch/density.mjs test_mechanics 0,1`（引数で任意ステージ可に改修済み）で 状態数・L押・貪欲NG・deadlock・最短解本数・強制手率 を出す。**⚠️ 既知の落とし穴：改修直後版はフル BFS が `RangeError: Map maximum size exceeded`（状態が 23,0 の234万＝完全展開 より大＝まだ広い）で濃さ軸が出なかった＝「床58 は 23,0 より広い」証拠。** 完全展開できない＝その時点で「23,0 より狭く濃い」条件を満たさない＝**まず床を 23,0 並み（床48-54）まで絞る改修をユーザーに戻すか、こちらで詰める判断**が要る。L押だけなら軽い pull-BFS（`.scratch/pull-01.mjs test_mechanics 0,1`）で測れる。**⚠️ ただし pull-01 は「入口＝上辺開口・石不可＝入口/鍵/(9,5)(9,6)」を仮定した簡易版＝ユーザー改修で入口や扉位置が変わっていたら L=-1（goal 到達不能判定）になる＝スクリプト側の仮定を現盤面に合わせて直してから測る。**
        3. **基準機 23,0 と全軸比較**＝下限＝{状態が DENSITY_CAP 内で完全展開できる（＝狭い）／L押≥21／貪欲NG／deadlock比≥0.79／最短解本数≤6／強制手率≥0.13}。**全軸で 23,0 以上なら採用可。1つでも下回る（特に完全展開できない＝広すぎ）なら不採用**＝ユーザーに数値を示し「もっと狭く／別配置」を相談 or こちらで幾何を詰める。
        4. **採用可なら本番へ**＝`scripts/migrate-key-room-dark-tower-43.mjs` を 0,1 盤面で走らせ dark_tower[4,3] と test_mechanics[24,0] ミラーを同時に書く（片方だけ直すと乖離）。`check-dungeon-integrity.mjs` 0エラー・`check-dungeon-connectivity.mjs dark_tower` PASS・実機 pageerror ゼロを確認。テストは `tests/dungeon-key-gate.spec.js`（既に stonesPlaced 更新済み）。
        5. **判定結果を PROGRESS.md に記録**＝採用/不採用と各軸の実測値。
      **── 以下は差し替え前の旧・完了記述（薄いと判明した純倉庫番版・履歴として残す）──**
      **✅（旧）純倉庫番（石4・L=42・貪欲NG）（2026-08-09）。**
      **✅ 最終確定＝色ゲートを使わない純倉庫番（石4）。** 当初は合成型（石4＋色ゲート＝`test_mechanics 26,0` 移植）を狙ったが、2026-08-09 にユーザーと2段階で方針転換：(1) 移植より**一から純倉庫番で深い L を狙う**、(2) 旧測定手法（下記4軸のフル BFS）が石4×広い盤面で**OOM＝測れない**と実測判明 → **合格基準を2軸に再設計**。
      - **合格基準（再設計後・2軸だけ）＝① L（押し手数）② 貪欲NG。** deadlock 数・強制手率・noEscape は**捨てた**（全到達状態の列挙が定義＝難しい倉庫番＝状態爆発するほど列挙不能＝この枠組みで難易度を保証しようとするのが破綻。倉庫番の求解は PSPACE 完全）。
        - **① L＝逆算 pull-BFS の「引き距離」で測る**（押しと引きは厳密に逆操作 ∴ goal から引いた距離 = その配置から goal への最短押し手数）。石+プレイヤーだけの軽い状態＝**フル BFS を廃止＝OOM の主因を除去**。盤面は 10×12 で上限固定 ∴ L が大きい＝石が曲がりくねる＝難しい（[[blade-puzzle-must-verify-with-solver]]）。
        - **② 貪欲NG＝素朴なヒルクライム（近づく押しだけ）では解けない**（前向き探索だけ＝OOM しない・一直線押しの水増し L を弾く保険）。
      - **成果物：**
        - **生成器 `scripts/generate-key-room-dark-tower-43.mjs`**＝pull-BFS で候補を逆算し貪欲NG で篩い L 降順に採用。v1（床66・4隅ボタン＋中央千鳥ピラー）で **貪欲NG かつ最大 L=42** の配置を発見：**石 `4,3 5,8 6,6 7,7`・プレイヤー初期 5,3**（PULL_CAP=5M で引き距離32打ち切り＝さらに深い L がある可能性はログに明示済み）。解の存在は pull-BFS が構成的に保証（goal から引いた配置 ∴ 逆に押せば必ず解ける）。
        - **移行 `scripts/migrate-key-room-dark-tower-43.mjs`**（自己検査型・生成器の `analyze/assertGeometry` を再利用＝規則を二重化しない）で `work/blade-of-lumia.json` の dark_tower `4,3` へ書き込み済み。`showConditions {"7,6":{trigger:"stonesPlaced"}}`／`fluteEffect {type:"resetStones"}`／敵・`enemyDirs`・`floorItems` を全消し／`links:[]`／北入口(0,5)(0,6)・南鍵扉(9,5)(9,6)・鍵 K(7,6) は据え置き。
      - **検査：** `check-dungeon-integrity.mjs` 0エラー（既存 D3 水警告1件のみ・無関係）／`check-dungeon-connectivity.mjs dark_tower` PASS／実ゲーム（vite dev・`work` を直読み）で pageerror ゼロ・敵ゼロ・石4/ボタン4/鍵扉2枚が正しく描画（`.scratch/shots/dt43-sokoban.png` で目視裏取り）。
      - **テスト＝`tests/dungeon-key-gate.spec.js`：** ① EXPECTED 表の `dark_tower/4,3` を `stonesPlaced` に更新／⑨ 脅威度の単調増加から `dark_tower[4,3]` を除外（純倉庫番＝敵ゼロ＝脅威度0・V を THREAT から削除）。stonesPlaced の挙動（足踏み不成立・全石で成立・恒久ロック）は D4/D6/D8 が同じ conditions.js を共有して担保済み ∴ 4,3 固有の再生テストは追加していない。
      - **▶ ユーザーが試すには（正規ステージ）：** 本番盤面のミラーを `test_mechanics[24,0]`（`sokoban_darktower`）に置いた＝**dark_tower[4,3] と1文字違わぬ同一盤面**でゴール挙動（stonesPlaced→鍵出現・笛 resetStones）も同一。**検証ステージは fixture でなく test_mechanics に置く決まり**（DECISIONS 2026-07-25／PLAN 4.6-(4)）に従う＝エディタで開ける・プレビューで試せる（当初 `.scratch` の使い捨て HTML で出したが決まり違反 ∴ 是正・使い捨ては削除）。`migrate-key-room-dark-tower-43.mjs` が本番と test を同時に書く（片方だけ直すと乖離）。試す URL（vite dev・port 18080）＝`…/game/index.html?fromEditor=1&layer=test_mechanics&stage=24,0&row=0&col=6&ps_flute=1`、または editor でレイヤー `test_mechanics` → `24,0` を開いてプレビュー。`tests/test-stage-keys.js` に `sokoban_darktower: '24,0'` を登録済み。
      - **⚠️ 別件で発見した進行の穴（この部屋とは無関係・キューに積む判断が必要）**＝dark_tower は **鍵ゼロでボス部屋に入れる**。`b1fStairsUp` の着地セル `0,1 (0,5)` がボス部屋 `0,0` の `:` 扉 (9,5)(9,6) の真下にある∴`0,1` → ワープ → `1,0 (6,4)` → 一歩降りて踏み直す → ワープで `0,1 (0,5)` に戻る（着地直後はクールダウンで再発火しない）→ 北へ歩くだけで `0,0` に入れる。**既存チェッカーは検出できない**（`check-dungeon-connectivity.mjs` の徒歩 BFS はワープを辿らない／`:` は `SOLVABLE_GATES` 扱いでデッドエッジにならない）。再現スクリプトは `.scratch/dt-reach3.mjs`（到達集合）と `.scratch/dt-path.mjs 0,0`（経路復元）。

- [x] **5.5j. 10室の通し検証＝ユーザーが進行順に実プレイして判定する（2026-08-12 完了・合格）（🧠 Opus）**。**✅ ユーザー最終判定（2026-08-12）＝10室すべて OK。** 残っていた `dark_tower[1,2]`（A 戦闘型③）も 0番の AI 作り直し後の実プレイで合格＝**「だいぶバランスよくなった。なにより剣獣自体がいい感じの強さになった」**＝関門10室の型の配分・難化カーブ・理不尽さの無さがユーザー判定で確定した。∴**Phase 5.5（鍵の関門を本物の難易度へ）は合格**（残りの 5.5k／5.5m は「敵カタログを増やす」「世界へ配置する」＝関門の合否とは別軸のタスク）。全10鍵を進行順に取り、① 型が毎回違うと感じるか ② 詰まって投げる関門が無いか ③ 序盤→終盤で難化を感じるか を判定する（＝5.5 の合否）。機械側は `check-dungeon-integrity.mjs`（検査1〜9）＋`check-dungeon-connectivity.mjs` ＋ 各室の実機テストを全部緑にしておく。
      **▶ ユーザー判定の途中経過（2026-08-10）＝10室中9室 OK。残 1室のみ NG ＝ `dark_tower [1,2]`（A 戦闘型③・killAll）。** ユーザー指摘＝「そもそも通常敵の種類が少なすぎる（陸上の通常敵は E パトロール／C チェイサー／F センチネルの3種だけ・ボス系は除く）。もう少し強い通常敵が欲しい。最低10種くらいに増やし、その中の最強のものをこの部屋に集めたい」。∴ **`dark_tower [1,2]` の合格は下記 5.5k（通常敵の拡充）が前提**＝5.5j 全体の最終合否は 5.5k 完了後に 1,2 を再判定して出す。**他の9室（D1〜D8＋dark_tower 4,3）は判定 OK。**
      **▶ 残1室の決着（2026-08-12・合格）＝`dark_tower [1,2]` は構成（剣獣 `μ`×5＝脅威度 50.0＋射線を切る柱2本）を据え置いたまま、剣獣の行動 AI を作り直して合格した**（攻撃硬直・プレイヤーより遅い速度・遠隔／近接の二相＝下記「0番 完了記録」）。判定の経過＝(1) k-2 の組み直し直後の実プレイは**「だめだね」で否決**（攻撃中も動く／逃げ切れない／複数体で倒せない／ビームを撃たない／1セルまで詰めてくる）＝**データ上は閉じていたが敵として成立していなかった** → (2) 0番で AI を作り直し（フル 557 passed）→ (3) 再判定で**合格**（「だいぶバランスよくなった。なにより剣獣自体がいい感じの強さになった」）。**∴ 10室すべて OK＝5.5j 完了。**

- [ ] **5.5k. 通常陸上敵を拡充する＝新規15種を実装（既存 E/C/F と合わせ計18種）（🧠 Opus）**。**⚠️ 着手前に必ず `ENEMY-DIRECTIONAL-GUIDE.md` を読むこと（2026-08-10 skeleton の①機構実装＋ガード実効化で踏んだ罠の集約＝guardRangeの罠／敵と重なれない制約／歯の確認の落とし穴）。スプライトを作るときは `SPRITE-PIPELINE.md`（複数フレームは `quantize-shared.mjs` 必須＝§3-★）も必ず読む。** **この位置の理由（2026-08-10 ユーザー指示）＝5.5j の通し判定で 10室中 dark_tower `[1,2]`（A 戦闘型③）だけが NG。原因は「通常陸上敵が3種（E/C/F）しかなく、最終盤の戦闘部屋に集める"最強格"の選択肢が無い」＝敵カタログの貧弱さそのもの。∴ 5.5j の最終合否（1,2 の再判定）と 6番（深洋O）より前に、まず敵種を増やす。**
      - **✅ ブレスト完了（2026-08-10・superpowers:brainstorming）＝下記ロスター18種で確定した。** 当初「最低10種」だったが、ユーザー指示「無理に10に絞らず良い案は全部入れたい」で**新規15種＝計18種**に拡大。各行動が重複しないよう組んである（＝18種でも [[blade-no-mass-production]] に反しない一点物・量産ではない）。
      - **🖼️ 解像度＝全敵を 32×32 ドットに統一する（2026-08-10 ユーザー指示）。** プレイヤー本体（`heroD` 等）は **32×32**、既存の通常敵（`patrol`/`chaser`/`sentry`）は 12×16・水棲雑魚も同程度・2×2 大型ボスは 24×24 とバラバラ＝**ドット密度がプレイヤーより粗い**。ユーザー要望＝「スプライトのドットサイズをプレイヤーと同じにしたい。既存の敵も。でも表示サイズは今まで通り」＝**解像度（配列の縦横ドット数）だけプレイヤーと同一の 32×32 に上げ、画面表示サイズは据え置き**。**✅ エンジン変更は不要と裏取り済み**＝`shared/sprites.js drawSprite()` は `canvas.width = grid[0].length`（配列のドット数をそのまま canvas 内部解像度に採る）／表示サイズは CSS `.char-abs canvas.sprite { width: var(--cell) }` ＋ `image-rendering: pixelated`（`game/css/board.css:102,178`）で固定。∴**スプライト配列を 32×32 にするだけで表示サイズは変わらない**。**範囲＝新規15種＋既存全敵（E/C/F・水棲3・全ボス2×2 は 64×64 相当）を一括で 32×32 化する**（ユーザー確定 2026-08-10）。ただし一括は重い∴**まず #8 で 1体パイプラインを確立してから反復する**（下記のレビュー駆動を全敵に適用）。
      - **🎨 スプライト方式＝OpenAI 画像 API で「ドット絵として描け」とプロンプトし PNG 生成→量子化スクリプトで既存形式（パレット番号の2次元配列・32×32）に落とす**（既存の描画経路・テスト不変・ゲームの見た目が統一される）。**⚠️ OpenAI 生成は今回が初回∴まず1体だけ試す運用（ユーザー確定）：** (i) `.scratch/`（git-ignore）に使い捨てスクリプトで OpenAI 画像 API を叩く（キー＝`.env.local` の `OPEN_API_KEY`・gitignore 済み確認）／(ii) **使うプロンプトをユーザーに報告**／(iii) **生成 PNG をユーザーに見せる**／(iv) 量子化して canvas 再描画したものも見せる／(v) **ユーザー OK 後に `shared/sprites-enemies.js` に登録**。**最初の1体＝#8 骸骨剣士（形が単純＝試金石向き）で往復パイプラインが回ることを確認してから残り14種へ。** 量産的に一気に生成しない（1体ずつレビュー駆動）。
      - **✅ 生成手段の確定（2026-08-10・第N セッション）＝OpenAI（`gpt-image-1`）を採用。** `.scratch/gen-enemy-sprite.mjs`（使い捨て・git-ignore）で #8 骸骨剣士を生成し、ユーザーが実ファイルを開いて**透明背景・グロー無し・くっきりドットで合格と判定**（AI が Read で見た縮小版はグレー地＋グローに見えたが、それは AI 側の画像プレビュー縮小の癖＝実ファイルはクリーン）。参考に Gemini でも同プロンプトを試し良好だったが**OpenAI で確定＝Gemini は保留（フォールバック候補）**。API パラメータ＝`{model:'gpt-image-1', size:'1024x1024', background:'transparent', n:1}`。
      - **✅ 量子化パイプライン確立（2026-08-10・第N+1 セッション）＝5.5k の「本当の関門」を突破した。** `.scratch/quantize-sprite.mjs`（Playwright chromium 内で `drawImage`→`getImageData`）＝①α閾値128でグロー/半透明を落とし不透明ピクセルの BBox を取る ②正方パディングで縦横比を保ちつつ 32×32 グリッドへ平均縮小（不透明が35%未満のセルは透明）③**最遠点法 k-means（k-means++ の決定版・乱数なし）で約6色に量子化**し index0 を transparent 固定・輝度順ソート ④パレット番号2次元配列＋パレット hex を JSON 出力＋拡大再描画 PNG。**当初は明度順均等サンプルで面積の大きいベージュ（骨）に色が偏り、剣灰/盾赤を取りこぼした→最遠点法に直して `#8b3b31`（盾赤）・`#7f7f7e`（剣灰）を正しく捕捉。∴ 量子化での色破綻は解消し、保留していた Gemini 切替は不要と確定。** **採用スプライト＝量子化後の 32×32（比較画像の下段）。**
      - **✅ 参照画像方式を確定（2026-08-10）＝ポーズごとの配色ズレ・黒縁混入を解消。** ポーズを別々にプロンプトすると配色がバラつき黒縁が出た（ユーザー指摘「横向きの画像もプロンプトに渡し統一感を出すべき」）→ `gen-enemy-sprite.mjs` に `images/edits`（FormData `image[]`）経路を追加し、**合格した横向き素材 `openai-skeleton.png` を参照画像として渡す**（第4引数・カンマ区切り可・渡すと `images/edits`、無ければ従来の `images/generations`）。配色統一＋黒縁除去が自動で効くことを確認。**目視確認ツール＝`.scratch/compare-sprite.mjs`（1体を3枚比較）／`.scratch/compare-multi.mjs`（複数ポーズを上段=生成PNG・下段=量子化のグリッド比較）→`.scratch/shots/compare-*.png`。**
      - **✅ #8 DOWN 4枚が全て確定（2026-08-10）。** 最初は採用可が歩行1のみ（歩行2=黒縁＋剣消失／攻撃=盾左右両方＋剣消失／ガード=盾消失＋くり抜き）→ プロンプト改善策 **B+A+C 併用**で全4枚確定：B=参照2枚（色基準の横向き素材＋同方向の採用済みフレーム）／A=持ち物・利き手・線・色をポーズ語の**前**に定数プリアンブルで毎回固定／C=ポーズ語を具体動作に言い換え＋向きを `FULLY FRONT-FACING / NOT a side view` と明示。攻撃・歩行2は非決定性対策で同プロンプト2枚引き。確定＝`.scratch/skel-down-final-{walk1,walk2,atk,guard}.png`（比較=`.scratch/shots/compare-skel-down-FINAL.png`）。**📖 スプライト生成の手順・プロンプトの型・ハマりどころは `SPRITE-PIPELINE.md`（新規・恒久文書）に集約した＝残り方向・残り14種・既存全敵で必ず参照する。**
      - **✅ #8 データ疎通確認（2026-08-10）＝skeleton を ENEMY_META/PAL/SPRITES/TILE に最小登録し test_mechanics 0,0 で実機描画を確認した**（type=`k`・hp5・32×32 canvas・不透明196px 描画・pageerror なし）。既存の char-layer 機構に新 32×32 スプライトがそのまま載ることを裏取り。**ただし敵として3つの欠陥が判明（ユーザー指摘）→ 設計を `DECISIONS.md`（2026-08-10「陸上敵の真4方向＋攻撃/ガードポーズ機構」）に確定した：** (1) walk1/walk2 の身長・接地が不揃い（歩行でなくズームに見える）＝量子化の独立BBox正規化が体格差を増幅、(2) 攻撃/ガードモーションを出す発動機構が敵側に無い、(3) 常に正面固定（向き別攻撃絵を活かせない）。**調査の最重要事実＝このプロジェクトに真の向き別描き分けの前例はゼロ**（既存ボスの D/R/L/U は全て正面絵の参照エイリアス＋flipX・向き差替は hitAndAway ボス経路のみ・敵の攻撃/ガードポーズ差替は不在）。
      - **▶ 3段階着手（DECISIONS.md 2026-08-10）＝これは #8 単体でなく陸上敵15種の共通土台。**
        - **✅ ①エンジン基盤 完了（2026-08-10・⚡ Sonnet）。** `shared/enemies.js` SKELETON に `directional:true`＋`attack`を`sword`（range1.5/cooldown900）に変更。`shared/sprites-enemies.js` に `skeletonD/R/U`＋`*Atk`/`*Guard` の参照エイリアス（**L は登録せず既存 dirSuffix マップどおり R の flipX で代用**）。`game/enemy-ai.js` に `resolveEnemySprite(e,meta,now)`（`game.js getHeroSpriteName()` が雛形＝攻撃窓→構え窓→通常の3段）＋`syncDirectionalSprite()`（DOM差替）＋`tickGuard()`を新設し、`enemyTick()` が `meta.directional` の敵だけ毎tick経由（既存 hitAndAway ボス経路とは独立＝後方互換）。攻撃発火時に `e._atkUntil`（`ATTACK_POSE_MS`共有）、近接時に `e._guardUntil` を立てる。`game.js getEnemiesSnapshot()` に `dir/sprite/atkUntil/guardUntil` を追加（テスト観測用）。**恒久テスト＝`tests/enemy-directional.spec.js` 新規6本**（歯の確認済み）。フルスイート **539 passed**（回帰なし）。詳細は DECISIONS 2026-08-10。
        - **✅ ガードの実効化 完了（2026-08-10・同日・⚡ Sonnet・ユーザー実機報告「防御全然してくれない」で即修正）。** ①の`_guardUntil`は見た目の窓だけでダメージは無効化されていなかった＝**再設計＝ガード中は移動/攻撃を止め向きをロック、ロック方向からの攻撃だけ無効化（側面/背後は通る）、ブーメラン命中は常にスタン＋ガード強制解除**（ユーザー確定の攻略手段＝ブーメランで止めて殴る／盾ガードの隙に回り込む）。`enemy-ai.js tickGuard()` を状態機械に全面書き換え・`combat.js isGuardBlockingDir()`新設＋`dealDamageToEnemy()`に配線。**guardRange のバグ＝旧「威圧」時代の値(`range*1.6`)のままだとガード中は剣の射程外になり判定と無関係にhp不変＝歯のないテストを書いてしまった**→`guardRange=sword.range`に修正して発覚・テスト修正。テスト4本追加（歯の確認済み）。フルスイート**543 passed**（うち②の分含む）。
        - **✅ ②量子化を共通スケール化 完了（2026-08-10・同日・⚡ Sonnet・ユーザー実機報告「歩行アニメの1コマの身長がでかいまま」で即修正）。** `quantize-shared.mjs`＝画像ごと独立BBox正規化だと生成PNGの体格差（walk1高さ22px/walk2高さ32px）がスケール差に増幅される＝**全フレーム共通の接地ライン＋共通スケールを先に計算し全フレームに適用**するよう修正。skeleton確定4枚を再量子化し`ENEMY_SPRITES.skeleton`（歩行2）/新設`skeletonAtk`/`skeletonGuard`に実データを反映。実機で身長が揃ったことを画像確認済み。詳細は DECISIONS 2026-08-10。
        - **✅ ③向き別 R/U＋攻撃/ガードのスプライト（#8 骸骨剣士）完了（2026-08-11・🧠 Opus）。** 12枚（DOWN/RIGHT/UP × 歩行2＋攻撃＋構え）を確定し `shared/sprites-enemies.js` の参照エイリアスを実データへ差し替え（`skeletonR/RAtk/RGuard`・`skeletonU/UAtk/UGuard` は実データ／`skeletonD/DAtk/DGuard` は spawn 既定の `skeleton` と同じ絵∴エイリアス据え置き）。DOWN も新しいスケール基準で作り直した。実機で **4方向×4ポーズ＝16枚**を撮って確認（`.scratch/shots/ingame-skel-dirs.png`）＋登録データを名前どおり並べた12枚（`.scratch/shots/registered-skel-12.png`）・フルスイート **543 passed**（回帰なし）。**再利用手順は SPRITE-PIPELINE.md §6 に6ステップで確定**＝残り14種はこれをそのまま反復する。詳細は DECISIONS 2026-08-11。
          - **🔑 方式確定（2026-08-11・ユーザー指摘を受けた転換）＝1方向につき生成するのは「歩行1」1枚だけ。歩行2・攻撃・ガードは歩行1の 32×32 グリッドを直接編集して作る**（`.scratch/grid-tool.mjs` 新設・SPRITE-PIPELINE.md §1／§3-1★★）。契機＝ユーザー指摘「1,3 がちょっと低く 2,4 がちょっと高い＝サイズをあわせる必要がある」＋「右を向いているのだから盾は右側を正面として向いていなければならない（プレーヤーが右向きの時と同じ盾の表示）」。**前者の根因は量子化ではなく生成側の体格差**（ポーズごとに引くと頭の大きさ・身長が変わる∴選んでも揃わない）／**後者はプロンプトで押し切れない**（位置まで指定しても6枚とも参照どおり左に円盤を置く）。**同じボディを流用すれば体格・接地は原理的に一致し、盾も機構（`render-chars.js` の `shieldSide`＝幅0.17／高0.44 の側面バンド）に合わせて正確に置ける。** 副産物＝API 呼び出しが 1/4・引き直しが消える。
          - **▶ RIGHT 方向 4枚 確定（2026-08-11）。** 歩行1＝生成 `skel-right-walk1.png`（=f1a）／歩行2・攻撃・ガード＝グリッド構築（歩行2＝脚を stride→passing に描き替え・攻撃＝腕を右へ伸ばし水平の灰色の刃＋鍔・ガード＝左の赤い円盤を消し右側に幅5×高14 の赤バンド＋外側1列の暗色の縁、剣は背側に立てる）。**確認画像＝`.scratch/shots/quant-right-set-E.png`**（1=歩行1／4=歩行2／5=ガード／6=攻撃・2と3は体格が大きい生成物で破棄）。却下＝f1b/atkb/guarda/guardb（黒縁）・f2a（剣と盾が消失）・f2c（全面グローで α<128＝量子化不能）・guarde/guardf（盾が左のまま／黒縁）・f2b/atka（体格が他と揃わない）。
          - **⚠️ UP（背面視）は画像生成では作れなかった＝6回連続で剣の刃が消える**（プロンプトを3系統試した：刃の明示／刃指定を先頭へ／参照を刃の大きい RIGHT フレームへ差し替え。いずれも柄だけ描いて灰色の刃を描かない＝背面視だと剣が体の陰に隠れると解釈されるらしい）。∴ **方針転換＝生成するのは胴体だけ・刃は 32×32 グリッドで描き足す**（1024px PNG 上での移植＝`.scratch/graft-blade.mjs` は半透明グローの灰色を拾って bbox が全面になる∴廃止。**決着＝量子化後のグリッドに `fill` で刃を引く**＝共通パレットなのでスタイルは崩れない）。**採用＝`skel-up-f1b.png`**（黒縁なし・背面で顔なし）＋`flipx`（背面視は持ち手が左右入れ替わる）。
          - **✅ `quantize-shared.mjs` のバグ2件を修正した（2026-08-11）。** (1) **接地の扱いを2度直した**＝共通接地ライン（全フレームの最大 `maxY`）では足の位置が高いフレームの頭が切れる→`oy` を `minY` でクランプしたが、今度は図がウィンドウ内で浮いて**フレームごとに背が高い/低いに見える**（＝ユーザー指摘の直接原因）。**確定＝共通にするのはスケール（辺長）だけ・下端は各フレーム自身の `maxY`＝足を窓の最下段に揃える**（`oy = box.maxY - commonSide + 1`・`commonSide >= bbox 高さ` ∴頭も切れない）。(2) **6枚以上で Node が OOM**＝1024²×4 の生ピクセル配列を全画像分まとめて page 境界を越えて渡していた→**1枚ずつ2パス（パス1=bbox のみ／パス2=32×32 グリッドのみ）**に変更。**12枚（3方向×4フレーム）を1回で共通量子化する必要がある∴この修正は③の前提。**
          - **✅ `quantize-shared.mjs` のバグさらに2件を修正した（2026-08-11・向きをまたいだサイズ不一致の根因）。** (3) **サイズを揃える軸は「身長（32行中の行数）」**＝新オプション `--height 28`。辺長を `max(幅,高さ)` にすると横に広い方向（剣を横に構える RIGHT・盾を横に出す DOWN）は幅が窓を決めて身長がその分縮む（実測＝DOWN 26行／RIGHT 31行／UP 29行＝向きを変えると背が伸び縮みして見える）。辺長 = bbox高さ × 32/行数 で逆算＝**修正後 28/27/28**。(4) **bbox は不透明ピクセルの連結成分から取る**（4近傍ラベリング・最大成分の2%未満を捨てる）＝生成PNGの縁に残る**長いゴミ線**（実測＝DOWN は row 0 に 162px の横線／RIGHT は row 144 に 8px の点列）を図と誤認すると bbox が画像の縁まで伸び、そのグループ全フレームが小さく描かれる。行/列ごとの不透明数に閾値を掛ける方式では足りない。
          - **▶ DOWN も作り直した（2026-08-11）。** 5.5k② で確定した4枚は旧スケール基準∴新しい `--height 28` の基準ボディから歩行2・攻撃・ガードを再構築（歩行2＝足を**2行**上げるだけ〈当初1行＝差分8ドットで歩行に見えず作り直した〉／攻撃＝橙の鍔＋幅3の刃をフレーム下端まで／ガード＝赤い凧形の盾を胸の前）。**却下した中間案＝細い棒＋浮いた板（棒立ちに見える）・太いブロック（踏み台/金床に見える）・幅12の橙バー（ベンチに見える）・右へ突き出す長い前腕骨・胸を覆う赤い塊（肋骨が消える）。** 描き直しの基準は好みではなく**既に採用済みの `skeletonAtk`/`skeletonGuard` のグリッド**に合わせた。
          - **▶ 骨の色を方向間で揃える必要があった（2026-08-11）。** 共通パレットで量子化しても生成PNGの明るさが方向ごとに違うと**同じ骨が別 index に落ちる**（実測＝DOWN `#fad8aa`=6／RIGHT `#f0c78f`=5）＝向きを変えると色が変わる。`grid-tool.mjs` に `map from,to,rect`（index置換）を追加し RIGHT に `map 5,6` を掛けて解決。
          - **▶ 登録は機械的に流し込む・確認は実機（2026-08-11）。** `.scratch/emit-skeleton.mjs`＝`quant-skel12b.json` から `shared/sprites-enemies.js` のパレット行＋SKELETON セクションを置換（12×32行を手で書き写すと転記ミスが混ざる）。実機確認＝`.scratch/shot-skel-dirs.mjs`（`test_mechanics 0,0` の常設 skeleton に4方向から寄せ敵の canvas を直接撮る）。**★ left の反転は `getComputedStyle().transform` では検出できない**＝`makeSprite(name,pal,true,flipX)` が**ピクセルに焼く**（CSS transform ではない）∴目視で確認する。
          - **⚠️ 「完了」報告のあとユーザー指摘で3件を修正した（2026-08-11・同日）。確定 json＝`quant-skel12n.json`。** (a) **検証スクショのラベルが「意図」だった**＝歩行/ポーズの枠に、勝手に立つ構え窓・攻撃窓の絵が入り込んで読み違いを生んだ→**ラベルは実スプライト名（`k.sprite`）から作る・攻撃窓と構え窓を別々に待って両方撮る（16枚）・盤面が出たら即 `pause()`（上下からの接近は距離2しかない）・`animFrame` 待ちで `resume()` しない**。併せて登録データを名前どおり並べるだけの版 `.scratch/shot-skel-frames.mjs` を追加。 (b) **UP の構えに RIGHT の側面バンドを流用していたのが誤り**（側面バンドは盾の正面が画面左右を向くときの形）。 (c) **作り直した UP の盾が長方形の板だったのも誤り**＝DOWN の盾は凧形（平らな上辺＋下端が尖る・9幅×15行）∴**同じ盾に見えない**。**確定＝盾のシルエットは全方向で共通・変わるのは正面の向きと重なりだけ**＝UP は DOWN と同じ凧形を、DOWN の位置を体の中心で鏡像にした場所（体 x9-23／盾 x20-28）へ**歩行→構えの差分（3行持ち上げ・体側へ寄せ一段大きく）も鏡像で**当てる。盾は体より奥∴`map 0,1`（透明セルだけ盾色に置換）で骨を手前に残す。裏取りは `game/render-chars.js`（プレイヤーも**上向きだけ**盾を z-index -1＝体の裏に描く）。 🔑 **「DOWN の盾は横に構えているから全形が見える」は誤り＝正面（向いている方向＝カメラ側）に構えているから全形が見える。画面左寄りなのは腕の側だから**（ユーザー訂正）。詳細は DECISIONS 2026-08-11「⚠️ ③完了報告の誤り」。
      - **📐 プロンプトの知見（2026-08-10）＝「32×32 いっぱいに描く・マージン最小」を明示する**。初回の #8 は OpenAI が 32×32 グリッドに対しマージンを取って描き、キャラが少し小さめになった（Gemini はほぼフレームいっぱい）。∴プロンプトに `The figure should fill the entire 32x32 frame edge to edge, with minimal margin / no empty border` の趣旨を足す。**確定プロンプト雛形（#8 で使い OpenAI 成功したもの）：** "A single pixel-art sprite of a {敵名・特徴} enemy for a top-down 2D action RPG in the style of the original NES Legend of Zelda. The character faces to the RIGHT (side view). {装備・色の指定}. Rendered as clean pixel art on a strict 32x32 pixel grid: bold flat colors, hard black outline, no anti-aliasing, no gradients, a very limited palette of about 6 colors. **The figure fills the entire 32x32 frame edge to edge with minimal margin.** Fully transparent background (no floor, no shadow, no border, no text). Centered, standing upright." ＝敵は**右向き素材**（既存 `patrol`/`chaser`/`sentry` と同じ・左は描画時 flipX）で統一する。
      - **🐣 確定ロスター（陸上通常敵・計18種／新規15種）。** 脅威度=`hp*atk/(def+1)`。🆕=新エンジン機構／♻=既存語彙の組合せ：
        - **既存3種（据え置き）：** E パトロール(charge鈍足・脅威3)／C チェイサー(charge通常・10)／F センチネル(spear遠隔・12)。
        - **1. 地中蟲** ♻＝既存 `submerge`（水棲鮫）を陸に転用。潜伏↔急浮上のリズム、浮上した一瞬だけ殴れる/殴られる。弱点なし・脅威 低。**✅ 実装完了（k-3a＝エンジン・タイル `α`・脅威 4.0・機構は `meta.hide`／k-3b＝専用スプライト 32×32 歩行2枚）。**
        - **2. 跳躍蜘蛛** 🆕跳躍移動＝不規則ジャンプで間合いを詰め、滞空中は当たり判定消失（着地の瞬間を狙う）。接触ダメージ・脅威 低〜中。**✅ 実装完了（k-3a＝エンジン・タイル `β`・脅威 8.0・機構は `meta.leap` の4相／k-3b＝専用スプライト 32×32 歩行2枚・ただし `windup` の絵は k-3c）。⚠️ 「不規則ジャンプ」は乱数にしていない**＝跳躍は間合い（`minRange`/`maxRange`）とクールダウンで決まる＝プレイヤーがリズムを読める・テストが決定論的（GUIDE §7-3）。
        - **3. 分裂スライム** 🆕被弾分裂＝剣で1回叩くと2体の小型へ分裂（小型は分裂しない）。放置で数に押される。弱点 bomb・脅威 中。
        - **4. 盾騎士** 🆕方向依存被ダメ＝正面からの剣は弾く（0ダメージ＋ノックバック）、背後/側面のみ通る＝回り込み強制（既存 flank モードが初めて効く）。弱点なし（位置取り）・脅威 中〜高。
        - **5. 術士** 🆕瞬間移動して撃つ＝一定間隔で消え別セルに再出現→魔弾(stone型流用)。弱点 arrow・脅威 中。
        - **6. 爆弾鬼** 🆕範囲爆発の投擲＝放物線で爆弾を投げ着弾で範囲爆発（`!` も壊せる）。距離を取ると危険＝間合いを詰める圧。弱点なし・脅威 中〜高。
        - **7. 剣獣（ライネル型）＝最強格** 🆕剣ビーム＋高機動＝高HP・高速・飛ぶ斬撃を撃つ。**これを dark_tower [1,2] に集める。** 弱点なし・脅威 高（最上位）。
        - **8. 骸骨剣士** ♻徘徊近接＝ランダム徘徊しつつ接近で斬りかかる。雑魚の骨格。弱点なし・脅威 中。**（スプライト試作の最初の1体）**
        - **9. コウモリ群** 🆕ジグザグ高速飛行＝不規則高速で狙いを付けにくい。単体は脆い。弱点なし・脅威 低。**✅ 実装完了（k-3a＝エンジン・タイル `ξ`・脅威 2.0・機構は `move:'air'`＋`meta.zigzag`／k-3b＝専用スプライト 32×32 羽ばたき2枚・横向き**「コウモリ1匹」・小型（22×13）・翼を上げた/下げたの羽ばたき2枚**＝大量のコウモリはステージ配置で表す〈5.5m〉）。⚠️ 無敵窓は持たせなかった**（脆さと機動が売りの敵に無敵は要らない＝k-3a の項）**・「不規則」は乱数でなく `e.id` 由来の決定的な位相**。
        - **10. ブーメラン鬼（ゴーリヤ型）** ♻往復飛び道具＝往復するブーメランを投げる（避けても帰りに当たる二度読み・プレイヤーのブーメランと同エンジン）。弱点なし・脅威 中。
        - **11. ルピー喰い（ライク型・#11 は盾喰い→ルピー喰いに変更）** 🆕吸着奪取＝接触すると張り付き所持ルピーを吸う（倒すと一部戻る）。**盾を奪うと今のゲームは盾入手箇所が少なくリカバリ不能で理不尽∴ルピーに変更（ユーザー確定 2026-08-10）。** 倒す優先度を強制。弱点なし・脅威 中。
        - **12. 火吐き亀** 🆕甲羅ガード開閉＋火炎＝甲羅に籠もると全ダメージ無効、口を開けた瞬間に炎を吐く（そのスキに殴る）。盾騎士と別方向のタイミング型。弱点なし・脅威 中〜高。
        - **13. 呪い火（バブル型）** 🆕剣封じデバフ＝触れると数秒剣が使えなくなる（ダメージ小）。妨害特化＝弓/爆弾で処理。弱点なし・脅威 中(妨害)。
        - **14. 突進猪** ♻直線突進＋壁ヒット気絶＝プレイヤーの行/列に入ると高速直線突進、壁に当たると気絶（そこが好機）。既存 charge の強化版。弱点なし（気絶時のみ）・脅威 中〜高。
        - **15. 毒沼ヒル** 🆕遅延毒(DoT)＝接触で継続ダメージの毒を付与（時間で減衰）。鈍足だが後を引く。弱点なし・脅威 中。
      - **🔧 新メカニクスは共通機構に寄せる（個別実装しない・規則は1箇所＝[[blade-execution-queue-single-source]] の精神）：**
        - **隠れ↔出現の無敵窓**（#1地中蟲・#2跳躍蜘蛛）＝既存 `submerge`（e.submerged で無敵・攻撃なし）を一般化し陸/空にも適用。**✅ k-3a で実装（2026-08-13）＝`meta.hide`（style 付き）/`e.hidden` に改名。#9 コウモリ群はこの枠から外し `move:'air'`＋`zigzag` にした**（理由は k-3a の項）。
        - **被弾トリガー**（#3分裂・#11ルピー喰い着脱）＝`combat.js dealDamageToEnemy` にフック。
        - **方向依存の被ダメ**（#4盾騎士・#12火吐き亀）＝`dealDamageToEnemy` に「敵の向き vs 攻撃の入射方向」判定。亀は開閉フェーズで窓を開ける。
        - **投擲物の種別追加**（#6範囲爆発・#7剣ビーム・#10往復ブーメラン）＝`projectile.js` に proj type 追加（既存 stone/waterShot/waterBlade と同経路）。
        - **プレイヤー状態異常**（#13剣封じ・#15毒DoT・#11ルピー減）＝player 側に一時デバフ窓（既存 `_atkUntil` と同型の論理時間窓）。
        - **瞬間移動**（#5術士）＝`enemy-ai.js` に blink 行動（消える→別セル再出現→撃つ）。
      - **▶ 5.5k 内の実装順（2026-08-11 確定・上から順に1つずつ）＝敵1体ずつではなく「共通機構ごと」に束ねる。** 順序の根拠＝(a) 5.5j の残1室（dark_tower[1,2]）を閉じるものを最優先 (b) それ以降は**1つの新機構が解禁する敵の数が多い順**（機構を作ってから敵を並べれば、同じ機構の2〜3体目はほぼデータ追加で済む）。各段の中は「エンジン→歯のあるテスト→スプライト（SPRITE-PIPELINE §6）」の順。
        - **✅ k-1. #7 剣獣のエンジン（完了 2026-08-11）** ＝`guards:false`＋新攻撃タイプ `swordBeam`。詳細は下記の完了記録。
        - **✅ k-2. #7 の実スプライト（3方向×{歩行1,歩行2,攻撃}＝9枚）→ dark_tower[1,2] を #7 中心に組み直す（完了 2026-08-12）。** `guards:false` なので Guard フレームは不要＝#8 の12枚より軽い。詳細は下記の完了記録。**5.5j の残 1室はこれで決着した**＝組み直し直後の実プレイは「だめだね」で否決 → 0番で行動 AI を作り直し（構成は据え置き）→ **再判定で合格（2026-08-12）**。
          - **▶ k-2 の確定事項（絵の素材と仕様。完了後も参照する）。** 生成物は `.scratch/`（gitignore・ディスクに残る）。
            - **基準ボディ3枚＝`.scratch/quant-beast5c.json`**（frame 0=RIGHT / 1=DOWN / 2=UP）。プレビュー＝`.scratch/shots/beast-final5.png`。実測＝3方向すべて rows 28 / minY 4 / maxY 31（身長・接地が一致）・steel 43/17/27（刃あり）。
            - **共通パレット（登録時に `swordBeast` の6色を差し替える）**＝`['transparent','#741d17','#a22f1e','#6f7161','#c37124','#e0a12c','#a6a899']`（1/2＝暗紅・紅の毛、3/6＝鋼の影・光、4/5＝金のたてがみ）。
            - **絵の確定仕様（ユーザー判定）＝四足で前脚を腕のように使う**（ゼルダのライネル型だが腕は増やさない）／**剣は必ず個体の右前脚**（DOWN＝画面左・RIGHT＝前方・UP＝画面右。背面視は左右が一致し正面視は反転する）／**色は RIGHT 候補 R-b の系統に3方向とも統一**・黒縁なし／**DOWN の顔は生成候補 D-c**（`beast-down-f1c.png`）を採用＝頭部だけ D-d2 のボディへ移植した（`clear 0,4,24,16` → `copy 2,4,4,24,16,0,0`）／**DOWN の非剣側の前脚は地面（row 31）まで接地させる**（旧版は row 26 で終わり浮いていた）。
            - **元 PNG**＝RIGHT `beast-right-f1b.png` / DOWN `beast-down-d2.png`（頭は `beast-down-f1c.png`）/ UP `beast-up-u2.png`。UP は生成では刃が暗くなる/消えるので「明るい鋼色の刃・暗い縁なし」を明記して引き直した版。
            - ❌ 失効（2026-08-12・すべて完了した）＝旧「残り」リスト (a) 攻撃3枚のグリッド構築 (b) `shared/sprites-enemies.js` への登録 (c) 実機/データ直並べシートの自己確認 (d) フルスイート (e) `dark_tower[1,2]` の組み直し。**今どうすべきか＝下記「k-2 完了記録」を見る**（残っているのはユーザーの実プレイ判定だけ）。
        - **✅ k-3a. 隠れ↔出現の無敵窓＝エンジン＋歯のあるテスト（解禁3体＝#1 地中蟲 `α`・#2 跳躍蜘蛛 `β`・#9 コウモリ群 `ξ`）（完了 2026-08-13・🧠 Opus）。** 既存 `submerge`（水棲鮫の潜行）を `hide`（style 付き）に改名して陸/空へ一般化した。詳細は下記「k-3a 完了記録」。**⚠️ 旧記述の訂正（2026-08-13）＝この見出しは3体すべてを「無敵窓」の枠に入れていたが、実装では #9 コウモリ群に無敵窓を持たせていない。** 理由＝hp2/def0 の脆さと高機動が売りの敵に無敵窓を足すと「脆いのに殴れない」で性格が濁る∴コウモリは **`move:'air'`（水/溶岩/空を飛び越える）＋ `zigzag`（進路が左右に振れる）** で「狙いを付けにくい空の敵」にした。∴k-3 が解禁する機構は2つ（`hide`/`leap` の無敵窓＋飛行/ジグザグ）。
        - **✅ k-3b. #1/#2/#9 の実スプライト（32×32・SPRITE-PIPELINE §6）（完了 2026-08-14・🧠 Opus）。** k-3a のスプライトは既存絵のエイリアス（地中蟲←lurkShark・跳躍蜘蛛←chaser・コウモリ群←fishSchool）＝GUIDE §2「機構が先・絵は後」に沿った仮置きだった。**この位置の理由＝各段の中は「エンジン→歯のあるテスト→スプライト」の順（5.5k 実装順の規則）∴k-4 より先に絵を入れる**（k-2 が #7 で同じ順を踏んだ）。3体とも `directional:false`＝1方向＋左右反転で足りる（向き別12枚は要らない）。詳細は下記「k-3b 完了記録」。
        - **k-3c. 跳躍蜘蛛の「溜め」を画面に出す（🧠→⚡ Sonnet・設計は下記2行で確定済み）。** **この位置の理由＝k-3b の絵を入れた時点で判明した抜け＝`meta.leap` の4相のうち `windup`（360ms・跳ぶ直前の溜め）が画面に何も出していない**（`_leapPhase` は `game/enemy-ai.js` にしか出てこない＝`render-chars.js`・`board.css` のどちらも読んでいない・2026-08-14 に grep で裏取り）。∴プレイヤーが読めるのは「敵が止まった」だけで、GUIDE §6-1「絵は機構を読ませるためにある」を満たしていない。**k-3 段の最後の1本＝この段を閉じてから k-4 へ進む**（絵と機構の対応は段ごとに閉じる／k-4 は別機構＝方向依存被ダメ）。**やること＝(a) `leapSpiderWindup` を歩行1のグリッド編集で作る（脚を胴へ引き付け重心を下げる＝跳ぶ前の縮み。追加生成はしない＝SPRITE-PIPELINE §3-1★★） (b) `render-chars.js` で `e.leapPhase === 'windup'` のとき差し替える（`resolveEnemySprite()` の攻撃窓と同型＝新経路を作らない） (c) テストは「windup 中だけスプライト名が変わる」＝`getEnemiesSnapshot()` の `leapPhase` と DOM の canvas を突き合わせる1本**。
        - **k-4. 方向依存の被ダメ（解禁2体＝#4 盾騎士・#12 火吐き亀）。** `combat.js` の `isGuardBlockingDir()`（#8 で新設済み）を「向き固定の常時ブロック」「フェーズで開閉する窓」へ拡張する＝ゼロからではない。
        - **k-5. 被弾トリガーのフック（解禁2体＝#3 分裂スライム・#11 ルピー喰い）。** `dealDamageToEnemy` にフック点を1つ作る。
        - **k-6. 投擲物の種別追加（解禁2体＝#6 爆弾鬼・#10 ブーメラン鬼）。** #7 で `fireEnemyProjectile(..., extra)` の拡張点を用意済み（`extra` に放物線/往復のパラメータを載せる）。
        - **k-7. プレイヤー側の一時デバフ窓（解禁2体＝#13 呪い火・#15 毒沼ヒル）。** 敵の `_atkUntil` と同型の論理時間窓を player 側に置く。
        - **k-8. 瞬間移動（#5 術士）／k-9. 直線突進＋壁で気絶（#14 突進猪）。** 1体ずつの単独機構∴最後。
      - **✅ k-1 完了記録＝#7 剣獣（SWORD_BEAST `μ`）のエンジン（2026-08-11・🧠 Opus）。**
        - **`shared/enemies.js`**＝`ENEMY_META[TILE.SWORD_BEAST]`（hp10/atk3/def2/exp20・`speed: ENEMY_SPEED_FAST`（❌ 失効 2026-08-12・0番で `* 0.85` に変更＝同速だと逃げ切れない）・`directional:true`・`attacks` は近接 `sword`(range1.5/cd700) と遠隔 `swordBeam`(range9/**minRange2.5**/cd1500/projectileSpeed2.0) の二段構え＝潜み鮫と同型）。**脅威度 `hp*atk/(def+1)` = 10。**（0番で `attackFreezeMs: 360` と `combat`（遠隔／近接の二相）を追加した＝下記 0番 完了記録）
        - **🆕 `guards: false` を新設**＝**高機動の敵が立ち止まって盾を構えるのは設計と矛盾する**∴ガード状態機械（`tickGuard`）に乗せない（ガード役は #4 盾騎士の担当）。副産物＝**向き別スプライトが3方向×2ポーズ＝6枚で足りる**（#8 は Guard を持つので12枚）。`tickGuard()` 冒頭で `meta.guards === false` を早期 return。
        - **🆕 新攻撃タイプ `swordBeam`（飛ぶ斬撃）**＝プレイヤーのビーム剣と同じ `'beam'` 投擲物を `owner:'enemy'` で飛ばす。**縦横が揃ったときだけ撃つ**（`|dx|<1.0` または `|dy|<1.0`・斜めには飛ばさない＝プレイヤーは行/列を外して避ける）。`fireEnemyProjectile(e, type, ndx, ndy, speed, extra = {})` に `extra` を追加（k-6 の拡張点）。**⚠️ スイッチ/色スイッチのトグルは `owner === 'player'` に限定**＝敵のビームでギミックが動いてしまう抜け道を塞いだ。見た目は `.sword-beam.beam-enemy`（赤紫）でプレイヤーの光刃と区別する。
        - **検証ステージ＝`test_mechanics[27,0]`（`sword_beast`）。幾何の芯＝水路で隔てること**＝陸上敵は水に入れない（`enemyTilePassable`）が投擲物は飛び越える（`isTilePassableForProj` は壁だけで止まる）∴**速度 FAST の剣獣に密着される前に遠隔攻撃だけを観測できる**。`scripts/migrate-test-sword-beast.mjs`（自己検査型＝水路が途切れていないこと・東側の床が row 4 だけであること・陽性/陰性の距離条件を書き込み前に assert）。
        - **テスト＝`tests/enemy-sword-beast.spec.js` 新規7本**（歯の確認3つとも赤くなることを確認済み＝`guards:false→true` でガードのテストが赤／`swordBeam` 分岐を潰すと陽性2本が赤／`if (!sameCol && !sameRow) continue;` を消すと陰性が赤）。フルスイート **550 passed**。
        - **♻ 派生の是正＝`tests/dungeon-key-gate.spec.js` の手書き敵表を `ENEMY_META` 由来に置き換えた**（[[blade-enemy-tables-derive-from-meta]]）。旧記述は敵タイルを `/[WECFVXZALNJOUGI]/` の文字クラスで、脅威度を手書きの表で持っていた＝**skeleton も sea_lord も既に漏れており、5.5k の新規15種は全部漏れる**。導出後も D1 24 / D7 36 / dark_tower[1,2] 48 が数値どおり再現することを確認。
        - **♻ エディタの敵パレットに `SKELETON`/`SWORD_BEAST`/`SEA_LORD` を追加**（skeleton と sea_lord はパレットに存在しなかった＝エディタで置けなかった）＋`TILE_SPRITE_MAP` に向き別敵の代表絵（DOWN）を登録。
        - ❌ 失効（2026-08-12・k-2 で解消）＝旧記述「現在の絵は sentry の形の借り物＋`swordBeast` パレット」。**今どうすべきか＝剣獣は専用の 32×32 を9枚持っている**（下記 k-2 完了記録）。
      - **✅ k-2 完了記録＝#7 剣獣の実スプライト9枚＋`dark_tower[1,2]`「剣獣の巣」への組み直し（2026-08-12・🧠 Opus）。**
        - **スプライト＝3方向（DOWN/RIGHT/UP）×{歩行1, 歩行2, 攻撃}＝9枚**（当初想定の6枚に歩行2を足した＝他の敵と同じく2フレームで歩かせる）。OpenAI で生成したのは基準ボディ3枚だけ（`.scratch/quant-beast5c.json`）で、残り6枚は `.scratch/grid-tool.mjs` の 32×32 グリッド編集で作った（SPRITE-PIPELINE §6＝追加生成なし）。最終データ＝`.scratch/quant-beast9f.json`／登録は `.scratch/emit-sword-beast.mjs`（`shared/sprites-enemies.js` のパレット行と SWORD_BEAST セクションを機械的に差し替える＝9×32行を手で書き写さない）。
        - **⚠️ 自分で見つけて直した絵の欠陥＝剣のシルエットが方向ごとに違っていた**（刃幅は RIGHT 3・DOWN 1・UP 2、影の側も RIGHT は左／UP は右）。ENEMY-DIRECTIONAL-GUIDE §6-2「同じ物は全方向で同じシルエット」に反する∴**3方向とも「明鋼2列＋影1列・鍔は幅5」に正規化してから**歩行2・攻撃を作った（[[judge-obvious-visual-defects-yourself]]）。
        - **攻撃ポーズ＝向いている方向へ刃をフレーム端まで突き出す**（DOWN は下端 row31／RIGHT は右端 col31／UP は上端 row0）。RIGHT は3回作り直した（①刃が短くて突きに見えない ②頭から生えて見える ③前脚の上に構える＝採用）。歩行1/2 の差分ドットは 16/17/27（skeleton で合格した16以上）・歩行フレームは3方向とも rows 4-31（28行）で接地が一致。
        - **自分で見た確認シート**＝`.scratch/shots/registered-beast-9.png`（登録データを実スプライト名の順に並べた＋LEFT は R を flipX した行）／`.scratch/shots/ingame-dt12-dirs.png`（実機・4方向＋攻撃）／`.scratch/shots/ingame-dt12-nest.png`（実機の盤面）。
        - **`dark_tower[1,2]` の組み直し＝`scripts/migrate-key-room-dt12-sword-beast.mjs`（自己検査型）。構成は剣獣×5＝脅威度50.0**（旧 W1+C3 の 48.0 も D7 の 36.0 も上回る＝単調増加を壊さない）。**内部に 2×2 の柱を2つ立てた（(2-3,5-6)(5-6,5-6)）**＝剣獣は speed FAST かつ range9 のビームを撃つ∴開けた 10×12 に5体置くと十字砲火で一方的になる。柱は入口 (0,5)(0,6) の真下を塞いで「入室即被弾」を構造で消し、row4 だけを東西に射線が通る廊下として残す（投擲物は `TILE.WALL` で消える＝`game/projectile.js` の `canProjectilePass`）。
        - **書き込み前 assert（`migrate-key-room-d7-tower.mjs` から強化した点を含む）**＝形10×12・外周不変・鍵(7,6)は `K`・鍵扉(9,5)(9,6)は `D`・`killAll` 据え置き・脅威度は `ENEMY_META` から導出して D1(24)<D7(36)<新構成 かつ **旧48.0 を下回らない**・**歩ける床の実数**（柱を引いた76）÷敵数 ≧ 15・**入口から全床へ BFS 到達**（柱で床を孤立させていない）・**`enemyDirs` のキー集合が盤面の敵セルと完全一致**（旧データに残っていた失効キー `7,10` のような取り残しを弾く）。
        - **テスト＝`tests/dungeon-key-gate.spec.js ⑨` に2本追加**（フル **552 passed**＝基準550 に +2）：(a)「敵は通常敵の最強格（剣獣）だけを集めている」＝最強格は `ENEMY_META` から導出する（剣獣の脅威度10.0はチェイサーと同値＝**最大タイ**∴「これを上回る通常敵が現れたら巣の構成を見直せ」という警報として書いた） (b)「入口セルへ通る剣ビームの射線が無い」＝柱を消すと `敵(8,6)→入口(0,6)/(1,6)` が現れて赤くなることを実測で確認（歯あり）。既存の単調増加テストにも「旧構成 48.0 より弱くしない」を追加した。
        - **▶ 実測で分かったこと＝`enemyDirs` の初期向きは実質1フレームしか見えない**（剣獣は speed FAST で 1 tick 目にはもうプレイヤーの方を向く）。また spawn 直後の `e.sprite` は `meta.sprite`（=`swordBeastD`）のままで、向き別の絵は `syncDirectionalSprite` が走る tick で初めて入る。∴実機の4方向シートは「プレイヤーの立ち位置を3か所変えて集める」方法で撮った（`.scratch/shot-dt12-nest.mjs`）。
        - ❌ 失効（2026-08-12・ユーザー実プレイで NG＝「だめだね」）＝旧記述「▶ 残り＝ユーザーが `dark_tower[1,2]` を実プレイして『歯応えのある最終戦闘』と判定すること」。**今どうすべきか＝下記「0番 完了記録」の AI 修正版を実プレイして再判定する**（部屋の構成＝剣獣×5・柱2本は据え置き。指摘③「複数体で強すぎ」の原因は構成ではなく AI だったと実測で確定した）。
        - ❌ 失効（2026-08-12・0番で変更）＝旧記述「剣獣は speed FAST（プレイヤーと同速）」。**今どうすべきか＝`speed: ENEMY_SPEED_FAST * 0.85`**（同速だと真っすぐ逃げても間合いが開かない＝逃げ切れない）。柱を立てた理由（射線を切る）は現在も有効。
      - **✅ 0番 完了記録＝剣獣の行動 AI の作り直し（2026-08-12・🧠 Opus / WORKFLOW 🔀分岐B＝バグ報告フロー）。**
        - **原因（先に数字で特定した。5指摘は独立な3原因に還元される）**
          - **(A) 敵に攻撃硬直が無かった。** プレイヤーは `player.js:401` で `_atkUntil` の間 `processHeldKeys` を無視する＝攻撃中は動けない。敵側には同じ窓が無く、斬りながら移動できた（指摘①）。
          - **(B) 敵の速度がプレイヤーと完全同速だった。** `ENEMY_SPEED_FAST = 1.0` かつプレイヤーも `speed 1.0`（`processHeldKeys` が毎tick `MOVE_STEP 0.5`）∴**直線で逃げても間合いは1ミリも開かない**（指摘②）。
          - **(C) 行動が「常に `enemyChase`」しかなかった。** 剣獣は密着し続ける∴`swordBeam` の `minRange 2.5` に一度も届かず飛ぶ斬撃が発火しない（指摘④⑤）。5体いれば5体が同時に密着する（指摘③）。
        - **機構（敵種を問わない汎用・opt-in。`game/enemy-ai.js`）**
          - **`meta.attackFreezeMs`＝攻撃硬直の窓**。`markAttack(e, meta, i, now)` を新設し、6か所に散っていた `e._attackTimes[i] = now` を全部これに通した（＝クールダウン記録・`_atkUntil`（絵のポーズ）・`_freezeUntil`（移動禁止）を1か所で立てる）。既定は `directional ? ATTACK_POSE_MS : 0`＝**既存敵の挙動は変わらない**（非 directional は硬直0）。
          - **`meta.combat`＝遠隔／近接の二相**（`{keepMin, keepMax, rangedMs, meleeMs, startMode}`）。`tickCombatMode()` が固定時間で交替させ、遠隔相は新設の `enemyKeepDistance()`（`keepMin` より近ければ**退がる**・`keepMax` より遠ければ寄る・間なら**行/列を揃えて撃つために止まる**）、近接相は従来の `enemyChase()`。**`combat` を持たない敵は今までどおり `enemyChase` だけ**。
          - **交替のタイミングは乱数にしない＝`e.id` から決定的に位相をずらす**（`phaseOffsetMs`）。理由＝(1) 5体が同時に突っ込むと「一斉に袋叩き」に戻る (2) 乱数はテストが不安定になる (3) プレイヤーがリズムを読める。
          - **μ の値**＝`speed: ENEMY_SPEED_FAST * 0.85`／`attackFreezeMs: 360`（3 tick）／`combat: {keepMin: 3.0, keepMax: 6.5, rangedMs: 3000, meleeMs: 1800, startMode: 'ranged'}`。
        - **観測できないと直せない＝`getEnemiesSnapshot()` に `freezeUntil`/`cmode`/`cmodeUntil` を追加**。⚠️ `window.__game.getEnemies()` は**ホワイトリスト**＝ここに無い内部フィールドを `e._foo` で読むテストは常に `undefined`（＝assert が静かに空虚になる）。
        - **検証ステージ＝`test_mechanics[28,0]`（`sword_beast_arena`・`scripts/migrate-test-sword-beast-arena.mjs` が自己検査して書く）。外周だけ壁・内部は全面床＝遮蔽ゼロ**。理由＝27,0 は水路で東の廊下が5セルしかなく「モードが切り替わって寄ってくる／逃げると開く」を測れない。遮蔽を置かないのは**敵が止まった/離れたのが壁のせいか AI のせいかを混同しないため**。
        - **テスト＝`tests/enemy-sword-beast.spec.js` に4本＋`tests/dungeon-key-gate.spec.js ⑨` に1本（フル 557 passed）。歯も実測で確認した**：`attackFreezeMs: 0` にすると硬直テストが赤（硬直窓が 0 回）／`keepMin: 1.0` にすると遠隔テストが赤（1.0 < 2.5）／`speed` を `ENEMY_SPEED_FAST` に戻すと逃走テストが赤（間合いが 1.50→1.80 しか開かない）。
        - **▶ 測定の落とし穴（次に同種のテストを書くとき必ず踏む）**
          ...
          - **ユークリッド距離だけ見ると「逃げているのに縮む」。** 遠隔相で剣獣が縦に退がっていると、近接相でその縦のずれを食いながら追ってくる（実測 dist 3.0→2.12）。∴逃走の測定は**密着（近接相・dist ≤ 1.5）まで待ってから**始める。
          - **遠隔相の間合いは「切り替わった直後」では測れない**（近接相の終わりは 1.0 に居る∴退がるのに5〜8 tick かかる）。切替から10 tick 以上経った落ち着いたサンプルだけを見る。
          - **hp を積むのは「時間を進めるため」だけ**（gameover 中は `step()` が世界を止める＝そのあと来るモード交替を観測できない）。難易度の主張には使わない。
        - **指摘③（複数体で強すぎ）の判定＝部屋の構成は変えないと結論した。根拠は実測（`.scratch/probe-dt12-pressure.mjs` / `probe-dt12-clear.mjs`）**
          - 入口で **300 tick（36秒）立ち尽くしても「2セル以内に3体以上」が 0 回**（＝囲まれない）。剣の間合いに敵が来た tick は 71/300＝**反撃の窓が 24%**。
          - **この時点で実際に取れる装備**（世界に置いてあるのは木の剣 tier0 のみ・防具は鎖かたびら def4＝`player.atk 4` / `def 4`）で、素直に殴るボットが **5体を 54 tick（6.5秒）・被弾 3 で全滅させられた**（μ def2 ∴ 1発2ダメージ×5発×5体＝25 発）。∴「倒せない」は構成ではなく AI が原因だったと確定。
          - ∴ `scripts/migrate-key-room-dt12-sword-beast.mjs` と脅威度50.0・柱2本・`PREV_THREAT = 48` は**据え置き**。代わりに **`tests/dungeon-key-gate.spec.js ⑨` に「5体でも袋叩きにならない（2セル以内に3体以上が同時に居ない／反撃の窓がある）」を追加**＝二相を外して全員追跡に戻ると赤くなる。
        - **▶ 残り＝ユーザーが `dark_tower[1,2]` を実プレイして再判定すること**（5.5k 完了条件(3)）。
      - **✅ k-3a 完了記録＝隠れ↔出現の無敵窓を陸/空へ一般化＋新規3種のエンジン（2026-08-13・🧠 Opus）。**
        - **🔤 語彙の一般化＝`meta.submerge`→`meta.hide`（`{hiddenMs, shownMs, style}`）／`e.submerged`→`e.hidden`。** 「潜行」は水専用の名前∴陸（地中）・空（滞空）に広げるなら名前も一般化する（規則は1つ・名前も1つ）。`style` が見た目だけを分岐する（`water`＝波紋／`burrow`＝土の盛り上がり／`air`＝真下の影）。**改名は「読み手を全部直すまでが1セット」**（[[blade-water-single-source-readers]] の教訓）＝`shared/enemies.js`（潜み鮫の定義）／`game/enemy-ai.js`（`tickHide`・接触判定）／`game/combat.js`（無敵の早期 return）／`game/game.js`（spawn の初期値＋`getEnemiesSnapshot()`）／`game/render-chars.js`（再描画時のクラス復元）／`game/css/board.css`／既存スペック2本（`tests/sea-enemies.spec.js`・`tests/field-arm-o.spec.js`）。
        - **🆕 機構1＝タイマー駆動の隠れ（`meta.hide`）**。`tickHide(e, meta, now)` が `e._hideUntil` で `e.hidden` をトグルする。隠れている間＝**無敵**（`dealDamageToEnemy` 早期 return）・**接触ダメージなし**（`checkEnemyContact` スキップ）・**攻撃しない**（`enemyTick` が `enemyAttack` を呼ばない）。**追跡だけは続く**＝「殴れないが寄ってくる」がリズムの芯。
        - **🆕 機構2＝行動駆動の隠れ（`meta.leap`）＝4相の状態機械** `ground → windup（溜め・動かない）→ air（滞空＝隠れ＝当たり判定なし）→ recover（着地硬直＝殴れる窓）→ ground`。`tickLeap()` は自分が tick を所有している間 true を返し、`enemyTick` の移動/攻撃をまとめて止める（＝相ごとに if を散らさない）。**因果が hide と逆**＝時間で窓が開くのではなく、敵の行動が窓を開ける。`minRange` で「密着では跳ばない」（すり抜けを防ぐ）。
        - **🆕 機構3＝飛行 `move:'air'`（`game/passable.js enemyTilePassable`）**＝水・溶岩・空（虚空）を越え、**壁とゲートだけが障害**。既存の `move:'water'`（水しか泳げない）と対の3つ目の移動媒体。
        - **🆕 機構4＝ジグザグ飛行（`meta.zigzag = {amplitude, periodMs}`）**＝主軸（ずれの大きい軸）で詰めつつ、副軸はプレイヤーの脇 `amplitude` セルを目標に取り `periodMs` ごとに左右を入れ替える。**位相は `e.id` から決定的**（乱数なし＝GUIDE §7-3・テストが再現する）。**▶ 実測で決めた値**＝**1手おきに副軸へ振る**（毎手を副軸に使うと目標の入れ替わりに追いつけず1歩も近づかない／主軸を先に詰め切ると「遠距離は直線・密着してから振れる」＝ジグザグが見えない）。横の実効速度は `speed/2`＝0.167セル/tick ∴ `periodMs` は「片側へ振り切る時間」より長く取る＝**amplitude 1.0・periodMs 1440（12 tick）で左右対称に2セル幅**（720ms では±0.5・1200ms では -1.5〜0 の片側だけ、と実測）。
        - **3種の値（`ENEMY_META`・脅威度 `hp*atk/(def+1)`）**＝地中蟲 `α` hp4/atk2/def1/exp6（**4.0**・NORMAL・`hide:{1600,1000,burrow}`・噛みつき range1.4/cd900）／跳躍蜘蛛 `β` hp4/atk2/def0/exp8（**8.0**・SLOW＝地上は鈍足で跳躍が唯一の詰め手・`leap:{windup360, cells3, airSpeed1.0, cooldown1000, min1.8, max6.0}`・接触ダメージのみ）／コウモリ群 `ξ` hp2/atk1/def0/exp4（**2.0**・`FAST*0.7`＝プレイヤーより必ず遅い・`sideView`・`move:'air'`・`zigzag`）。3種とも剣獣（10.0）を超えない＝最強格の座を動かさない。
        - **🔡 ギリシャ文字タイル**＝`α`（地中蟲）/`β`（跳躍蜘蛛）/`ξ`（コウモリ群）。ASCII は使い切っている（5.5k のタイル予約表どおり）。`shared/tiles.js` の `TILE`＋`TILE_META`／`shared/tile-sprites.js`／`editor/editor-palette.js` の敵カテゴリまで3点セットで登録した（**パレット漏れ＝エディタで置けない敵**＝k-1 で skeleton/sea_lord が実際に踏んだ穴）。
        - **観測できないと固定できない＝`getEnemiesSnapshot()`（ホワイトリスト）に `hidden`/`hideUntil`/`leapPhase`/`leapUntil` を追加。**
        - **検証ステージ＝`test_mechanics[29,0] burrow_worm` / `[30,0] leap_spider` / `[31,0] bat_swarm`**（`scripts/migrate-test-hide-window-arenas.mjs`・自己検査8項目）。**1枚に3体を混ぜない**＝どの機構が壊れたか混ざらない。前2枚は遮蔽ゼロ（敵が止まった理由に地形が混ざらない）。**コウモリの盤面だけ中央に水の縦帯（col6・rows1-8）を置き、陸の4近傍 BFS で東西が完全に分断されていることを書き込み前に assert**＝「越えたら飛行の証明」になる幾何をデータ側で保証する。
        - **テスト＝`tests/hide-window-enemies.spec.js` 新規13本。フル 570 passed**（基準557 +13）。**歯の確認＝機構を7通りに潰して、狙ったテストだけが赤くなることを確認済み**（無敵を外す→⑤⑨／接触スキップを外す→⑥／攻撃スキップを外す→⑦／`tickHide` を止める→④⑤⑦／`tickLeap` を止める→⑧⑨⑩／`move:'air'` を外す→①⑪／`zigzag` を外す→①⑫）。
        - **▶ 実画面で自分で見つけて直した欠陥（[[judge-obvious-visual-defects-yourself]]）＝滞空の影が真っ黒（`rgba(0,0,0,.42)`）でダンジョンの暗い床の上では何も見えなかった。** 縁に薄い明色（`rgba(225,235,255,.3)`）を足して、明るい草地でも暗い石床でも輪郭が出るようにした。
        - **▶ テストで踏んだ落とし穴**＝エディタのタイルパレットは**初期状態でパネルが畳まれていて visible にならない**（DOM には在る）∴`waitForSelector` は `{state:'attached'}` で待つ（visible で待つと 30 秒タイムアウトで落ちる）。
        - ❌ 失効（2026-08-14・k-3b 完了）＝旧記述「▶ 残り＝k-3b（実スプライト3体）／スプライトは既存絵のエイリアス（`burrowWorm`←lurkShark／`leapSpider`←chaser／`batSwarm`←fishSchool）」。**今どうすべきか＝3体とも専用の 32×32 を2フレームずつ持っている**（下記「k-3b 完了記録」）。3種は**まだ本編レイヤーに置いていない**（配置は 5.5m）＝スペック⑬がそれを不変条件として固定している（置いたらこの1本を配置検証に書き換える）＝この点は k-3b でも変えていない。
      - **✅ k-3b 完了記録＝#1 地中蟲 `α`／#2 跳躍蜘蛛 `β`／#9 コウモリ群 `ξ` の実スプライト（2026-08-14・🧠 Opus）。**
        - **成果物＝3体 × 歩行2フレーム＝6枚（32×32）＋専用パレット3本**（`shared/sprites-enemies.js`）。**向き別9〜12枚は要らない**＝3体とも `ENEMY_META` に `directional` を持たない＝GUIDE §2 の `D/R/U/Atk/Guard` 命名は directional な敵だけの要件（#7 剣獣9枚・#8 骸骨12枚との違いはここ）。1エントリに2枚入れると `shared/sprites.js` が `animFrame % frames.length`（400ms の独立 `setInterval`）で交互に出す。
        - **視点はエンジンのフラグから導いた（GUIDE §6-3＝絵の根拠は実装）。** `batSwarm` は `sideView:true`＝`player.x < e.x` で反転する（`game/render-chars.js:420`）∴**素の絵は右向きの横顔・眼は1つだけ**（初回生成は正面顔で眼が2つ見え、反転すると矛盾するので却下した）。`burrowWorm`/`leapSpider` は `sideView` なし＝**一度も反転されない**∴向きに依らない姿（蟲＝正面から見た体節の筒／蜘蛛＝真上から見た姿）で描いた。
        - **⚠️ コウモリは「1匹」で描く（ユーザー方針 2026-08-14＝初回登録の3匹の群れ絵を作り直した）。** 1体の敵＝コウモリ1匹（胴＋Vに立てた翼＋赤い眼1つ＋白い牙）で、**「大量のコウモリ」はステージに何体も配置して表す**（配置は 5.5m）。1枚の絵に群れを詰めると (a) 1体を倒したのに絵の中の他の個体が残る＝当たり判定と絵が食い違う (b) 32×32 に3匹を入れると1匹あたり10px 弱で形が読めない。**さらにユーザー指摘2件で作り直した（同日）＝(a) 大きすぎる (b) 羽ばたきに見えない。** (a) セルいっぱいだと「巨大蝙蝠」に見え「小さいが多数で変則的だから厄介」という位置づけと食い違う∴**22×13 に収めた**（グリッドは 32×32 のまま＝`shared/sprites.js drawSprite` は cols×rows の canvas を CSS で伸ばす∴外周の透明がそのまま「小ささ」になる／他2体は 28〜30行＝並べれば明らかに小型）。(b) **「翼が上にある状態／下にある状態」の2枚でパタパタ飛ばす**＝生成物の加工（垂直に潰す・行をずらす）では動きが弱く、胴が翼より大きいせいで翼を下げると虫の脚に見えた∴**全部手描きに切り替えた**（上記）。**小さい模様は縮小では残らない**（60%縮小で眼2×3と牙が1px幅の縦棒になり赤と白が並んで「鼻」に見えた）∴手描き側で眼は 2×2 の赤＋縁1・牙は白1×2 と決め打ちしている。**タイル名/内部名は `ξ`/`batSwarm`/「コウモリ群」のまま**＝改名は読み手（`shared/enemies.js`・`tiles.js`・`tile-sprites.js`・`editor-palette.js`・`sprites-enemies.js`・`tests/hide-window-enemies.spec.js`・`scripts/migrate-test-hide-window-arenas.mjs`＋各文書）を全部直すまでが1セット（[[blade-water-single-source-readers]]）∴別タスクとして提案する。
        - **隠れ・滞空の見た目はスプライトではなく CSS が描く**（`board.css` の `.hiding` ＋ `.hide-burrow`/`.hide-air` の `::after`）∴「潜っている姿」「滞空の影」の絵は持たない＝実体化した姿だけでよい。実機で `class="char-abs hiding hide-burrow"` と土の盛り上がりが出ることを確認済み。
        - **生成は各体1枚だけ・歩行2はグリッド編集で作った**（SPRITE-PIPELINE §3-1★★＝ポーズを生成し直すと体格が変わり 400ms 交替で「ズーム」に見える）。蟲＝体節の境界3本を1行ずらす蠕動（脚で歩く生き物ではない・輪郭と身長は不変・差分36ドット）／蜘蛛＝`.scratch/shift-spider-legs.mjs`（胴の列 x9-22 を固定し左脚群を1行下・右脚群を1行上＝矩形 op では放射状の脚を切り出せない・差分146ドット）／**コウモリだけ生成画像を使わず全部手描きにした**（`.scratch/draw-bat.mjs`＝行スパンから組み立て・22×13）。**理由＝生成物の翼を潰す/ずらす加工では羽ばたきに見えず、しかも生成物は胴が翼より大きいので翼を下げると「脚が生えた虫」になった**（ユーザー指摘 2026-08-14）∴**コウモリは「大きい翼＋小さい胴」で描く**＝胴は 8×7 の塊＋耳1つ、翼は腕（外側の縁）と胴の間を膜で埋めた三角形（細い帯だと触角/脚に見える）、2枚は肩の行（row15）を軸に上下反転した「翼を上げた／下げた」。
        - **▶ 実画面で自分で見つけて直した欠陥2件（[[judge-obvious-visual-defects-yourself]]）＝どちらも「暗い床に沈む」。** (a) コウモリの生成色（1匹版も同じ＝影 `#1f113e`＝相対輝度 23.3・主要面 `#442770`＝50.5）は石床の暗部（`#2a2838`＝41.6）と近く輪郭が沈んだ→紫を一段明るく（影 51.5／主要面 101.6／翼膜の明部 120.6）・眼は既存の赤 `#e04040` に揃えた。(b) 蜘蛛の生成色（胴 `#533261`＝60.4）は石床の**明部**（`#3a3848`＝57.6）と輝度差が3しかなく胴が沈んだ→一段上げて 58.6/75.8/94.0（＝コウモリの翼膜 101.6・蟲の体 98.8 と同水準）。**(b) は登録データ直並べの図（下記）で見つけた**＝k-3a の「滞空の影が真っ黒」と同じ種類の欠陥＝**新しい絵は必ず床の色と輝度で比べる**。
        - **登録は機械的に流し込む＝`.scratch/emit-k3-sprites.mjs`**（`quant-worm2f/spider2f/bat1f.json` からパレット行と3宣言を差し替え・再実行で自分のブロックを置換する冪等）。6×32行を手で書き写さない（転記ミス防止＝k-2/③と同じ作法）。
        - **確認は2枚（GUIDE §6-5）＝①登録データ直並べ `.scratch/shot-enemy-frames.mjs`→`shots/k3-registered.png`（`shared/sprites-enemies.js` を import し名前どおり並べる・背景は石床の明暗チェッカー）／②実機 `.scratch/shot-k3.mjs`→`shots/k3-*.png`（`test_mechanics[29,0]/[30,0]/[31,0]`・**ファイル名は意図でなく実データ**＝`hidden`/`leapPhase` の実値から作る）。** フルスイート **570 passed**（基準570・回帰なし）。**⚠️ 1回目の全走で深洋Oの潮パズル3本が落ちたが、単独実行では 13/13・11/11 緑で再走も 570 passed＝並列負荷の flaky**（本変更はスプライトデータのみ＝潮に触れていない）。
        - **▶ 判明した抜け＝`windup`（跳躍の溜め）が画面に出ていない∴k-3c を挿入した**（上記キュー）。
      - **やること：** (1) `shared/enemies.js` の `ENEMY_META` に15種を追加する（[[blade-enemy-tables-derive-from-meta]]＝タイル一覧・脅威度・エリート判定は `ENEMY_META` から導出される単一ソース∴ここに足せば表は自動追従。手書きの敵表を別に作らない）。(2) 各敵のスプライト（上記 OpenAI パイプライン＝`shared/sprites-enemies.js` 等・[[blade-tile-sprite-single-source]]＝タイル→スプライトは `shared/tile-sprites.js` が単一の真実・絵文字使わない）。(3) 行動 AI（`game/enemy-ai.js`＝上記の共通機構）。(4) 弱点属性が要るなら `weakness` 定義。(5) **最強格 #7 剣獣を `dark_tower [1,2]` に集める**＝現状の脅威度合計は D1(24.0)→D7(36.0)→dark_tower[1,2](48.0) の単調増加（`tests/dungeon-key-gate.spec.js ⑨`）＝新最強格で 1,2 を組み直しても**この単調増加を壊さない**（D7 < 1,2 を維持）。移行は `scripts/migrate-key-room-d7-tower.mjs`（境界不変・脅威度単調増加・敵密度の逃げ場を書き込み前に assert する自己検査型）を流用/拡張する。**✅ 完了（2026-08-12）＝`scripts/migrate-key-room-dt12-sword-beast.mjs`（剣獣×5＝50.0・柱で射線を切る・BFS 接続と `enemyDirs` 一致も assert）。詳細は下記 k-2 完了記録。**
      - **✅ 新規タイル文字の割り当てを確定した（2026-08-11）＝新規15種はギリシャ文字を使う。** ❌ 失効（2026-08-11・下記の実測で覆った）＝旧記述「実装時に空き文字を確認し衝突させない」だけでは足りなかった。**実測＝ASCII の空きは残り14個しか無い**（英字 A-Z は全て使用済み／空きは `j q y z 0` と記号 `" + , - ? _ \` } '` のみ）＝敵15種でちょうど使い切り、今後のギミック/地形タイルに1文字も残らない。∴**新規敵は非 ASCII（ギリシャ文字）に移す**（タイル文字はコード上「文字列の1要素」としてしか扱われない＝`charCodeAt` や `A-Z` の文字クラス判定は無い∴BMP 1文字なら動く。実証済み＝`θ`/`μ` が実ゲーム・エディタ・JSON 保存で動作）。**ラテン文字と紛らわしい字形（`ε ι κ ν ο ρ τ υ γ χ` と大文字 `Α Β Ε Ζ Η Ι Κ Μ Ν Ο Ρ Τ Υ Χ`）は除外する**＝マップのダンプを目で読むとき `o/ο`・`v/ν` を見間違えるのを防ぐ。
        - **予約表（ロスター番号→タイル文字。実装時にこの表どおりに取る＝取り合いを起こさない）：**
          `1 地中蟲=α` / `2 跳躍蜘蛛=β` / `3 分裂スライム=δ` / `4 盾騎士=ζ` / `5 術士=η` / `6 爆弾鬼=λ` / **`7 剣獣=μ`（実装済み）** / `8 骸骨剣士=θ`（実装済み・`'k'` から移行） / `9 コウモリ群=ξ` / `10 ブーメラン鬼=π` / `11 ルピー喰い=σ` / `12 火吐き亀=φ` / `13 呪い火=ψ` / `14 突進猪=ω` / `15 毒沼ヒル=Γ`
          （安全な小文字は `α β δ ζ η θ λ μ ξ π σ φ ψ ω` の14個ぴったり∴15種目だけ大文字 `Γ` を使う。既存タイルにギリシャ文字は1つも無いことを確認済み＝衝突なし。）
        - **⚠️ `'k'`→`'θ'` の移行はマップデータも書き換える**（`scripts/migrate-test-sword-beast.mjs` が全レイヤーを走査して実施済み＝該当1セル `test_mechanics[0,0] (3,5)`。以後 `'k'` は空き文字に戻る）。
      - **テスト：** 新規敵ごとに (a) `ENEMY_META` から導出される表に載ること (b) スプライトが引けること (c) AI の行動（近接/遠隔/突進/分裂/瞬間移動/ガード 等）が意図どおり動くこと（新機構は歯のあるテストにする＝機構を潰すと赤くなる）。1,2 は `tests/dungeon-key-gate.spec.js ⑨`（脅威度単調増加＋全滅で鍵出現）を新構成に合わせて更新。**✅ 完了（2026-08-12・剣獣の巣に合わせて2本追加＝下記 k-2 完了記録）。**
      - **完了条件＝(1) 陸上通常敵が18種（新規15種が `ENEMY_META`・スプライト・AI とも実装済み） (2) dark_tower [1,2] に最強格(#7剣獣)が集まり脅威度の単調増加が保たれる (3) ユーザーが 1,2 を実プレイして「歯応えのある最終戦闘」と判定＝これで 5.5j の 1,2 も合格になる。**
      - **▶ 配置は別タスク（下記 5.5m）＝敵種ができた後に各ステージへ進行順で配置する。**

- [ ] **5.5m. 新規敵15種を各ステージへ配置する（🧠 Opus）**。**この位置の理由（2026-08-10 ユーザー指示）＝5.5k で敵種を実装しただけでは"カタログにあるが世界に居ない"状態＝プレイヤーは出会わない。∴敵種の実装（5.5k）が済んだ直後に、進行順で難化するよう各ダンジョン/フィールドへ配置する。5.5j の残（dark_tower [1,2]）は 5.5k で 1,2 に最強格を集めた時点で片付くが、他の新敵を世界に出すのはこのタスク。**
      - **やること：** (1) 各ダンジョン/フィールドの位相と難度に合う敵を選んで配置する（進行順＝序盤は低脅威の新敵〈地中蟲・跳躍蜘蛛・コウモリ群〉、中盤〜終盤に高脅威〈盾騎士・術士・爆弾鬼・火吐き亀・剣獣〉）。**配置も一点物として設計する**（[[blade-no-mass-production]]＝どの部屋に何を何体、逃げ場をどう残すか）。(2) 脅威度が進行順で崩れないことを確認（既存の鍵部屋 A 型の単調増加テストと整合）。(3) 弱点持ちの敵は、その弱点道具が入手済みの地点以降に置く（例：bomb 弱点の分裂スライムは爆弾入手後）。
      - **⚠️ 敵配置は接続性/整合性チェッカーの対象外＝実プレイ判定が主。** 配置後にユーザーが進行順に遊んで「敵の多様性を感じる／理不尽な密度でない」を判定する（＝このタスクの合否）。
      - **テスト：** 配置した各ステージが 0 pageerror でロードすること・敵タイルが `ENEMY_META` に存在すること（未定義タイルの混入検出）。
      - **完了条件＝新規15種が全て最低1ステージに配置され、ユーザーが進行順プレイで多様性と難化を確認する。**

- [x] **5.5（親タスク・上記 5.5a〜5.5j に分割済み・2026-08-12 完了）。鍵の関門を `killAll` から本物の難易度へ強化する（🧠 Opus）**。**✅ 完了＝子タスク 5.5a〜5.5j すべて完了し、5.5j（10室の通し実プレイ）でユーザー判定が全室 OK になった（2026-08-12）。** 元要件「あまりに簡単に鍵を取得できたらゲームとして成り立たない」に対し、10鍵は道具型3・倉庫番型3・合成型2・戦闘型2 の配分で進行順に難化する形に置き換わった。**⚠️ 未完で残るのは 5.5k（敵カタログ15種の実装）と 5.5m（世界への配置）＝どちらも「関門の難易度」ではなく敵の品揃えの話∴この親タスクの合否とは別軸。****この位置の理由（2026-08-05 ユーザー確定＝5番の着手時に「まず killAll で背骨を通す→後で強化」を選択）＝背骨（進行可能・クラッシュなし・機械検査で緑）は5番で通った。∴ここからは「難易度」の話＝5番の続きであり、深洋O 残り（6番）より前に置く。** ユーザーの元要件は「あまりに簡単に鍵を取得できたらゲームとして成り立たない。強敵全滅・難しいパズル等の難易度を鍵取得に与える必要がある」で、現状の `killAll` は**その部屋の敵を全部倒すだけ＝10個すべて同じ型**＝要件の下限しか満たしていない。**やること：** (1) 4番の方法論（`PUZZLE-DESIGN.md`）＋4.7 で合格した合成パズル（`test_mechanics 26,0`＝色ゲート/色スイッチ＋石）を**ダンジョンの鍵部屋の語彙に翻訳する**（鍵部屋は10×12 の1画面・ボス部屋への導線上にある＝深洋Oの寄道パズルと制約が違う）。(2) 10個の鍵に**型の配分**を決める（進行順で難化＝D1〜D2 は `killAll` 据え置き／中盤は倉庫番・弓・爆弾の1道具型／終盤 D7・D8・dark_tower は合成型）。**型を混ぜること自体が目的＝同じ型が3連続したら設計をやり直す。** (3) 各鍵部屋を1枚ずつ設計して作り込む（**量産しない**）＋`measure-puzzle.mjs` の4軸で下限を満たすことを確認。(4) 5番で作った不変条件（鍵収支・全鍵に関門・鍵ゼロから全扉開けきり）を**壊さない**ことをチェッカーで確認する。⚠️ **関門を強化しても「歩けば拾える／ブーメランで運べる」の抜け道は塞ぎ続けること**（5番の3点ガード）。⚠️ **`showConditions` のトリガー語彙で足りるかを先に確認する**（`killAll`/`switchOn`/`allSwitchesOn`/`wallBroken`/`torchesLit`/`bushBurned`/`flutePlayed`/`hasItem`＝`conditions.js evaluateConditions`）。倉庫番の「全石を目標に乗せた」は `allSwitchesOn`（ボタン `S` を目標にする）で代用できるが、`stonesPlaced` が要るなら IDEA §51 の実装を含める。
...
- [x] **エディタ対応（2026-08-05 訂正・修正済み）**：`initActiveColor` のステージ設定は実装済みだったが、**パレットへのタイル追加は漏れていた**＝`editor-palette.js` の `PALETTE_CATEGORIES` は明示リスト方式で、`SWITCH_RED`/`SWITCH_BLUE`/`GATE_RED`/`GATE_BLUE` が「ギミック」カテゴリに入っていなかった（当時「tiles.js に足せば自動でパレットに出る」と誤って書いて `[x]` にしていた＝実機で裏取りせずに完了扱いした）。PLAN 4.7 のパズル編集作業でユーザーが実際にエディタを開いて発覚。4タイルをカテゴリに追加し、Playwright で実機確認（パレットに4ボタンとも出現）。
