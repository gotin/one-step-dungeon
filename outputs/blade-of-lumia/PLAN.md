...
#### ▶▶ 次セッションはここから再開する（2026-07-05 更新・設計①〜④完了）

> **土台是正＋★1〜★4 確定＋設計①〜④ すべて完了（下表・下記チェック済み）。次にやるのは設計⑤＝森 or 火山1地域を理想形で試作し密度基準を確立する（🧠 Opus）。いきなり全画面は触らない。**
>
...
>
> **∴ 実体の進行順（新体系）：** ①面白さの言語化 ✅ → ①.5 面白要素カタログ ✅ → ②地域テンプレ設計 ✅（§4） → ③地域固有ギミック ✅（§3） → **④不変条件テスト化 ✅（§7・ratchet／`field-quality.mjs`）** → **⑤【次】1地域を理想形で試作し密度基準（P配分・1画面要素数）を確立＋予告編の道具所持タイミングを 9-1 連鎖表と突き合わせ最終確定（4-4 末尾の残論点）** → ⑥⚡ Sonnet で全地域量産。
>
> **⚠️ 設計⑤ 着手冒頭でユーザー確認（§7-5・前セッションはユーザー離席で暫定デフォルト）：** 論点1（塔まわり `8,0`/`8,1` を2軸ルールの allowlist で除外＝9-2T に委ねる）／テスト戦略（ratchet 採用）／2軸判定（タイル推定方式）の3点。違えばテスト側の定数を差し替えるだけ。
>
> **⚠️ 密度基準が固まる前に全画面は流し込まない**（また塗り絵になる）。⑤試作で数が下がったら `tests/field-invariants.spec.js` の BASELINE を締める。着手前に `FIELD-9-6-DESIGN.md` §4・§7 と本セクションを読む。
>
