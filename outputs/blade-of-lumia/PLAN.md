...
- [ ] **5.5c. D3 鍵部屋＝道具型②（弓の2段）（⚡ Sonnet 可）**。部屋＝`dungeon_3 / 1,0`（**通り抜け**＝東↔南の通行路を1本必ず空ける）。芯＝水で隔離した `Y`①を撃つ→ゲートが開いて `Y`②への射線ができる→鍵の囲いが開く。トリガー＝`switchOn`。⚠️ 弓ゲートは `Y` を水/壁で隔離し撃てる方向に1レーンだけ開ける（接続チェッカーでは迂回を検出できない）。
