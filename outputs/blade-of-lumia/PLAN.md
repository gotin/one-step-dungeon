...
#### チェックリスト（⚡ 流し込み）
- [ ] **9-4a を先に完了**（テーマ地形タイル新設＝下記）。タイルが無いと塗り分けられない。
- [ ] **全 field 画面の `.`FLOOR を地域別地面タイルへ一括置換**（`scripts/migrate-field-terrain.mjs`・地域基準図の region→tile 対応表で決定論的に）。
- [ ] **水100%の内側死に画面を作り直す**（島＋橋 or はしご関門）。外周海ボーダーは維持。
- [ ] **湖(D3周辺11画面)・沼(M地域)に橋 `v` の徒歩導線を敷く**（現状 `v`=0個）。
- [ ] **`check-field-connectivity.mjs` PASS 維持**（村 `7,14` から全徒歩ダンジョン入口へ到達・孤立は外周海/山のみ）。
...
- [ ] **`killEnemy` にドロップ抽選を追加**（雑魚 E/C/F のみ・isBoss は対象外）。抽選テーブル：矢・爆弾・ハート（HP+1〜2）・ルピー・ハズレ。
- [ ] **所持数連動の確率**：矢/爆弾は「現在の所持数が少ないほどドロップ率up」（例：`count===0` なら高確率・`count>=max/2` なら低確率 or 0）。**弾切れ時のセーフティネット**として機能させる（詰み防止の要）。
- [ ] **上限クランプ**：ドロップで得た矢/爆弾も 9-5a の `maxBombs`/`maxArrows` でクランプ。
- [ ] **エフェクト**：`spawnDropEffect(r,c,icon,color)`（`combat.js:54`）を流用（💣/🏹/❤/◆）。取得音は `playSound('item')`。
- [ ] **⚠️ 敵の弱点属性との整合：** ドロップは撃破手段に依存しない（剣で倒しても矢が出る）。弓/爆弾を消費して雑魚を倒す運用でも収支がプラスになる率に調整（プレイで詰める）。
- [ ] **テスト**：所持0で撃破→高確率で弾ドロップ（乱数固定 or 多数回試行）／満タン時は弾ドロップしない／ドロップが max を超えない。
