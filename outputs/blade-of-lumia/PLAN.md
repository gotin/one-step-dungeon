...
- [ ] **4.7. `test_mechanics/24,0` をバグなしで可解な合成パズルに作り直す＝色ゲートを「プレイヤーの通路」に使って石4の天井を超える（🧠→⚡ Sonnet＝設計は 2026-08-02 に `PUZZLE-DESIGN.md` §3-2e へ書き出し済み。ただし I4〈両色必須〉が幾何を振っても通らない場合は幾何の作り直し＝🧠 に戻す）**。**▶ 設計は §3-2e が仕様（機構＝「石が塞いだ迂回路を色ゲートで開ける」／不変条件 I1・I1'・I3・I4・I5＝**「関門を閉じても全床へ歩ける」という旧条件（I2）は削除**（門が飾りになる自己矛盾・ユーザー指摘）／合否＝L 単独で比べず I4＋解の細さ／作業単位6段）。実装はそこを読んで進める。****この位置の理由（2026-08-02 ユーザー指摘でキューを是正）＝4.6 の4枚は判定済みで、その判定が「もっと難しいものが必要」だった。その答えが 24,0（石＋色スイッチ/色ゲート）で、24,0 がバグ依存だったから 4.57 でバグを先に直した。∴ 次にやるべきは順序どおり「24,0 を、バグに頼らずクリアできるパズルに直すこと」**（私は一度これを「別座標に新規作成・24,0 はフィクスチャとして温存」と書いたが、ユーザーの意図は **24,0 そのものを可解にする**＝そちらを採る）。5番（鍵の関門）で「鍵に与えられる難易度の上限」を決めるにはこの軸が要る∴5番の前。 **設計方針（PUZZLE-DESIGN §3-2d の原則＋§3-2e の具体設計で確定済み・実装と実測が未了）：** 排他（色）を**石の搬送路**でなく**プレイヤーの立ち位置**の制約にする＝「赤にしないと石Aの押し位置に行けない／青にしないと石Bの押し位置に行けない」。押す順序が色の切替順に縛られ、切替は往復コストを持つ∴順序制約（§6.1）が生きる。**やること：**(1) **先に回帰フィクスチャを `25,0` へ退避**（24,0 が可解になると「解けたら失敗」の assert が成立しなくなる＝現 24,0 の盤面をそのまま 25,0 として書き、`test-stage-keys.js` の `sokoban_color` を回帰用の別名に付け替える。石 (4,4) が赤ゲート (4,5) の隣＝抜け道を実機で踏める幾何はこの盤面しか無い∴捨てない）→(2) 色ゲートがプレイヤー動線だけを跨ぐ幾何を作る（**石のレーンは門を跨がせない**＝門越しに石を渡す解は 4.57 の規則で作れない）→(3) `generate-sokoban-playable.mjs` の合成テンプレを差し替え（現テンプレは ⚠️ 撤回バナー付きで残置）→(4) 4軸測定で激難（L=89・最短解2本）を**質で**超えるか確認＝L だけでなく「両色が必須」（穴の壁化による動的検査）と最短解の細さで判定→(5) `24,0` を上書き（`fixture` フラグを外し §2 下限条件を当てる側に戻す）→(6) 実機で最短手順を再生するテスト＋回帰フィクスチャ（25,0）のテストが緑→(7) ユーザーが実際に解いて判定。**⚠️ 前提＝押し規則は 4.57 の1規則（押した後に入る石の元セルの下地が閉じていたら押せない／**開いた門への押し込みは正当**）。測ったソルバーで解けても実機で手順を再生して初めて成立。**
  - **進捗（2026-08-02・第1セッション）＝(1) と I3 の測定器まで完了。** ✅ (1) 旧盤面を `25,0`（キー `sokoban_gate_push_regression`）へ退避＝migrate/spec/キー表を付替え、回帰5本（Ⓐ〜Ⓔ）が緑。`24,0` には新パズルを書くまで旧盤面が残置（同じ盤面が2枚並ぶ一時状態）。✅ `puzzle-metrics.mjs` に `escapeTest`／`noEscape`（I3）を実装し migrate に接続。**🔴 その実測で判明＝出荷済みの帯4枚は I3 を満たしていない**（`noEscape` 385/4,604/21,252/**激難 103,249＝全状態の約4.4%**）。形＝row2 の口が1つで、その真下に石を押し込むと画面から出られない（石は row0-2 へ押せない）。画面内では回復不能／セーブ→ロードでのみ回復。**帯4枚は EXPECT に値を固定して退行検出だけ**、0 を要求するのは `requireEscape: true` の盤面のみ。**∴ 新盤面の幾何要件に「row2 の口を2つ」を追加**（PUZZLE-DESIGN §3-2e）。**▶ 残＝(2)〜(7)**＝生成器のテンプレ差し替え＋`assertGeometry` に I1・I1' を足し旧 I2 相当の静的検査を削除→生成・測定（バックグラウンド）→`24,0` 上書き→実機再生テスト→ブラウザ→ユーザー判定。
