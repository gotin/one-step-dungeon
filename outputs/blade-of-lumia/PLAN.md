...
> 2. **フィールド地形の品質改善（9-4）＝最優先（2026-07-04 ユーザー指摘「マップがとにかくまだまだ雑すぎる」）：** 地面の地域テーマ塗り分けは 2026-07-04 完了。**残＝水100%の内側死に画面の作り直し・湖/沼の橋 `v` 徒歩導線・各画面の起伏/導線づくり（開けた床にパラパラ置いただけを脱却）。** M2〜M4 は機械充填のままなので画面単位で作り込む。詳細は下記 9-4。
