...
- **進行中タスク：** **Phase 9-1（進行の背骨＝連鎖マップの設計）— ✅ 設計完了（序盤〜終盤の全連鎖を確定）**
- **⚠️ 次やること：** **9-2（ダンジョン作り込み）/9-3（序盤チュートリアル＋ガイド導線）の実装**。まず 9-2 着手前に **dungeon_1（12部屋）を実プレイでレビューし「本物のダンジョンの雛形」として言語化**。並行して**全アイテムの初期所持→報酬への移設**（実データを grep して撤去対象を洗い出す）。**🔑 鉄則：各ダンジョンは「それより前に入手済みのアイテムだけでクリア可能」**にする（自分の報酬アイテムを内部で要求しない）。
