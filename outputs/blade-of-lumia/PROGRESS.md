...
- **進行中タスク：** **Phase 7-4（リスク・リワード設計）**
- **⚠️ 次やること：** **Phase 7-4（リスク・リワード設計）🧠 Opus**。「この扉を開けると強い敵が出るが宝もある」系の選択要素・高難度任意エリアへの報酬配置・ルピーショップなどバランス設計が中心。設計未着手なので Opus 推奨。
- **直近の状態：** Phase 7-3（ハートの器の増設）の実装完了。ダンジョン2〜7のボスルーム宝箱をルピー→ハートの器に変更、dungeon_1 ゴーレムボスルームに宝箱新設、field/3,0 と dungeon_1/4,0 に隠し宝箱を追加（計+9個・合計14個）。上限なし（マップ配置数が自然な上限）。全3テストパス。
