...
- **直近の状態：** Phase 1-5 完了。**飛行移動システム＋暗黒の塔＋ラスボス ザーネル**を実装。終盤フローが一本に繋がった：全収集→祭壇で翼の羽衣→F キー/🪽で飛行→空島の虚空(SKY)を越える→暗黒の塔→ザーネル撃破→エンディング。**全32テストグリーン**（飛行/塔の新テスト3本含む）。実ブラウザでザーネル撃破→エンディング発火＋空島描画を確認。
...
| フェーズ | 状態 | メモ |
|---|---|---|
| Phase 0 技術基盤 | ✅ 完了 | 0-0〜0-6 全完了（テスト基盤・決定論ループ・game.js/sprites.js/editor.js/CSS分割・4エディタ・TS/ビルド判断）。全32テストグリーン |
| Phase 1 ストーリー基盤 | ✅ 完了 | 1-1〜1-5 全完了（名称変更・エンディング・老賢者NPC・終盤フロー分岐・祭壇/翼の羽衣・飛行/暗黒の塔/ザーネル）。終盤フローが一本に繋がった。全32テストグリーン |
