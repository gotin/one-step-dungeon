...
- **進行中タスク：** **Phase 6 全完了 → Phase 7-1 へ**
- **⚠️ 次やること：** **Phase 7-1（剣の段階的強化）🧠 Opus**。バランス設計が未着手なので Opus 推奨。
- **直近の状態：** Phase 6-4（嘘をつくNPC）完了。全121テストで120グリーン（+4本）。**新コードはゼロ**（`work/blade-of-lumia.json` + テスト1本のみ変更）。3体の嘘つきNPCを配置：field 1,1 冒険者・field 2,0 旅人・field 1,0 老人。Phase 6 全タスク（6-1〜6-4）完了。
