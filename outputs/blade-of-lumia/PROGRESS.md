...
- **進行中タスク：** **Phase 6-3 完了**
- **⚠️ 次やること：** **Phase 6-4（嘘をつくNPC）⚡ Sonnet**。テキスト配置のみ、設計不要。
- **直近の状態：** Phase 6-3（フィールドの充実：廃城・廃村エリア）完了。全117テストで116グリーン（+5本）。`field 0,2` を廃城・廃村エリアに作り替え。**新コードはゼロ**（既存 SIGN `i`→`signData` 機構を再利用）、`work/blade-of-lumia.json` + テスト1本のみ変更。
