...
- **進行中タスク：** **Phase 2-1 完了**。次は **Phase 2-2**（各ダンジョンボスの固有化・ダンジョン内容の充実）⚡。
- **直近の状態：** Phase 2-1 完了。dungeon_2〜dungeon_7 の6ダンジョンをマップに追加し8ダンジョン体制を達成。星の欠片9個（DARK_LORD×7体+直接2個）が `calcTotalTriforces()` に自動反映。BFS 到達確認済み。遷移テスト2本追加。**全35テストグリーン**。
