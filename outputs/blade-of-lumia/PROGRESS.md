...
- **進行中タスク：** **Phase 4-5（アイテムの組み合わせギミック）🧠→⚡**。設計フェーズ（Opus）完了 → 次は実装フェーズ（⚡ Sonnet）。
- **⚠️ 次やること（最優先）：** **Phase 4-5 の実装（⚡ Sonnet・①弓矢スイッチから1サブギミックずつ）**。設計は DECISIONS.md「Phase 4-5（設計）」＋ PLAN.md 4-5 に書き出し済み。共通核＝「投擲物/爆弾がタイル状態に作用する」。①弓矢で SWITCH を撃つ（ラッチ）→ ②TORCH タイル＋ブーメラン炎運搬 → ③爆弾点火＋`torchesLit` トリガー、の順。
- **直近の状態：** Phase 4-5 の設計を確定（コード変更なし）。3サブギミックを「既存 projectileTick/boomerangStep/explodeBomb/showConditions に最小分岐を足す」共通核に集約。新タイル `TORCH('Y')`・新 Set `litTorches`/`shotSwitches`・新トリガー `torchesLit` を導入する方針。**全81テストグリーン（変更なし）**。
...
| Phase 4 サブアイテム | 🚧 進行中 | 4-1（はしご）＋4-1b/c＋4-2（笛）＋4-3（ロウソク）＋4-3b（炎ダメージ）＋4-4（ブーメラン回収）完了。ブーメラン回収：柵を貫通して鍵/ルピーを取得、パズル配置済み。全81テストグリーン。残：4-5 組合せギミック |
| Phase 5 謎解き | 🔲 未着手 | |
| Phase 6 世界観・NPC | 🔲 未着手 | |
| Phase 7 成長システム | 🔲 未着手 | |
| Phase 8 やりこみ・UX | 🔲 未着手 | |
