...
- **進行中フェーズ：** **Phase 1（ストーリー基盤）→ ほぼ完了**
- **進行中タスク：** 次は **Phase 2-1**（8ダンジョンへの拡張）⚡。Phase 1（1-1〜1-5）は全完了。
- **直近の状態：** Phase 1-5 完了＋飛行拡張。**飛行移動システム＋暗黒の塔＋ラスボス ザーネル**を実装し、飛行で自然物（木・茂み・柵）も飛び越え可に拡張（山・建造物は不可）。終盤フローが一本に繋がった：全収集→祭壇で翼の羽衣→F キー/🪽で飛行→空島の虚空(SKY)を越える→暗黒の塔→ザーネル撃破→エンディング。**全33テストグリーン**（飛行/塔の新テスト4本含む）。実ブラウザでザーネル撃破→エンディング発火＋空島/木上飛行を確認。
...
| Phase 2 8ダンジョン | 🔲 未着手 | 次フェーズ。現状ダンジョンは dungeon_1 / cave_1 / dark_tower の3つ |
| Phase 3 アクション深化 | 🔲 未着手 | |
| Phase 4 サブアイテム | 🔲 未着手 | |
| Phase 5 謎解き | 🔲 未着手 | |
| Phase 6 世界観・NPC | 🔲 未着手 | |
