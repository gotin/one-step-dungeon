...
- **進行中タスク：** **Phase 4-4（ブーメランでアイテム回収）✅ 完了**。次は **Phase 4-5（アイテムの組み合わせギミック）🧠→⚡**。
- **⚠️ 次やること（最優先）：** **Phase 4-5（アイテムの組み合わせギミック）🧠→⚡**。弓矢でスイッチを撃つ・ブーメランで炎を操作するなどの組み合わせパズル。設計未着手なので Opus 推奨。PLAN.md 4-5 参照。
- **直近の状態：** ブーメランによるフィールドアイテム回収を実装。`boomerangStep` に `collectFieldItem` 呼び出し追加、鍵K・ルピーr/R に対応。field `1,1` にブーメランパズル配置（柵に囲まれた鍵・施錠扉・宝箱）。**全81テストグリーン**（77→81）。
