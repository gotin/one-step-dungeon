...
- **進行中フェーズ：** **Phase 0 全完了 🎉 → Phase 1（ストーリー基盤）へ**
- **進行中タスク：** 次は **Phase 1-1**（テキスト・名称変更：「トライフォースのかけら」→「星の欠片」＋エンディング書き直し）⚡
- **直近の状態：** Phase 0-6（TypeScript・ビルドの検討）を判断完了。本番 Vercel は `outputs/` を素ファイルのまま静的配信（ビルドなし・4ゲーム同居）と確認。Playwright + CDP で本番同等の未バンドル構成の起動時間を計測（RTT のみ注入・帯域無制限で import 連鎖の影響を切り出し）：time-to-board は RTT 0/30/75ms で 269/482/817ms。ボトルネックは転送量（gzip後70KBで軽量）ではなく JS27本の import 連鎖×RTT。`vite build` で 300〜500ms 短縮見込みだが、無ビルド運用の手軽さ喪失・Vercel 設定整合コストに見合わず **plain ESModule 維持と判断 → Phase 0 クローズ**。DECISIONS.md に計測値と再判断トリガーを記録。
...
| Phase 1 ストーリー基盤 | 🔲 未着手 | 次はここ（1-1 名称変更・エンディング書き直し）|
