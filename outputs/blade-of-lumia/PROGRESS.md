...
- **進行中タスク：** **Phase 5-3（敵を使ったパズル）🧠 Opus**
- **⚠️ 次やること：** **Phase 5-3 の設計**。「敵をスイッチの上に誘導する」パズル設計。敵AIが石を踏んで押す・スイッチに乗るなどの仕掛け。設計未着手なので 🧠 Opus 推奨。
- **直近の状態：** Phase 5-2（隠し通路・隠し入口）実装完了。全99テストグリーン。3つの隠しギミックを配置（爆弾壊し壁・看板フェイク・ロウソク草燃やし→新レイヤー hidden_cave）。
...
### 2026-06-21 — Phase 5-2（完了・Sonnet）：隠し通路・隠し入口 3種配置 + テスト4本
