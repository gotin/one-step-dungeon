...
- **進行中タスク：** **Phase 9-1（進行の背骨＝連鎖マップの設計）— 序盤確定済み・中盤/終盤は未確定**
- **⚠️ 次やること：** **9-1 の続き（中盤・終盤の連鎖をユーザーと対話的に詰める）**。中盤＝D3/4/6 を任意順・はしご/弓矢/ロウソク/爆弾の分配。終盤＝D5・cave_1・D7→祭壇→塔。＋全アイテムの「初期所持→報酬」移設方針。詰めたら PLAN/DECISIONS に記録 → その後 9-2（ダンジョン作り込み）/9-3（チュートリアル動線）の実装へ。
- **直近の状態（2026-06-23）：** **Phase 2「8ダンジョン完了」は器だけだったと判明・計画を見直し**。本物のダンジョンは dungeon_1（12部屋）のみ、残りは2部屋スタブ。進行設計（背骨）が無かった。**ハイブリッド型の連鎖**で作り直すことを決定し、**序盤（村→剣半強制ピックアップ→D1で木の盾→NPC門番→D2でブーメラン）を確定**。PLAN.md に Phase 9 新設・DECISIONS.md に設計記録。
