...
- **進行中タスク：** **Phase 0-4 完了**（必要なスプライト・モーション洗い出し → `SPRITES_NEEDED.md`）→ 次は **Phase 0-5：エディタ機能拡張**（スプライトエディタ／キャラ・アイテム定義エディタ）
- **直近の状態：** Phase 1 以降で必要なスプライト・モーションを企画視点で洗い出し、`SPRITES_NEEDED.md` を新設。プレイヤーモーション（剣振り/被ダメ/チャージ/飛行/やられ）・テーマ別タイル（パレット差し替え方針）・新ボス/敵（ザーネル最優先・大型2×2ボス）・新アイテム/エフェクト（翼の羽衣・はしご・笛・剣ビーム等）を優先度付きで整理。**最優先5点**（翼の羽衣／古代の祭壇／ザーネル／飛行モーション／星の欠片）を抽出し、Phase 0-5 スプライトエディタへの要件（対応グリッドサイズ・マルチフレーム・パレット差し替え・反転・エクスポート）をフィードバックとしてまとめた。コード変更なし（ドキュメントのみ）。
...
| フェーズ | 状態 | メモ |
|---|---|---|
| Phase 0 技術基盤 | 🚧 進行中 | 0-0 ✅ / 0-1 ✅ / 0-2 ✅ / 0-2b ✅ / 0-3 ✅ / 0-3b ✅ / **0-4 ✅（SPRITES_NEEDED.md 洗い出し）**。**残りは 0-5（エディタ機能拡張）・0-6（TS/ビルド検討）。** |
