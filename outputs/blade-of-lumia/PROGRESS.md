...
- **進行中タスク：** **Phase 6-2 完了**
- **⚠️ 次やること：** **Phase 6-3（フィールドの充実：廃城・廃村・壊れた遺跡エリア）⚡ Sonnet**。エディタでのマップ作成が中心で設計不要なので Sonnet 推奨。
- **直近の状態：** Phase 6-2（世界の歴史を語る石碑）完了。全112テストグリーン（+4本）。ザーネルの悲劇の過去を語る「ザーネルの記憶」全8断片（其の一〜七＋終章）を dungeon_1〜7 と暗黒の塔の entry room に石碑として配置。**新コードはゼロ**（既存 SIGN `i`→`signData` 機構を再利用）、`work/blade-of-lumia.json` のみ変更。
