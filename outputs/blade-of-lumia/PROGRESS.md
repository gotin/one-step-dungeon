...
- **進行中フェーズ：** **Phase 5（謎解き・パズルギミック）→ Phase 5 全完了**
- **進行中タスク：** **Phase 6（世界観・NPC・テキスト）に着手予定**
- **⚠️ 次やること：** **Phase 6-1（NPCの台詞充実）⚡ Sonnet**。各村・フィールドNPCに「噂話」台詞を追加・ボス撃破後に変化する台詞。テキスト作成・配置が中心なので ⚡ Sonnet 推奨（設計不要）。
- **直近の状態：** Phase 5-3（敵を使ったパズル）実装完了。全101テストグリーン。「敵が石を押す」方式で、追跡してくる敵に石をボタンへ押させてゲートを開く誘導パズルを実装（dungeon_1 `5,0`）。
