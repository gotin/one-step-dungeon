...
- **進行中タスク：** **Phase 2-2 完了**。次は **Phase 2-3**（ダンジョン進行のヒント：石碑・NPCの噂話）⚡。
- **直近の状態：** Phase 2-2 完了。dungeon_2〜7 を完全リデザイン。各ダンジョンにテーマ地形・敵・宝箱・石碑を配置。ステージ構成を entry `0,0`（右端開口→boss）+ boss `1,0`（isBossRoom=true・DARK_LORD X・DOORWAY_BOSS `:`）に統一。デバッグ用一時テスト削除。**全37テストグリーン**。
...
| Phase 2 8ダンジョン | 🚧 進行中 | 2-1 完了（dungeon_2〜7 追加で8体制達成・35テストグリーン）。次は 2-2（ボス固有化・内容充実） |
| Phase 3 アクション深化 | 🔲 未着手 | |
| Phase 4 サブアイテム | 🔲 未着手 | |
| Phase 5 謎解き | 🔲 未着手 | |
| Phase 6 世界観・NPC | 🔲 未着手 | |
