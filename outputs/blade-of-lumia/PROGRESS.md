...
- **進行中タスク：** **Phase 4-3b（ロウソクの炎で敵にダメージ）✅ 完了**。次は **Phase 4-4（ブーメランでアイテム回収）⚡ Sonnet**。
- **⚠️ 次やること（最優先）：** **Phase 4-4（ブーメランでアイテム回収）⚡ Sonnet**。ブーメランが通過したマスのアイテム（鍵・ルピー）を回収できる機能。PLAN.md 4-4 参照。
- **直近の状態：** ロウソクの炎ダメージを実装。`playCandle()` に前方セルの敵検索 + `dealDamageToEnemy(e, CANDLE_FIRE_DMG, 'fire')` を追加。攻撃種別 `'fire'` を新設し、`CANDLE_FIRE_DMG=3`（`constants.js`）。氷のリヴァイアサン(L)と古森の巨人(O)の弱点を `fire`(×3/×2) に設定。茂みの有無・燃焼済みに関わらず敵判定は毎回実行。**candle テスト6本（+2本）＋全体77テストすべてグリーン**。
...
| Phase 4 サブアイテム | 🚧 進行中 | 4-1（はしご）＋4-1b/c＋4-2（笛）＋4-3（ロウソク）＋4-3b（炎ダメージ）完了。炎ダメージ：前方敵に `dealDamageToEnemy('fire')`・氷/森ボス弱点設定。全77テストグリーン。残：4-4 ブーメラン回収 / 4-5 組合せ |
| Phase 5 謎解き | 🔲 未着手 | |
| Phase 6 世界観・NPC | 🔲 未着手 | |
| Phase 7 成長システム | 🔲 未着手 | |
| Phase 8 やりこみ・UX | 🔲 未着手 | |
