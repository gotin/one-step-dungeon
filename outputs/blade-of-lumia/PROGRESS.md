...
- **進行中タスク：** **Phase 0-5 スプライトエディタ 完了**（🎨タブ・`editor-sprite.js`）→ 次は **0-5 残り**（キャラクター定義エディタ／アイテム定義エディタ／タイルバリエーション設計支援）。または 0-2b 残り・0-6（TS/ビルド検討）
- **直近の状態：** Phase 0-5 のうち最優先の **スプライトエディタ** を実装。`editor/index.html` に🎨スプライトタブ＋`#view-sprite` を追加、自己完結モジュール `editor/editor-sprite.js` を新設（editor.js とは showView 経由で疎結合）。機能：グリッド可変（32²/16²/12×16/8²/6²/64²）・パレット編集（色追加/変更・index0=透明）・ドット描画（ペン/消しゴム）・複数フレーム（追加/複製/削除/再生プレビュー）・左右反転・既存スプライト読込・`sprites-*.js` 配列形式エクスポート＆コピー。スタイルは `editor/editor-sprite.css`（新規）に分離。editor スモークテスト2本追加。**全19テストグリーン**（デグレなし）。
...
| フェーズ | 状態 | メモ |
|---|---|---|
| Phase 0 技術基盤 | 🚧 進行中 | 0-0〜0-4 ✅ / **0-5 🚧（スプライトエディタ ✅ / キャラ・アイテム定義・タイル支援は未着手）** / 0-6 未着手 |
